...
from typing import (
    Dict, Callable, Any, Optional, List, TypeVar, Generic,
...
from aio_pika.abc import (
    ...
    AbstractQueue, AbstractIncomingMessage
)
...
class MessageBusMetrics:
    ...
    def avg_publish_latency_ms(self) -> float:
        return sum(self.publish_latency_ms[-100:]) / max(len(self.publish_latency_ms[-100:]), 1)
    
    @property
    def avg_consume_latency_ms(self) -> float:
        return sum(self.consume_latency_ms[-100:]) / max(len(self.consume_latency_ms[-100:]), 1)
...
class MessageBus:
    ...
    async def publish_event(
        ...
        try:
            ...
            logger.error(f"🔴 Circuit breaker open, rejecting publish to {exchange_name}")
            return False
        ...
        async def message_handler(message: AbstractIncomingMessage) -> None:
            ...
            async with message.process(ignore_processed=auto_ack):
                try:
                    ...
                    if idempotency_key and await self._is_duplicate_async(idempotency_key):
                    ...
                    if retry_count < max_retries:
                        ...
                        logger.warning(f"Retrying message (attempt {retry_count + 1}/{max_retries}): {e}")
                        ...
                        try:
                            ...
                            if target_exchange:
                                ...
                                logger.error(f"Cannot retry: exchange '{original_exchange}' not found, dead-lettering")
                                self.metrics.messages_dead_lettered += 1
                            
                            await message.ack()
                        except Exception as retry_err:
                            logger.error(f"Failed to re-publish for retry, dead-lettering: {retry_err}")
                            self.metrics.messages_dead_lettered += 1
                            await message.reject(requeue=False)
                    else:
                        logger.error(f"Max retries ({max_retries}) exceeded, dead-lettering: {e}")
                        self.metrics.messages_dead_lettered += 1
                        await message.reject(requeue=False)
        ...
        if self._redis:
            try:
                ...
                if loop.is_running():
                    ...
                    future = asyncio.ensure_future(self._is_duplicate_redis(idempotency_key))
    ...
    def _mark_processed(self, idempotency_key: str) -> None:
        ...
        if len(self._processed_ids) > 10000:
            ...
            self._processed_ids = {
                k: v for k, v in self._processed_ids.items()
                if v > cutoff
            }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self._is_connected:
            try:
                await self.channel.declare_queue("health.check", passive=False, auto_delete=True)
