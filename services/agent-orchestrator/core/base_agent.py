...
class BaseAgent(ABC):
    ...
    async def start(self) -> None:
        ...
        async with self._lifecycle_lock:
            if self.status in (AgentStatus.ACTIVE, AgentStatus.IDLE, AgentStatus.PAUSED):
                if self._processor_task and not self._processor_task.done():
                    return
            if self._needs_cleanup or self._consumer_tags or any(
                not task.done() for task in self._active_tasks
            ) or (self._processor_task and not self._processor_task.done()):
                raise RuntimeError("Previous agent run has not stopped; check health and stop again")
            try:
                ...
                for queue_name, tag in list(self._consumer_tags.items()):
                    try:
                        ...
                        self.logger.error(f"Partial startup cancellation failed: {cancel_error}")
        ...
        async with self._lifecycle_lock:
            ...
            try:
                ...
                if not self._message_queue.empty():
                    raise RuntimeError("Accepted messages remain unprocessed; stop is incomplete")
                await self.cleanup()
                self._needs_cleanup = False
    ...
    async def _drain_tasks(self, tasks: set, deadline: float) -> None:
        ...
        if pending:
            raise TimeoutError("Agent work is still draining; check health before retrying stop")
        for task in done:
            # Cancellation of a worker is a failed drain, not cancellation of
            # the operator's HTTP request (which must receive a clear failure).
            if task.cancelled():
                raise RuntimeError("Agent task was cancelled before drain completed")
            task.result()

    async def pause(self) -> None:
        """Pause message processing"""
        if self.status not in (AgentStatus.ACTIVE, AgentStatus.IDLE, AgentStatus.DEGRADED, AgentStatus.PAUSED):
    ...
    def _on_processor_done(self, task: asyncio.Task) -> None:
        ...
        if task.cancelled():
            ...
            if not self._shutdown_event.is_set():
                self.status = AgentStatus.ERROR
                self.metrics.record_error("Message processor was unexpectedly cancelled")
            return
