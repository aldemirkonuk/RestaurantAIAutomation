...
class SommelierAgent(BaseAgent):
    ...
    async def initialize(self) -> None:
        ...
        if self.mock_mode:
            ...
            try:
                ...
                self.logger.info("✓ Gemini client initialized (google-genai SDK, model: gemini-2.0-flash)")
            except ImportError:
            ...
            await self.publish(
                ...
                message_body={
                    ...
                    "payload": {
                        "query": query,
                        "response": response,
                        "wines": [w.model_dump() if hasattr(w, 'model_dump') else w for w in wines[:5]],
    ...
    async def _handle_enrichment_request(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            wine = await self.database.wine_library.get_by_id(wine_id) if wine_id else None
    ...
    async def _suggest_wines_for_event(
        ...
        """
        wine_types = self.event_wine_mapping.get(event_type, ["red", "white"])
