...
class RecurringOrderAgent(BaseAgent):
        ...
        try:
            ...
            for order in recurring_orders:
                try:
                    ...
                    logger.error(f"Error processing recurring order {order.get('id')}: {e}")
    ...
    async def _send_executed_notification(self, order: Dict):
        ...
        try:
            ...
            await self.notification_agent.send_notification(notification_data)
            logger.info(f"Sent execution notification for recurring order {order['id']}")
    ...
    async def _update_next_order_date(self, order: Dict):
        ...
        try:
            ...
            next_date = self._calculate_next_date(current_date, frequency, frequency_day)
