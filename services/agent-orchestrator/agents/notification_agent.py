...
Please try again or add items to inventory manually.""",
    ...
    async def _track_notification_delivery(
        ...
        try:
            result = (
                self.database.supabase.table("notification_deliveries")
                .insert(
                    {
                        "event_id": event_id,
            ...
            self.logger.warning(
                f"Failed to insert notification_deliveries pending row: {e}"
            )
        return None
    ...
    async def _mark_delivery_sent(self, notification_id: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to update notification_deliveries to sent: {e}"
            )
    ...
    async def _mark_delivery_failed(self, notification_id: str, error: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to update notification_deliveries to failed: {e}"
            )
    ...
    async def process_message(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Idempotency check failed, proceeding (fail open): {idempotency_exc}"
                )
            ...
            if _effective_event_id in self._dlq_escalated:
                self.logger.warning(
                    f"Notification already DLQ-escalated, dropping: {_effective_event_id}"
                )
                return
            ...
            try:
                ...
                if event_id:
                    await self._mark_processed(
                        event_id, result={"status": "sent", "channel": _primary_channel}
                    )
                ...
                if self._notification_retry_counts.get(_effective_event_id, 0) >= 3:
                    await self._send_to_dlq(
                        ...
                        retry_count=self._notification_retry_counts[
                            _effective_event_id
                        ],
            ...
            self.logger.error(
                f"Error processing notification message: {e}", exc_info=True
            )
            raise
    ...
    async def send_low_stock_alert(self, payload: Dict[str, Any]) -> None:
        ...
        if "sms" in channels and manager.get("phone"):
            ...
            result = await self.sms_client.send_sms(
                manager["phone"], sms_body, priority="high"
            )
    ...
    async def send_negotiation_complete_notification(
        ...
        self.logger.info(
            f"Sent negotiation complete notification for order {order_id} via {channels}"
        )
    ...
    async def send_delivery_confirmation_request(self, payload: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"Sent delivery confirmation request for order {order_id} via {channels}"
        )
    ...
    async def send_high_priority_alert(self, payload: Dict[str, Any]) -> None:
        ...
        message = payload.get(
            "message", "An anomaly has been detected that requires your attention."
        )
    ...
    async def send_vintage_mismatch_alert(self, payload: Dict[str, Any]) -> None:
        ...
        # Get manager
        manager = (
            await self._get_manager_for_restaurant(restaurant_id)
            if restaurant_id
            else None
        )
        ...
        if manager:
            try:
                ...
                self.logger.info("✓ Push notification sent for vintage mismatch")
            except Exception as e:
        ...
        if manager and manager.get("phone"):
            try:
                ...
                self.logger.info("✓ SMS sent for vintage mismatch")
            except Exception as e:
    ...
    async def send_inventory_discrepancy_alert(self, payload: Dict[str, Any]) -> None:
        ...
        actions = [
            {
                "id": "investigate",
                "label": "Investigate",
                "url": f"/inventory/{inventory_id}/investigate",
            },
            {
                "id": "adjust",
                "label": "Adjust Stock",
                "url": f"/inventory/{inventory_id}/adjust",
            },
            {
                "id": "recount",
                "label": "Recount",
                "url": f"/inventory/{inventory_id}/recount",
            },
        ]
        ...
        if manager:
            try:
                ...
                self.logger.info("✓ Push notification sent for discrepancy")
            except Exception as e:
                self.logger.error(f"Push notification failed: {e}")
        ...
        if (
            manager
            and manager.get("phone")
            and discrepancy_type in ["moderate", "major"]
        ):
            try:
                sms_message = (
                ...
                self.logger.info("✓ SMS sent for discrepancy")
            except Exception as e:
                self.logger.error(f"SMS failed: {e}")
    ...
    async def send_one_tap_order_alert(self, payload: Dict[str, Any]) -> None:
        ...
        if manager:
            try:
                ...
                self.logger.info("✓ One-tap order notification sent")
            except Exception as e:
                self.logger.error(f"Push notification failed: {e}")
        ...
        if manager and manager.get("phone"):
            try:
                ...
                self.logger.info("✓ One-tap SMS sent")
            except Exception as e:
                self.logger.error(f"SMS failed: {e}")
    ...
    async def send_wine_type_mismatch_alert(self, payload: Dict[str, Any]) -> None:
        ...
        if manager:
            try:
                ...
                self.logger.info("✓ Push notification sent for wine type mismatch")
            except Exception as e:
                self.logger.error(f"Push notification failed: {e}")
        ...
        if manager and manager.get("phone"):
            try:
                ...
                self.logger.info("✓ SMS sent for wine type mismatch")
            except Exception as e:
                self.logger.error(f"SMS failed: {e}")
    ...
    async def _select_channels(
        self, urgency: str, preferences: Dict[str, Any], notification_type: str
    ) -> List[str]:
        """
        ...
        for ch in channels:
            if await self._check_rate_limit(restaurant_id, ch):
                ...
                self.logger.warning(
                    f"Rate limit exceeded for {ch} on restaurant {restaurant_id}"
                )
    ...
    async def _get_manager_for_restaurant(
    ...
    async def send_recurring_order_executed(self, payload: Dict[str, Any]) -> None:
        ...
        if self.push_service:
            await self.push_service.send_notification(
                ...
                title=template["title"].format(wine_name=wine_name),
                body=message,
                data={"type": "recurring_order_executed", "order_id": order_id},
            )
    ...
    async def send_recurring_order_approval(self, payload: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"Sent recurring order approval request for {recurring_order_id}"
        )
    ...
    async def send_vendor_deadline_reminder(self, payload: Dict[str, Any]) -> None:
        ...
        if hours_remaining < 24 and self.sms_client and manager.get("phone"):
            ...
            result = await self.sms_client.send_sms(
                to_number=manager["phone"], message=sms_message
            )
            results.append(("sms", result))
    ...
    async def send_vendor_deadline_missed(self, payload: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"Sent vendor deadline missed notification for {provider_name}"
        )
    ...
    async def send_invoice_scan_failed(self, payload: Dict[str, Any]) -> None:
        ...
        message = template["message"].format(
            invoice_number=invoice_number, error_message=error_message
        )
        ...
        self.logger.info("Sent invoice scan failed notification")
