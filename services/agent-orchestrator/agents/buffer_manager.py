...
class BufferWindow:
    ...
    def __init__(self, inventory_id: str, window_minutes: int = 30):
        ...
        self.window_end: datetime = self.window_start + timedelta(minutes=window_minutes)
    ...
    def add_sale(self, quantity: int, timestamp: datetime, sale_data: Dict[str, Any]) -> None:
...
class BufferManagerAgent(BaseAgent):
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(f"Initializing Buffer Manager (window={self.buffer_window_minutes}min)")
    ...
    async def _handle_manual_override(self, message: Dict[str, Any]) -> None:
        ...
        if not inventory_id or new_stock_live is None:
            self.logger.error("Manual override event missing inventory_id or new_stock_live")
            return
        ...
        if new_stock_live < threshold_min:
            ...
            estimated_stockout_days = (
                new_stock_live / sales_velocity_7d
            ...
            if in_transit == 0:
                urgency = "critical" if new_stock_live == 0 else "high" if estimated_stockout_days < 2 else "medium"
                ...
                await self.publish(
                    ...
                    message_body={
                        ...
                        "payload": {
                            ...
                            "wine_name": inventory.get("wine_name", "Unknown") if inventory else "Unknown",
                            ...
                            "estimated_stockout_days": round(estimated_stockout_days, 1),
                            "urgency": urgency,
    ...
    async def _periodic_evaluation(self) -> None:
        ...
        """
        self.logger.info(f"Started periodic buffer evaluation (interval={self.evaluation_interval}s)")
        ...
        while True:
            try:
                ...
                if expired_buffers:
                    self.logger.info(f"Evaluating {len(expired_buffers)} expired buffers...")
    ...
    async def _evaluate_buffer(self, inventory_id: str, buffer: BufferWindow) -> None:
        ...
        try:
                        ...
                        await self.publish(
                            ...
                            message_body={
                                ...
                                "payload": {
                                    ...
                                    "sales_velocity_7d": sales_velocity_7d,
                                    "estimated_stockout_days": round(estimated_stockout_days, 1),
                                    "urgency": "high" if estimated_stockout_days < 2 else "medium",
                                },
                            },
            ...
            self.logger.error(f"Failed to evaluate buffer for {inventory_id}: {e}", exc_info=True)
