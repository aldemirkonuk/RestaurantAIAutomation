...
class InventoryEngineAgent(BaseAgent):
    ...
    def _next_sequence(self, aggregate_id: str) -> int:
        ...
        self._event_sequence[aggregate_id] = (
            self._event_sequence.get(aggregate_id, 0) + 1
        )
    ...
    async def _handle_stock_evaluated(self, message: Dict[str, Any]) -> None:
        ...
        if not message_id:
            self.logger.warning(
                "No stable message_id or event_id in message — idempotency bypassed"
            )
        ...
        try:
            ...
            if success:
                ...
                await self.log_decision(
                    ...
                    output={
                        "new_state": self._calculate_inventory_state(inventory).value,
                        "success": success,
                    },
            ...
            if success:
                ...
                await self._mark_processed(
                    ...
                    result={
                        "status": "ok",
                        "inventory_id": inventory_id,
                        "corrected_stock": corrected_stock,
                    },
                )
                await self.log_decision(
                    "manual_correction",
