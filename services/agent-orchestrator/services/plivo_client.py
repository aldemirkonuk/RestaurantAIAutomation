...
class PlivoSMSClient:
    ...
    async def send_sms(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"✅ SMS sent to {to_number} (UUID: {response.get('message_uuid')})")
    ...
    async def _send_via_plivo(self, to_number: str, message: str) -> Dict[str, Any]:
        ...
        return {
            "message_uuid": response[1].message_uuid if hasattr(response[1], 'message_uuid') else None,
            "status": "queued"
        }
    ...
    def _record_sms_sent(self, phone: str) -> None:
        ...
        self.sms_sent_timestamps[phone] = [
            ts for ts in self.sms_sent_timestamps[phone]
            if ts > two_hours_ago
        ]
    ...
    async def verify_delivery_status(self, message_uuid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            message = await loop.run_in_executor(
                None,
                lambda: self.client.messages.get(message_uuid)
            )
            
            return {
                "message_uuid": message_uuid,
                "status": message.message_state if hasattr(message, 'message_state') else "unknown",
                "delivered": message.message_state == "delivered" if hasattr(message, 'message_state') else False
            }
        except Exception as e:
