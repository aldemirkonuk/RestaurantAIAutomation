...
class PushNotificationService:
    ...
    async def send_web_push(
        ...
        notification_payload = {
            ...
            "tag": (
                data.get("tag", "wineops-notification")
                if data
                else "wineops-notification"
            ),
        }
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = await self._send_webpush(
                    subscription_info, json.dumps(notification_payload)
                )
                ...
                logger.error(
                    f"Web Push error (attempt {attempt + 1}/{max_retries}): {e}"
                )
    ...
    async def _send_webpush(
        self, subscription_info: Dict[str, Any], payload: str
    ) -> Dict[str, Any]:
        ...
        await loop.run_in_executor(
            ...
            lambda: webpush(
                ...
                vapid_claims={"sub": self.vapid_email or "mailto:admin@wineops.ai"},
            ),
        )
    ...
    async def send_fcm_notification(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(
                    f"FCM HTTP error (attempt {attempt + 1}/{max_retries}): {e}"
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "web_push_configured": bool(
                self.vapid_private_key and self.vapid_public_key
            ),
