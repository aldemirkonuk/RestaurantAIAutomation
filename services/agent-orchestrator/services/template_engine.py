...
class TemplateEngine:
    ...
    def validate_template(
        ...
        if open_count != close_count:
            result["valid"] = False
            result["errors"].append(f"Unmatched braces: {open_count} open, {close_count} close")
...
class MessageTemplateManager:
    ...
    async def create_template(
        ...
        try:
            ...
            response = await self.database.supabase.table("message_templates") \
        ...
        try:
            ...
            if "body" in updates:
                validation = self.template_engine.validate_template(
                    updates["body"],
                    updates.get("variables", current.get("variables"))
                )
                if not validation["valid"]:
                    raise ValueError(f"Invalid template: {validation['errors']}")
            ...
            logger.info(f"Updated template: {template_id} (version {updates['version']})")
    ...
    async def delete_template(self, template_id: str) -> bool:
        ...
        try:
            await self.database.supabase.table("message_templates") \
                .update({"is_active": False, "deleted_at": datetime.utcnow().isoformat()}) \
                .eq("id", template_id) \
                .execute()
        ...
        try:
            ...
            if restaurant_id:
                if include_global:
                    ...
                    query = query.or_(f"restaurant_id.eq.{restaurant_id},restaurant_id.is.null")
                else:
                    query = query.eq("restaurant_id", restaurant_id)
    ...
    async def render_template(
        ...
        try:
            # Get template
            template = await self.get_template(template_id=template_id, restaurant_id=restaurant_id)
            if not template:
                raise ValueError(f"Template not found: {template_id}")
    ...
    async def preview_template(
        ...
        return self.template_engine.render(template_body, sample_variables, strict=False)
