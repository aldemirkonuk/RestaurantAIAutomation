...
class CheckScannerService:
        ...
        try:
            ...
            if extracted_data.get('wine_sales') and extracted_data.get('wine_cost'):
                extracted_data['profit_margin'] = self._calculate_profit_margin(
                    extracted_data['wine_sales'],
                    extracted_data['wine_cost']
                )
    ...
    def _extract_line_items(self, lines: List[str]) -> List[Dict]:
        ...
        wine_keywords = [
            'wine', 'red', 'white', 'sparkling', 'champagne', 'prosecco',
        ...
        for line in lines:
            ...
            if any(skip in line.lower() for skip in [
                ...
                'server', 'table', 'check', 'bill', 'date', 'time'
            ]):
                continue
            ...
            if match:
                ...
                try:
                    ...
                    is_wine = any(keyword in item_name.lower() for keyword in wine_keywords)
                    
                    # Extract quantity if present
                    qty_match = re.search(r'(\d+)\s*x\s*[\$]?([\d,]+\.?\d{0,2})', item_name)
                    if qty_match:
                        quantity = int(qty_match.group(1))
                        item_name = re.sub(r'\d+\s*x\s*[\$]?[\d,]+\.?\d{0,2}', '', item_name).strip()
                    else:
                        quantity = 1
    ...
    async def batch_process_checks(
        self,
        file_paths: List[str]
    ) -> List[Dict]:
        """
