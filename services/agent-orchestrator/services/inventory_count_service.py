...
class InventoryCountService:
    ...
    async def record_physical_count(
        ...
        """
        logger.info(
            f"Recording physical count for {inventory_id}: {physical_count} bottles"
        )
        ...
        self.database.supabase.table("restaurant_inventory").update(update_data).eq(
            "id", inventory_id
        ).execute()
    ...
    async def _calculate_expected_stock(
        ...
        try:
            sales = (
                self.database.supabase.table("sales_events")
                .select("quantity")
            ...
            sum(s.get("quantity", 0) for s in (sales.data or []))
    ...
    async def get_count_summary(
        ...
        with_discrepancy = [
            i for i in counted if i.get("physical_stock", 0) != i.get("stock_live", 0)
        ]
        ...
        return {
            ...
            "count_completion_percent": round(
                len(counted) / max(len(items), 1) * 100, 1
            ),
        }
