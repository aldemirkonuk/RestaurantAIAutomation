...
class MobileCountSimulator:
    ...
    async def record_count(
        ...
        logger.info(
            f"📱 Count submitted: {inventory_id} = {count} bottles at {location}"
        )
    ...
    async def end_count_session(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_discrepancy_bottles": sum(
                abs(c.get("discrepancy", 0)) for c in discrepancies
            ),
            "counts": counts,
        ...
        logger.info(
            f"📱 Ended count session: {session_id} - {total_counts} items counted"
        )
