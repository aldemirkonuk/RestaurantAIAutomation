...
def notification_agent():
    ...
    a._get_manager_for_restaurant = AsyncMock(
        return_value={
            "id": "mgr-001",
            "phone": "+1415555000",
    ...
    if hasattr(a.sms_client, "send_sms_with_action_buttons"):
        a.sms_client.send_sms_with_action_buttons = AsyncMock(
...
def reporting_agent():
    ...
    inv_chain.execute.return_value = MagicMock(
        data=[
            {
                "id": "inv-uuid-001",
                "wine_name": "Caymus Cabernet 2021",
...
async def test_e2e_01_webhook_to_pos_event(pos_agent):
    ...
    pos_event_calls = [
        ...
        if (
            (
                c.kwargs.get("exchange_name") == "pos.events"
                or c.kwargs.get("exchange") == "pos.events"
            )
            and c.kwargs.get("routing_key") == "pos.sale.completed"
        )
        or ("pos.events" in str(c) and "pos.sale.completed" in str(c))
    ]
...
async def test_e2e_02_pos_event_to_inventory_decrement(inventory_agent):
    ...
    state_changed_calls = [c for c in published if "stock.state.changed" in str(c)]
    assert (
        state_changed_calls
...
async def test_e2e_full_golden_path(
    ...
    assert pos_result.get("status") in (
        "accepted",
        "success",
    ...
    assert notif_result is None or isinstance(
        notif_result, dict
