...
def mock_db():
    ...
    db.supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.single.return_value = provider_chain
    ...
    orders_chain.execute.return_value = MagicMock(data=[{
        "id": "order-uuid-1",
    ...
    db.supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.in_.return_value.order.return_value.limit.return_value = orders_chain
...
async def test_extract_invoice_from_email_text_returns_structured_json():
    ...
    with patch("services.model_clients.get_haiku_client") as mock_hc:
        ...
        with patch("agents.visual_verification_agent.VisualVerificationAgent._extract_invoice_from_email_text",
...
async def test_extract_invoice_from_email_text_fallback_on_haiku_failure():
    ...
    with patch("services.model_clients.get_haiku_client") as mock_hc:
        mock_hc.return_value.messages.create = AsyncMock(side_effect=Exception("timeout"))
        result = await vva._extract_invoice_from_email_text(
...
async def test_unknown_sender_returns_true_when_email_not_in_providers(email_agent):
    ...
    email_agent.database.supabase.table.return_value.select.return_value.eq.return_value.ilike.return_value.limit.return_value = empty_chain
...
async def test_known_sender_returns_false(email_agent):
    ...
    email_agent.database.supabase.table.return_value.select.return_value.eq.return_value.ilike.return_value.limit.return_value = known_chain
