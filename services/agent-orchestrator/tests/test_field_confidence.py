...
def test_should_auto_block_mostly_bad():
    ...
    fc = {f: {"value": "x", "confidence": 0.3, "source": "inferred"} for f in VISION_FIELDS[:12]}
...
def test_should_auto_block_mostly_good():
    ...
    fc = {f: {"value": "x", "confidence": 0.9, "source": "visible"} for f in VISION_FIELDS[:16]}
...
def test_e2e_extraction_to_review_queue():
    ...
    wine1_raw = {
        "wine_name":      {"value": "Barolo Riserva",   "confidence": 0.99, "source": "visible"},
        "producer":       {"value": "Giacomo Conterno", "confidence": 0.97, "source": "visible"},
        "vintage":        {"value": 2016,               "confidence": 0.99, "source": "visible"},
        ...
        "appellation":    {"value": "Barolo DOCG",      "confidence": 0.85, "source": "inferred"},
        "grape_variety":  {"value": "Nebbiolo",         "confidence": 0.93, "source": "inferred"},
        "price_bottle":   {"value": 145.0,              "confidence": 0.99, "source": "visible"},
        "section_name":   {"value": "Italian Reds",     "confidence": 0.99, "source": "visible"},
    }
    ...
    assert len(rejected1) == 0, f"Wine 1 should have 0 rejected fields"
    ...
    wine2_raw = {
        ...
        "sub_region":     {"value": "Bolgheri",          "confidence": 0.62, "source": "inferred"},  # review tier
        "appellation":    {"value": "Bolgheri DOC",      "confidence": 0.42, "source": "inferred"},  # rejected
        "grape_variety":  {"value": "Cabernet Sauvignon","confidence": 0.82, "source": "inferred"},
        "section_name":   {"value": "Super Tuscans",     "confidence": 0.99, "source": "visible"},
    }
    ...
    assert "sub_region" in review2_fields, f"sub_region (0.62) should be in review: {review2_fields}"
    assert "appellation" in rejected2, f"appellation (0.42 < 0.5) should be rejected: {rejected2}"
    # sub_region should also be in accepted (persisted)
    assert "sub_region" in accepted2, "review-tier fields must be persisted (in accepted)"
    assert should_auto_block(fc2) is False

    # --- Wine 3: mostly uncertain fields → auto_blocked ---
    wine3_raw = {k: {"value": "unknown", "confidence": 0.25, "source": "inferred"} for k in VISION_FIELDS}
    # Only wine_name is legible
    wine3_raw["wine_name"] = {"value": "House Red", "confidence": 0.95, "source": "visible"}
    ...
    assert len(fc3) == len(VISION_FIELDS), f"fc3 should have all {len(VISION_FIELDS)} fields"
