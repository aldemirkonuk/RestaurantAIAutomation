...
from core.agent_registry import LazyAgentProxy, AgentSpec
...
_class = next(n for n in _tree.body if isinstance(n, ast.ClassDef) and n.name == "AgentOrchestrator")
_methods = [n for n in _class.body if isinstance(n, ast.AsyncFunctionDef) and n.name in {"stop_agent", "restart_agent", "start_agent", "get_agent_health"}]
_namespace = {"Dict": dict, "Any": object, "AgentStatus": AgentStatus, "asyncio": asyncio, "logger": setup_logger(__name__)}
exec(compile(ast.Module(body=_methods, type_ignores=[]), str(_orchestrator_source), "exec"), _namespace)
AgentOrchestrator = type("AgentOrchestrator", (), {n.name: _namespace[n.name] for n in _methods})
...
class Agent(BaseAgent):
    def __init__(self, bus, **config):
        super().__init__("test", bus, MagicMock(), {
            "circuit_breaker_enabled": False, "max_retries": 1,
            "task_timeout_seconds": 2, "max_concurrent_tasks": 1, **config,
        })
...
def bind_agent_methods(agent, filename, names, **globals_):
    ...
    cls = next(n for n in tree.body if isinstance(n, ast.ClassDef) and any(
        isinstance(m, ast.AsyncFunctionDef) and m.name in names for m in n.body
    ))
    ...
    namespace = {"asyncio": asyncio, "datetime": datetime, "timedelta": timedelta,
                 "AUTONOMY_TIER": "propose_only", "_ERROR_BACKOFF_SECONDS": 60, **globals_}
...
def background_fixture(kind):
    ...
    if kind == "provider":
        ...
        bind_agent_methods(agent, "provider_conversation_agent.py", {
            "initialize", "cleanup", "_proactive_monitor_loop",
        })
    ...
    if kind == "recurring":
        ...
        bind_agent_methods(agent, "recurring_order_agent.py", {
            "initialize", "cleanup", "_scheduler_loop", "_sleep_until_next_check", "_wait_for_shutdown",
        })
...
@pytest.mark.asyncio
@pytest.mark.parametrize("kind", ["provider", "buffer", "recurring"])
...
async def test_failed_initial_start_stays_visible_and_can_be_stopped_and_restarted():
    ...
    orchestrator.registry = SimpleNamespace(
        ...
        registered_count=1, loaded_count=1, active_count=0,
    )
