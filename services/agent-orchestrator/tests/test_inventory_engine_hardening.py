...
def mock_db():
    ...
    db.supabase.table.return_value.update.return_value.eq.return_value.eq.return_value.execute.return_value.data = [
...
def agent(mock_db):
    ...
    a = InventoryEngineAgent(
        "inventory_engine", mock_bus, mock_db, {"default_threshold": 3}
    )
...
class TestHARD01Idempotency:
    ...
    async def test_duplicate_message_skipped(self, agent, mock_db):
        ...
        mock_db.supabase.table.return_value.select.return_value.eq.return_value.execute.side_effect = [
    ...
    async def test_idempotency_check_fail_open(self, agent, mock_db):
        ...
        mock_db.supabase.table.return_value.select.return_value.eq.return_value.execute.side_effect = Exception(
            "DB connection lost"
        )
    ...
    async def test_missing_message_id_and_event_id_logs_warning(self, agent, mock_db):
        ...
        with (
            ...
            with __import__("unittest.mock", fromlist=["patch"]).patch.object(
                ...
                assert any(
                    "idempotency bypassed" in m for m in warning_messages
...
class TestHARD01DecisionLogging:
    ...
    async def test_stock_update_logs_decision(self, agent, mock_db):
        ...
        assert any(
            d.get("decision_type") == "stock_update" for d in logged_inserts
    ...
    async def test_decision_log_includes_inventory_id(self, agent, mock_db):
        ...
        assert any(
            "inventory_id" in inp for inp in logged_inputs
    ...
    async def test_manual_correction_confidence_0_7(self, agent, mock_db):
        ...
        assert any(
            d.get("confidence") == 0.7 for d in logged_entries
    ...
    async def test_delivery_decision_type(self, agent, mock_db):
        ...
        assert any(
            d.get("decision_type") == "delivery_received" for d in logged_entries
...
class TestHARD01EventSourcing:
    ...
    async def test_stock_update_appends_event(self, agent, mock_db):
        ...
        assert any(
            e.get("aggregate_type") == "inventory" for e in event_inserts
    ...
    async def test_event_store_has_correct_event_type(self, agent, mock_db):
        ...
        assert any(
            e.get("event_type") == "StockUpdated" for e in event_inserts
    ...
    async def test_delivery_event_type(self, agent, mock_db):
        ...
        assert any(
            e.get("event_type") == "DeliveryReceived" for e in event_inserts
    ...
    async def test_manual_correction_event_type(self, agent, mock_db):
        ...
        assert any(
            e.get("event_type") == "ManualCorrectionApplied" for e in event_inserts
    ...
    async def test_sequence_increments_per_aggregate(self, agent, mock_db):
        ...
        stock_events = [
            e for e in event_inserts if e.get("event_type") == "StockUpdated"
        ]
    ...
    @pytest.mark.asyncio
    async def test_different_aggregates_have_independent_sequences(
        self, agent, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_manual_correction_sequence_continues_after_stock(
        self, agent, mock_db
    ):
        ...
        stock_ev = next(
            (e for e in event_inserts if e.get("event_type") == "StockUpdated"), None
        )
        correction_ev = next(
            (
                e
                for e in event_inserts
                if e.get("event_type") == "ManualCorrectionApplied"
            ),
            None,
        )
...
class TestHARD01EdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_manual_correction_missing_manager_id_returns_early(
        self, agent, mock_db
    ):
    ...
    async def test_stock_evaluated_zero_stock_allowed(self, agent, mock_db):
        ...
        mock_db.get_inventory_item = AsyncMock(
            return_value={
                "id": "inv-1",
                "wine_name": "Opus One",
                "stock_live": 0,
