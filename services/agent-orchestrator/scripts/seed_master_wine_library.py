...
class MasterWineLibrarySeeder:
    ...
    def __init__(self):
        ...
        self.data_file = Path(__file__).parent.parent / "data" / "master_wine_library_seed.json"
    ...
    async def clear_existing_data(self):
        ...
        try:
            ...
            await self.db.supabase.table("master_wine_library").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
            print("   ✅ Cleared")
        ...
        for i, wine in enumerate(wines, 1):
            try:
                ...
                wine_record = {
                    ...
                    } if wine.get("barcode") and wine.get("vintage") else None,
                }
                
                # Insert into database
                result = await self.db.supabase.table("master_wine_library").insert(wine_record).execute()
    ...
    async def verify_seed(self) -> Dict[str, Any]:
        ...
        # Count total wines
        count_result = await self.db.supabase.table("master_wine_library").select("id", count="exact").execute()
        total_count = count_result.count or 0
        
        # Count by wine type
        types_result = await self.db.supabase.table("master_wine_library").select("wine_type").execute()
        type_counts = {}
        ...
        # Count by country
        countries_result = await self.db.supabase.table("master_wine_library").select("country").execute()
        country_counts = {}
        ...
        # Count with barcodes
        barcode_result = await self.db.supabase.table("master_wine_library").select("id").not_.is_("barcode", "null").execute()
