...
missing = [
    k
    for k, v in [("SERPER_API_KEY", serper_key), ("GEMINI_API_KEY", gemini_key)]
    if not v
]
if missing:
...
async def main():
    ...
    if verification is None:
        print(
            "ERROR: Gemini returned None. Check GEMINI_API_KEY and model availability."
        )
        sys.exit(1)
    ...
    for field in checkable_fields:
        ...
        print(
            f"  {status_icon} {field}: existing={existing_val!r}  web={web_val!r}  → {verdict.upper()}"
        )
