...
class RealtimeWeekDemo:
    ...
    def __init__(
        ...
        self.time_scale = (
            1 / 60 if accelerated else 1
        )  # 1 minute = 1 second in accelerated
    ...
    async def setup(self):
        ...
        print(
            f"   ✅ Toast API client ({('REAL' if self.use_real_toast else 'MOCK')} mode)"
        )
    ...
    async def time_9am_weekly_report(self):
        ...
        print(
            f"   ✓ sales_events: {len(sales.value) if sales.is_available() else 0} rows"
        )

        orders = await report_generator.query_procurement_orders(start_date, end_date)
        print(
            f"   ✓ procurement_orders: {len(orders.value) if orders.is_available() else 0} rows"
        )

        inventory = await report_generator.query_inventory()
        print(
            f"   ✓ restaurant_inventory: {len(inventory.value) if inventory.is_available() else 0} rows"
        )
    ...
    async def time_12pm_delivery_verification(self):
        ...
        await self.message_bus.publish(
            ...
            message_body={
                ...
                "payload": {
                    ...
                    "restaurant_id": self.restaurant_id,
                    "order_id": (
                        self.demo_orders[0] if self.demo_orders else str(uuid4())
                    ),
                    "wine_name": wine_name,
        ...
        print(
            f'   "SKU is {ordered_vintage} but they sent {received_vintage}. Do you want to update?"'
        )
        ...
        print('   "An order has been delivered wrong!"')
        print(
            f'   "Ordered {vintage_2} {ordered_type}, got {vintage_2} {received_type}"'
        )
    ...
    async def time_2pm_inventory_count(self):
        ...
        session_id = await self.mobile_simulator.start_count_session(
            restaurant_id=self.restaurant_id, staff_name="Alex M."
        )
        ...
        if self.demo_inventory:
            ...
            if item.data:
                wine_name = item.data.get("master_wine_library", {}).get(
                    "name", "Demo Wine"
                )
    ...
    async def time_4pm_low_stock_alert(self):
        ...
        if self.demo_inventory:
            ...
            if item.data:
                ...
                provider_name = (
                    item.data.get("providers", {}).get("name", "Demo Vendor")
                    if item.data.get("providers")
                    else "Demo Vendor"
                )
                ...
                print(
                    f'   "Stock: {stock_live}/{threshold} - Tap to order {suggested_qty} bottles"'
                )
    ...
    async def time_5pm_order_approval(self):
        ...
        if self.demo_inventory:
            ...
            provider_name = (
                providers.data[0]["name"] if providers.data else "Demo Vendor"
            )
...
async def main():
    ...
    parser.add_argument(
        "--use-real-toast", action="store_true", help="Use real Toast API"
    )
    parser.add_argument(
        "--time",
        choices=["9am", "12pm", "2pm", "4pm", "5pm", "evening"],
        help="Run specific time slot",
    )
    parser.add_argument(
        "--real-time", action="store_true", help="Run in real-time (not accelerated)"
    )
