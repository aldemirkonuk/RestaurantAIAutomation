...
class WeeklyReportScheduler:
    ...
    async def get_all_managers_with_preferences(self) -> List[Dict[str, Any]]:
        ...
        try:
            # Get managers
            managers_result = await self.db.supabase.table("managers").select("*").execute()
            managers = managers_result.data or []
            
            # Get preferences for each manager
            for manager in managers:
                prefs_result = await self.db.supabase.table("manager_preferences") \
                    .select("*") \
    ...
    async def generate_weekly_report(self, manager: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # 1. Sales Summary
            sales_result = await self.db.supabase.table("sales_events") \
                .select("*") \
            ...
            # 2. Orders Review
            orders_result = await self.db.supabase.table("procurement_orders") \
                .select("*") \
                .eq("restaurant_id", restaurant_id) \
                .gte("created_at", report["period"]["start"]) \
                .execute()
            ...
            completed_orders = [o for o in orders_data if o.get("status") == "COMPLETED"]
            pending_orders = [o for o in orders_data if o.get("status") in ["PENDING", "NEGOTIATING", "APPROVED"]]
            
            report["sections"]["orders"] = {
                "total_orders": len(orders_data),
                "completed": len(completed_orders),
                "pending": len(pending_orders),
                "total_spent": sum(o.get("final_confirmed_cost", 0) or 0 for o in completed_orders),
            }
            
            # 3. Inventory Status
            inventory_result = await self.db.supabase.table("restaurant_inventory") \
                .select("*, master_wine_library(name)") \
                .eq("restaurant_id", restaurant_id) \
                .execute()
            
            inventory_data = inventory_result.data or []
            low_stock_items = [i for i in inventory_data if i.get("stock_live", 0) <= i.get("threshold_min", 3)]
            ...
            report["sections"]["financials"] = {
                ...
                "gross_margin": total_sales - report["sections"]["orders"]["total_spent"],
            }
    ...
    def schedule_weekly_reports(self):
        ...
        print(f"📅 Weekly reports scheduled for Monday 9:00 AM (America/Los_Angeles)")
...
async def main():
    ...
    if args.now:
        ...
        print("Usage:")
        print("  python weekly_report_scheduler.py --now      # Run reports immediately")
        print("  python weekly_report_scheduler.py --schedule # Run as scheduler daemon")
