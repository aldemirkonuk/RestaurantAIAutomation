...
class DemoOrderingScenario:
    ...
    async def create_demo_data(self):
        ...
        if restaurants.data:
            ...
            print(f"   ✅ Using restaurant: {restaurants.data[0].get('name', 'Unknown')}")
        else:
            print("   ❌ No restaurants found in database!")
            print("   Creating demo restaurant...")
            # Create with required slug
            new_restaurant = self.db.supabase.table("restaurants").insert({
                "name": "Demo Wine Bar",
                "slug": "demo-wine-bar",
            ...
            print(f"   ✅ Created restaurant: Demo Wine Bar")
        
        # 2. Get manager from manager_preferences (no separate managers table)
        manager_prefs = self.db.supabase.table("manager_preferences").select("*").limit(1).execute()
        if manager_prefs.data:
            self.demo_manager_id = manager_prefs.data[0].get("manager_id", "demo-manager")
            ...
            print(f"   ⚠️ No manager preferences - using placeholder")
        ...
        if providers.data:
            ...
            new_provider = self.db.supabase.table("providers").insert({
            ...
            print(f"   ✅ Created provider: Demo Wine Distributors")
        
        # 4. Create demo wine in MASTER WINE LIBRARY (global catalog - every wine in the world)
        wines = self.db.supabase.table("master_wine_library").select("*").limit(1).execute()
        if wines.data:
            master_wine_id = wines.data[0]["id"]
            print(f"   ✅ Using wine from Master Library: {wines.data[0].get('name', 'Unknown')}")
        else:
            new_wine = self.db.supabase.table("master_wine_library").insert({
            ...
            print(f"   ✅ Added to Master Wine Library: Château Demo Reserve 2019")
        ...
        existing_inventory = self.db.supabase.table("restaurant_inventory").select("*").eq(
            "restaurant_id", self.demo_restaurant_id
        ).eq("master_wine_id", master_wine_id).limit(1).execute()
        ...
        if existing_inventory.data:
            ...
            print(f"   ✅ Using existing inventory (reset to low stock)")
        else:
            ...
            print(f"   ✅ Created new inventory item")
        
        print(f"\n   📊 INVENTORY STATUS:")
        print(f"   Wine: Château Demo Reserve 2019")
        print(f"   Current Stock: 3 bottles")
        print(f"   Threshold: 4 bottles")
        print(f"   Status: ⚠️ LOW STOCK (below threshold)")
    ...
    async def step_2_manager_push_notification(self):
        ...
        print(f"   Actions: Approve | Customize | Dismiss")
    ...
    async def step_3_manager_approves_order(self):
        ...
        print(f"   Communication: Email + SMS (Formal Template)")
...
"""
    ...
    async def step_7_manager_final_approval(self):
        ...
        print(f"   Approved Price: $25.50/bottle")
        print(f"   Approved Quantity: 12 bottles")
        print(f"   Total: $306.00")
...
"""
        ...
        print(f"   Previous shadow_stock: 0")
        print(f"   Adding: +12 bottles (confirmed order)")
        ...
        print(f"   New shadow_stock: 12 bottles")
        print(f"   Status: IN_TRANSIT")
        print(f"\n   💡 Shadow stock = Confirmed orders not yet delivered")
        print(f"   💡 Expected total when delivered: 3 + 12 = 15 bottles")
    ...
    async def step_9_delivery_arrives(self):
        ...
        print(f"   Vendor: Demo Wine Distributors")
    ...
    async def step_10_invoice_verification(self):
        ...
        print(f"   {'-'*50}")
        print(f"   Wine Name: ✅ MATCH")
        print(f"   Quantity: ✅ MATCH (12 = 12)")
        print(f"   Price: ✅ MATCH ($25.50 = $25.50)")
        print(f"   Total: ✅ MATCH ($306.00 = $306.00)")
        print(f"   Vintage: ✅ MATCH (2019)")
        ...
        print(f"   📝 Verification logged to order record")
    ...
    async def step_11_manager_finalizes(self):
        ...
        print(f"   Verification: All items received and verified")
    ...
    async def step_12_inventory_updated(self):
        ...
        print(f"   {'-'*50}")
        print(f"   Wine: Château Demo Reserve 2019")
        print(f"   Previous Stock: 3 bottles")
        print(f"   Received: +12 bottles")
        print(f"   New Stock: 15 bottles")
        print(f"   Threshold: 4 bottles")
        ...
        print(f"   shadow_stock: 12 → 0 (delivered)")
        print(f"   in_transit: 12 → 0 (received)")
        print(f"   stock_live: 3 → 15 (updated)")
    ...
    async def step_13_financial_reports_updated(self):
        ...
        print(f"\n   📊 Updated Reports:")
        print(f"   - Daily Procurement Report")
        print(f"   - Weekly Financial Summary")
        print(f"   - Vendor Payment Tracking")
    ...
    async def run_full_scenario(self):
        ...
        print("Low Stock → Notification → Approval → Negotiation → Order → Delivery → Update")
        ...
        try:
            ...
            print(f"   Final Price: $25.50/bottle")
            print(f"   Total Cost: $306.00")
            print(f"   Inventory: 3 → 15 bottles")
