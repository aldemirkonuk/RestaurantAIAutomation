...
class InventoryDiscrepancyScenario:
    ...
    async def scenario_1_shrinkage(
        ...
        if discrepancy < 0:
            ...
            print(
                f'   "[{wine_name}] sold {sales_today} bottles but we\'re down {abs(discrepancy)}"'
            )
    ...
    async def scenario_2_surplus(
        ...
        if discrepancy > 0:
            ...
            print("   Possible: Unrecorded delivery, count error, or return")
    ...
    async def scenario_3_investigation_flow(
        ...
        if action == "investigate":
            ...
            print("   [ ] Check delivery logs")
            print("   [ ] Verify POS transactions")
            print("   [ ] Interview staff")
            ...
            print("   ✓ Stock adjusted to physical count")

        elif action == "recount":
            print("\n   🔄 Requesting recount...")
            print("   Assigning to different staff member...")
    ...
    async def scenario_4_batch_count_with_discrepancies(
        ...
        # Get restaurant
        restaurants = (
            self.db.supabase.table("restaurants").select("id").limit(1).execute()
        )
    ...
    async def run_all_scenarios(self):
        ...
        try:
            ...
            print(f"{'='*70}")
            print(
                f"   Scenario 1 (Shrinkage): {result1.get('unaccounted', 0)} bottles unaccounted"
            )
            print(
                f"   Scenario 2 (Surplus): +{result2.get('discrepancy', 0)} bottles extra"
            )
            print(f"   Scenario 3 (Investigation): {result3.get('action', 'N/A')}")
            print(
                f"   Scenario 4 (Batch): {result4.get('items_with_discrepancy', 0)} discrepancies"
            )
