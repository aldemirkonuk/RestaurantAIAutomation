...
class WeeklyReportLLM:
    ...
    def _record_query(
        self, table: str, query: str, data: List[Dict], success: bool, error: str = None
    ):
    ...
    async def query_procurement_orders(
        self, start_date: datetime, end_date: datetime
    ) -> DataField:
    ...
    async def query_providers(self) -> DataField:
        ...
        try:
            ...
            self._record_query(
                "providers", "SELECT * WHERE is_active=true", result.data, True
            )
    ...
    def aggregate_report_data(
        ...
        if orders_data.is_available() and orders_data.value:
            ...
            pending = [
                o
                for o in orders
                if o.get("status") in ["PENDING", "APPROVED", "CONFIRMED", "IN_TRANSIT"]
            ]
    ...
    def generate_gmail_subject(self, data: Dict[str, Any]) -> str:
        ...
        week_of = datetime.strptime(data["period"]["start"], "%Y-%m-%d").strftime(
            "%B %d, %Y"
        )
...
"""
    ...
    def generate_sms_summary(self, data: Dict[str, Any]) -> str:
        ...
        revenue = (
            f"${financials['total_revenue']:,.0f}"
            if financials.get("available")
            else "N/A"
        )
        orders = (
            f"{procurement.get('completed_orders', 0)} done, {procurement.get('pending_orders', 0)} pending"
            if procurement.get("available")
            else "N/A"
        )
        low_stock = (
            inventory.get("low_stock_count", "N/A")
            if inventory.get("available")
            else "N/A"
        )
...
"""
    ...
    async def run(self):
        ...
        print(
            f"Mode: {'Supabase + Toast API' if self.include_toast else 'Supabase Only'}"
        )
        ...
        try:
            ...
            print(
                f"   ✓ restaurants: {'Found' if restaurant.is_available() else 'Not found'}"
            )
            ...
            print(
                f"   ✓ providers: {len(providers.value) if providers.is_available() else 0} rows"
            )
...
async def main():
    ...
    parser.add_argument(
        "--include-toast", action="store_true", help="Include Toast POS data"
    )
