#!/usr/bin/env python3
"""The security gate can fail, its verdict does not move on its own, publishing
cannot quietly narrow what the Security tab sees, and the baseline it forgives is
a subset of the 2026-09-12 set whose count moves only by an edit here.

`security` ("Security Scan") is rolled into `CI Complete`, one of main's required
status contexts (Security Scan itself is not one of the five -- measured
2026-09-17). Until 2026-09-12 its only scanning step set neither `exit-code`
...
A fourth pass (the 2026-09-17 judge of the Codex adoption) found the PyYAML
version sound but too narrow, and this revision closes what it measured: a tag
pin on an action whose tags were all hijacked on 2026-03-19; a scan that never
read the manifest production installs; and a publishing half the main check did
not police (--severity on the SARIF step, a refilled ignore file, a second SARIF
writer, an upload of a different file, a step exporting TRIVY_* after the gate
all exited 0 here).

It refuses:

  * a `security` job whose steps are not exactly, in order: checkout, install,
    pinned download, gate, latest download, SARIF, upload -- or a step carrying
    any key its role does not run with;
  * a gate step that is missing, duplicated, conditional, allowed to fail,
    ...
    --ignorefile .trivyignore, --skip-db-update, --cache-dir, --exit-code 1,
    --format and --file-patterns 'pip:requirements.*\\.txt' (single-quoted,
    exactly), scanning `.`;
  * a SARIF step that is not one `: >` truncation of a file under $RUNNER_TEMP
    followed by one trivy command whose flags are exactly --format sarif,
    --output, --skip-db-update, --cache-dir (the latest database's),
    --ignorefile (the truncated file) and the gate's --file-patterns; an upload
    whose `with:` is not exactly the SARIF step's --output; and a publishing step
    that does not run under `if: ${{ !cancelled() }}`;
  * database downloads carrying any flag but --download-db-only, --timeout,
    --cache-dir (and --db-repository on the pinned one), or a cache outside
    $RUNNER_TEMP;
  * setup-trivy referenced by anything but the full commit SHA recorded below
    (GHSA-69fq-xp46-6x23), or with a `with:` other than exactly {version: vX.Y.Z};
  ...
  * a `.trivyignore` that is not a subset of the recorded baseline (a swapped id is
    ...
    wildcard, or whose entry count is not exactly BASELINE_COUNT; and a baseline
    file that no longer hashes to the value pinned below.

What it does NOT catch, stated rather than implied: exemptions are keyed by
advisory id alone, so a baselined advisory reappearing in a NEW manifest or
package is forgiven by the gate (measured 2026-09-17). Scoping the baseline to
(id, path) is a founder decision, open in ADR 0142.

Exit 0 pass, 1 fail, 2 COULD NOT CHECK. Two is not a skip. A `security` job that
is missing or renamed is a FAIL (exit 1): the gate is gone, not unreadable.
...
# The number of ids .trivyignore carries, EXACTLY. This was a ceiling of 107, and
# a ceiling is not a ratchet: an id removed in one PR could be re-added in another
# without touching this file. Now a removal must lower this number in the same
# change, and a re-addition must raise it -- a visible edit to the guard.
BASELINE_COUNT = 107

# setup-trivy is pinned BY COMMIT. GHSA-69fq-xp46-6x23 (published 2026-03-24):
# on 2026-03-19 all seven setup-trivy tags were replaced with credential-stealing
# commits, and the security job runs the action with `security-events: write`.
# This SHA is v0.3.1, read from the GitHub API on 2026-09-17: refs/tags/v0.3.1
# points at it, its signature verifies, it is identical to the action's main, and
# the v0.3.1 release is immutable. The exact SHA is pinned rather than any 40-hex
# value because GitHub resolves a commit from a FORK of the action under the same
# name -- the imposter-commit shape that same advisory describes.
SETUP_TRIVY = "aquasecurity/setup-trivy"
SETUP_TRIVY_SHA = "81e514348e19b6112ce2a7e3ecbafe19c1e1f567"
FULL_SHA = re.compile(r"^[0-9a-f]{40}$")

# trivy's default pip pattern matches only `requirements.txt`, so the manifest
# production installs (services/agent-orchestrator/Dockerfile:
# requirements.prod.txt) was never scanned. The gate and the SARIF step both widen
# the pip pattern, spelled exactly this way: single-quoted, so bash neither globs
# the `*` nor eats the backslash before trivy sees it.
FILE_PATTERNS = r"pip:requirements.*\.txt"
FILE_PATTERNS_SPELLED = re.compile(r"(?:^|\s)--file-patterns\s+" + re.escape("'%s'" % FILE_PATTERNS) + r"(?=\s|$)")
SARIF_OUTPUT = re.compile(r"^[A-Za-z0-9_.-]+\.sarif$")

# The security job's steps, exactly and in this order: (role, keys the step may
# carry, keys it must carry). Any step added -- after the gate included -- can
# re-scope what is scanned or published, so there is no room for one.
STEP_ROLES = (
    ("checkout", {"name", "uses"}, {"uses"}),
    ("install", {"name", "uses", "with"}, {"uses", "with"}),
    ("pinned download", {"name", "run"}, {"run"}),
    ("gate", {"name", "run"}, {"run"}),
    ("latest download", {"name", "if", "run"}, {"if", "run"}),
    ("SARIF", {"name", "if", "run"}, {"if", "run"}),
    ("upload", {"name", "if", "uses", "with"}, {"if", "uses", "with"}),
)
PUBLISHING_ROLES = ("latest download", "SARIF", "upload")
PINNED_DOWNLOAD_FLAGS = ("--download-db-only", "--timeout", "--cache-dir", "--db-repository")
LATEST_DOWNLOAD_FLAGS = ("--download-db-only", "--timeout", "--cache-dir")
SARIF_FLAGS = ("--format", "--output", "--skip-db-update", "--cache-dir", "--ignorefile", "--file-patterns")
...
def _role(st):
    """Which of STEP_ROLES a step plays, read from what it runs, never its name."""
    uses = st["uses"]
    if uses.startswith("actions/checkout@"):
        return "checkout"
    if uses.startswith(SETUP_TRIVY + "@"):
        return "install"
    ...
    if st["trivy"]:
        ...
        if "--exit-code" in f:
            return "gate"
        if "--download-db-only" in f:
            return "pinned download" if "--db-repository" in f else "latest download"
        if f.get("--format") == ["sarif"]:
            return "SARIF"
    return None


def _file_patterns_token(st):
    """(command index, token index) of the step's one --file-patterns value when it is spelled
    exactly `--file-patterns 'pip:requirements.*\\.txt'`, else None.

    shlex strips quotes, so `'x*'` and a bare `x\\*`-style spelling bash would glob can
    tokenise alike. The raw logical command is what says the `*` was quoted; the token
    count says the quoted spelling is the flag's value and not text inside some other
    token (any such token still contains `*`, and is refused as an expansion)."""
    spelled = sum(len(FILE_PATTERNS_SPELLED.findall(c)) for c in st["cmds"])
    hits = [(ci, ti) for ci, toks in enumerate(st["toks"]) for ti, t in enumerate(toks) if t == FILE_PATTERNS]
    if spelled == 1 and len(hits) == 1:
        ci, ti = hits[0]
        if ti > 0 and st["toks"][ci][ti - 1] == "--file-patterns":
            return hits[0]
    return None


def _file_patterns_failures(st, what, out):
    vals = st["trivy"]["flags"].get("--file-patterns")
    if vals != [FILE_PATTERNS]:
        out.append("the %s's --file-patterns is %r, not exactly one %s; trivy's default pip pattern matches only "
                   "requirements.txt, so the manifest production installs (requirements.prod.txt) goes unscanned"
                   % (what, vals, FILE_PATTERNS))
    elif _file_patterns_token(st) is None:
        out.append("the %s's --file-patterns is not spelled exactly --file-patterns '%s', once and single-quoted; "
                   "any other spelling lets bash glob the `*` or eat the backslash before trivy sees it"
                   % (what, FILE_PATTERNS))


def _only_flags(st, allowed, what, out):
    f = st["trivy"]["flags"]
    for extra in sorted(set(f) - set(allowed)):
        out.append("the %s passes %s, which is not one of its flags (%s)" % (what, extra, ", ".join(allowed)))
    for k in allowed:
        if len(f.get(k, [])) > 1:
            out.append("the %s passes %s %d times; trivy honours the last" % (what, k, len(f[k])))


def failures_for(ci_text, ignore_lines, baseline_lines, baseline_sha, tracked, count=BASELINE_COUNT):
    """Every reason the tree's gate cannot be vouched for. `tracked` is `git ls-files`, or None
    when git could not say (the caller reports that separately). `count` is the exact number of
    ids .trivyignore must carry. Raises CannotCheck."""
    out = []
    try:
    ...
    # --- the exact step list, and the keys each step may carry ----------------
    roles = [_role(s) for s in steps]
    expected = [r[0] for r in STEP_ROLES]
    if roles != expected:
        seen = " -> ".join(r if r else "an unrecognised step %s" % s["label"] for r, s in zip(roles, steps))
        out.append("the `security` job's steps are %s; ADR 0142 decided exactly %s, in that order. Any step added "
                   "-- after the gate included -- can re-scope what is scanned or published (a TRIVY_* line "
                   "written to $GITHUB_ENV, a second writer of the SARIF file)" % (seen, " -> ".join(expected)))
    for st, role in zip(steps, roles):
        spec = [r for r in STEP_ROLES if r[0] == role]
        if not spec:
            continue
        _, allowed, required = spec[0]
        keys = set(st["keys"])
        extra, missing = sorted(keys - allowed), sorted(required - keys)
        if extra:
            out.append("the %s step %s carries %s; the only keys that step runs with are %s"
                       % (role, st["label"], ", ".join("`%s`" % k for k in extra), ", ".join(sorted(allowed))))
        if missing:
            out.append("the %s step %s lacks %s" % (role, st["label"], ", ".join("`%s`" % k for k in missing)))
        if role in PUBLISHING_ROLES and st["keys"].get("if") != PUBLISH_IF:
            out.append("the publishing step %s does not run under exactly `if: %s`, so a red gate can skip it and "
                       "leave the Security tab stale on exactly the builds that found something"
                       % (st["label"], PUBLISH_IF))
    ...
    uploads = [s for s in steps if s["uses"].startswith("github/codeql-action/upload-sarif@")]
    gate = gates[0] if len(gates) == 1 else None

    # --- what GitHub applies around a command, on every step the verdict or the Security tab rests on
    publishing = {id(s) for s in latest + sarifs + uploads}
    strict = {id(s) for s in checkouts + installs + trivy_steps + uploads}
    for st in steps:
    ...
    if len(installs) != 1:
        ...
        ref = installs[0]["uses"].split("@", 1)[1]
        if not FULL_SHA.match(ref):
            out.append("setup-trivy is not pinned by full commit SHA: @%s. A tag is a pointer anyone holding the "
                       "action's credentials can move, and on 2026-03-19 all seven of its tags were moved to "
                       "credential-stealing commits (GHSA-69fq-xp46-6x23)" % ref)
        elif ref != SETUP_TRIVY_SHA:
            out.append("setup-trivy is pinned to @%s, not the commit this guard records (%s, v0.3.1). A commit from "
                       "a fork of the action resolves under the same name, so moving the pin is an edit to this "
                       "guard as well" % (ref, SETUP_TRIVY_SHA))
        with_ = installs[0]["keys"].get("with")
        if not (isinstance(with_, dict) and set(with_) == {"version"}):
            out.append("the install step's `with:` is %r, not exactly {version: vX.Y.Z}; cache, path and token all "
                       "change where the trivy binary comes from" % (with_,))
    ...
    for st in trivy_steps:
        exempt = _file_patterns_token(st)
        bad = sorted({t for ci, toks in enumerate(st["toks"]) for ti, t in enumerate(toks)
                      if any(x in t for x in SHELL_EXPANSIONS) and (ci, ti) != exempt})
        if bad:
            out.append("trivy step %s has token(s) the shell expands before trivy sees them (%s): command "
                       ...
                       % (st["label"], ", ".join(bad[:3])))
        # A word that BEGINS with `#` starts a bash comment (mid-word it does not, and the
        # tokeniser keeps it either way). Everything after it is text this guard would
        # read as flags and bash would never run -- `--format #` swallows the next token
        # as a value, so no positional check sees it.
        hashed = [t for toks in st["toks"] for t in toks if t.startswith("#")]
        if hashed:
            out.append("trivy step %s has a word beginning with `#` (%s); bash reads it as a comment, so the flags "
                       "after it are ones this guard reads and trivy never receives" % (st["label"], hashed[0]))
    ...
    if len(pinned) != 1:
        ...
        _only_flags(p, PINNED_DOWNLOAD_FLAGS, "pinned database download", out)
        if p["trivy"]["pos"]:
            out.append("the pinned database download is given positional arguments %r" % p["trivy"]["pos"])
    ...
    if gate is None:
        ...
        _file_patterns_failures(gate, "gate", out)
        if gate["trivy"]["pos"] != ["."]:
    ...
    # --- publishing ---------------------------------------------------------
    # Policed as strictly as the gate. The baseline forgives an advisory for the
    # purpose of FAILING a build; the Security tab must still carry it, and GitHub
    # closes an alert the moment a later upload from the same tool omits it.
    ...
    if len(latest) != 1:
        ...
        lp = latest[0]
        why = _single_clean_trivy(lp)
        if why:
            out.append("the latest database download %s" % why)
        if lp["trivy"]["flags"].get("--download-db-only") != [True]:
            out.append("the latest database download gives --download-db-only a value")
        _only_flags(lp, LATEST_DOWNLOAD_FLAGS, "latest database download", out)
        if lp["trivy"]["pos"]:
            out.append("the latest database download is given positional arguments %r" % lp["trivy"]["pos"])
        if not _cache_ok(latest_cache):
            out.append("the latest database's --cache-dir %r is not a single path under $RUNNER_TEMP; a database "
                       "planted in the checkout can decide what the Security tab is shown" % (latest_cache,))
    sarif_output = None
    if len(sarifs) != 1:
        ...
        elif not _cache_ok(ign[0]):
            out.append("the SARIF step's --ignorefile %r is not under $RUNNER_TEMP" % ign[0])
        if latest_cache is not None and f.get("--cache-dir") != [latest_cache]:
        ...
        # Exactly: one truncation of the ignore file, then one trivy command. A
        # second command can refill the file (cp .trivyignore ...) or overwrite the
        # SARIF with a filtered scan, and neither changes a flag read above.
        shape = ["truncate" if len(t) == 3 and t[:2] == [":", ">"] else
                 "trivy" if t[:2] == ["trivy", "fs"] and not any(x in SHELL_OPERATORS for x in t) else
                 "other" for t in sp["toks"]]
        if shape != ["truncate", "trivy"]:
            out.append("the SARIF step runs %s; it must be exactly one `: >` truncation of its ignore file followed "
                       "by one trivy command, because anything else can refill that file or overwrite the SARIF"
                       % (" then ".join(shape) or "nothing"))
        _only_flags(sp, SARIF_FLAGS, "SARIF step", out)
        if f.get("--format") != ["sarif"]:
            out.append("the SARIF step's --format is %r, not exactly sarif" % f.get("--format"))
        if f.get("--skip-db-update") != [True]:
            out.append("the SARIF step's --skip-db-update is %r; without it the step fetches a database of its own"
                       % f.get("--skip-db-update"))
        _file_patterns_failures(sp, "SARIF step", out)
        if sp["trivy"]["pos"] != ["."]:
            out.append("the SARIF step scans %r, not the repository root '.'" % sp["trivy"]["pos"])
        sarif_output = _one(f, "--output")
        if not (isinstance(sarif_output, str) and SARIF_OUTPUT.match(sarif_output)):
            out.append("the SARIF step's --output is %r, not one plain *.sarif file name in the workspace"
                       % f.get("--output"))
            sarif_output = None

    if len(uploads) != 1:
        out.append("expected exactly one upload-sarif step, found %d" % len(uploads))
    else:
        w = uploads[0]["keys"].get("with")
        if not (isinstance(w, dict) and set(w) == {"sarif_file"}):
            out.append("the upload step's `with:` is %r, not exactly {sarif_file: <the SARIF step's --output>}; "
                       "category, ref, sha and checkout_path all change which alerts an upload closes" % (w,))
        elif sarif_output is not None and w["sarif_file"] != sarif_output:
            out.append("the upload step's sarif_file is %r, not the SARIF step's --output %r, so what reaches the "
                       "Security tab is not what trivy wrote" % (w["sarif_file"], sarif_output))
    ...
    if count > len(base):
        out.append("the guard records %d baselined ids, more than the %d in the pinned baseline" % (count, len(base)))
    if len(entries) > count:
        out.append(".trivyignore has %d entries against the %d this guard records; a line added silences a NEW "
                   "advisory, which is the one thing the gate exists to refuse" % (len(entries), count))
    elif len(entries) < count:
        out.append(".trivyignore has %d entries but this guard records %d. Removing a line pays the backlog down: "
                   "lower BASELINE_COUNT to %d in the same change, so the id cannot come back later without an edit "
                   "here" % (len(entries), count, len(entries)))
    ...
    steps:
      ...
      - name: Run Trivy vulnerability scanner
        ...
        run: |
          ...
          trivy fs --format sarif --output trivy-results.sarif --skip-db-update --cache-dir "$RUNNER_TEMP/trivy-latest" --ignorefile "$RUNNER_TEMP/empty.trivyignore" --file-patterns 'pip:requirements.*\.txt' .
...
GATE_RUN = "        run: |\n          trivy fs \\\n            --scanners vuln"
GATE_FP_LINE = "            --file-patterns 'pip:requirements.*\\.txt' \\\n"
GATE_TAIL = GATE_FP_LINE + "            .\n"
INSTALL_USES = "aquasecurity/setup-trivy@81e514348e19b6112ce2a7e3ecbafe19c1e1f567"
SARIF_CMD = ('trivy fs --format sarif --output trivy-results.sarif --skip-db-update --cache-dir "$RUNNER_TEMP/trivy-latest" '
             '--ignorefile "$RUNNER_TEMP/empty.trivyignore" --file-patterns \'pip:requirements.*\\.txt\' .')
TRUNCATE = '          : > "$RUNNER_TEMP/empty.trivyignore"\n'
LATEST_LINE = '        run: trivy fs --download-db-only --timeout 15m --cache-dir "$RUNNER_TEMP/trivy-latest"\n'
UPLOAD_WITH = '          sarif_file: "trivy-results.sarif"\n'
...
def _tail(flag):
    return [(GATE_TAIL, GATE_FP_LINE + "            %s \\\n            .\n" % flag)]
...
JOB_MUTATIONS = [
    ...
    ("setup-trivy@main with version latest",
     [(INSTALL_USES, "aquasecurity/setup-trivy@main"), ("version: v0.74.0", "version: latest")], "fail", "@main"),
    ...
    ("the SARIF step names no ignore file", [(' --ignorefile "$RUNNER_TEMP/empty.trivyignore" --file', " --file")],
     "fail", "names no --ignorefile"),
    ("the SARIF step names .trivyignore",
     [(' --ignorefile "$RUNNER_TEMP/empty.trivyignore" --file', " --ignorefile .trivyignore --file")], "fail", "not a file the same step empties"),
    ...
    ("R3 pushd newline trivy", [(GATE_RUN, "        run: |\n          pushd %s\n          trivy fs \\\n            --scanners vuln" % WD)],
     "fail", "commands, not exactly one"),
    ("R4 second positional", [(GATE_TAIL, GATE_FP_LINE + "            . %s\n" % WD)], "fail", "repository root"),
    ...
    ("mid-word # hides || true from a comment-aware tokeniser", [(GATE_TAIL, GATE_FP_LINE + "            .#||true\n")],
     "fail", "shell operator"),
    ...
    # ---- the 2026-09-17 judge pass (Codex adoption): each a measured exit 0 before --
    # G1: a tag pin on an action whose seven tags were all hijacked on 2026-03-19.
    ("setup-trivy pinned to the v0.3.1 tag", [(INSTALL_USES, "aquasecurity/setup-trivy@v0.3.1")], "fail", "full commit SHA"),
    ("setup-trivy pinned to a short SHA", [(INSTALL_USES, "aquasecurity/setup-trivy@81e5143")], "fail", "full commit SHA"),
    ("setup-trivy pinned to a different 40-hex SHA (a fork's commit resolves too)",
     [(INSTALL_USES, "aquasecurity/setup-trivy@" + "0" * 40)], "fail", "not the commit this guard records"),
    ("setup-trivy pinned to an upper-case SHA", [(INSTALL_USES, INSTALL_USES.upper().replace("AQUASECURITY/SETUP-TRIVY", "aquasecurity/setup-trivy"))],
     "fail", "full commit SHA"),
    # G9: where the trivy binary comes from.
    ("install step gains cache: true", [("          version: v0.74.0\n", "          version: v0.74.0\n          cache: true\n")],
     "fail", "not exactly {version"),
    ("install step gains path: ./bins", [("          version: v0.74.0\n", "          version: v0.74.0\n          path: ./bins\n")],
     "fail", "not exactly {version"),
    ("install step gains token", [("          version: v0.74.0\n", "          version: v0.74.0\n          token: x\n")],
     "fail", "not exactly {version"),
    # G3: the manifest production installs.
    ("gate --file-patterns removed", [(GATE_TAIL, "            .\n")], "fail", "requirements.prod.txt"),
    ("gate --file-patterns narrowed to requirements.txt only",
     [(GATE_FP_LINE, "            --file-patterns 'pip:requirements\\.txt' \\\n")], "fail", "requirements.prod.txt"),
    ("gate --file-patterns double-quoted", [(GATE_FP_LINE, "            --file-patterns \"pip:requirements.*\\.txt\" \\\n")],
     "fail", "not spelled exactly"),
    ("gate --file-patterns unquoted, bash globs it", [(GATE_FP_LINE, "            --file-patterns pip:requirements.*\\\\.txt \\\n")],
     "fail", "not spelled exactly"),
    ("gate --file-patterns=value form", [(GATE_FP_LINE, "            --file-patterns='pip:requirements.*\\.txt' \\\n")],
     "fail", "--file-patterns"),
    ("gate --file-patterns given twice", [(GATE_FP_LINE, GATE_FP_LINE + GATE_FP_LINE)], "fail", "--file-patterns 2 times"),
    ("gate: the quoted spelling in a comment, the real value unquoted",
     [(GATE_TAIL, "            --file-patterns pip:requirements.*\\\\.txt \\\n            . # --file-patterns 'pip:requirements.*\\.txt'\n")],
     "fail", "comment"),
    ("SARIF --file-patterns removed", [(" --file-patterns 'pip:requirements.*\\.txt' .\n      - name: Upload", " .\n      - name: Upload")],
     "fail", "SARIF step's --file-patterns"),
    ("--format # comments out the flags after it", [("            --format table \\\n", "")] + [(GATE_TAIL, "            --format # \\\n" + GATE_TAIL)],
     "fail", "comment"),
    # G5: publishing, policed by the main check and not only by the fixture match.
    ("M2 SARIF step --severity CRITICAL", [(SARIF_CMD, SARIF_CMD.replace("--format sarif", "--format sarif --severity CRITICAL"))],
     "fail", "SARIF step passes --severity"),
    ("SARIF step --scanners vuln (drops secret findings)", [(SARIF_CMD, SARIF_CMD.replace("--format sarif", "--format sarif --scanners vuln"))],
     "fail", "SARIF step passes --scanners"),
    ("SARIF step --skip-files", [(SARIF_CMD, SARIF_CMD.replace(" .", " --skip-files pnpm-lock.yaml ."))],
     "fail", "SARIF step passes --skip-files"),
    ("M3 SARIF step refills its emptied ignore file from .trivyignore",
     [(TRUNCATE, TRUNCATE + '          cp .trivyignore "$RUNNER_TEMP/empty.trivyignore"\n')], "fail", "exactly one `: >` truncation"),
    ("M4 a second, filtered trivy overwrites the SARIF",
     [(SARIF_CMD + "\n", SARIF_CMD + "\n          " + SARIF_CMD.replace('"$RUNNER_TEMP/empty.trivyignore"', ".trivyignore") + "\n")],
     "fail", "exactly one `: >` truncation"),
    ("the truncation moved after the trivy command",
     [(TRUNCATE + "          " + SARIF_CMD + "\n", "          " + SARIF_CMD + "\n" + TRUNCATE)], "fail", "exactly one `: >` truncation"),
    ("M5 upload sarif_file pointed elsewhere", [(UPLOAD_WITH, '          sarif_file: "docs/empty.sarif"\n')],
     "fail", "not the SARIF step's --output"),
    ("SARIF --output and upload both /dev/null",
     [(SARIF_CMD, SARIF_CMD.replace("--output trivy-results.sarif", "--output /dev/null")),
      (UPLOAD_WITH, '          sarif_file: "/dev/null"\n')], "fail", "plain *.sarif"),
    ("upload step gains category", [(UPLOAD_WITH, UPLOAD_WITH + "          category: elsewhere\n")], "fail", "not exactly {sarif_file"),
    ("upload step if: false", [(UPLOAD_NAME + "        if: ${{ !cancelled() }}\n", UPLOAD_NAME + "        if: false\n")],
     "fail", "does not run under exactly"),
    ("upload step continue-on-error", [(UPLOAD_NAME, UPLOAD_NAME + "        continue-on-error: true\n")], "fail", "continue-on-error"),
    ("SARIF step loses its !cancelled()", [(SARIF_NAME + "        if: ${{ !cancelled() }}\n", SARIF_NAME)], "fail", "does not run under exactly"),
    ("M6 a step after the gate writes TRIVY_SEVERITY to $GITHUB_ENV",
     [(LATEST_NAME, '      - name: tune\n        run: echo "TRIVY_SEVERITY=CRITICAL" >> "$GITHUB_ENV"\n' + LATEST_NAME)],
     "fail", "steps are"),
    ("a step appended after the upload", lambda t: t.replace(UPLOAD_WITH, UPLOAD_WITH + "      - run: echo done\n", 1) if t.count(UPLOAD_WITH) == 1 else _raise("anchor"),
     "fail", "steps are"),
    ("the latest download moved before the gate", lambda t: _move_before(t, LATEST_NAME, GATE_NAME), "fail", "steps are"),
    ("latest download gains --insecure", [(LATEST_LINE, LATEST_LINE.replace(" --cache-dir", " --insecure --cache-dir"))],
     "fail", "latest database download passes --insecure"),
    ("latest download cache inside the checkout", [(LATEST_LINE, LATEST_LINE.replace('"$RUNNER_TEMP/trivy-latest"', ".github/cache")),
                                                   ('--cache-dir "$RUNNER_TEMP/trivy-latest" --ignorefile', "--cache-dir .github/cache --ignorefile")],
     "fail", "latest database's --cache-dir"),
    ("pinned download gains --insecure", [(PINNED_LINE, PINNED_LINE.replace(" --cache-dir", " --insecure --cache-dir"))],
     "fail", "pinned database download passes --insecure"),
    ("a timeout-minutes key on the SARIF step", [(SARIF_NAME, SARIF_NAME + "        timeout-minutes: 1\n")], "fail", "carries `timeout-minutes`"),
    # G8: a renamed job is a FAIL, not CANNOT CHECK -- the notice in ci.yml says so.
    ("the security job renamed", [(JOB_HEAD, "  security-scan:\n    name: Security Scan\n")], "fail", "no `security` job"),
]
...
def _move_before(text, block_name, before_name):
    """Move the step that starts at `block_name` to just before the step at `before_name`."""
    if text.count(block_name) != 1 or text.count(before_name) != 1:
        raise ValueError("anchor")
    start = text.index(block_name)
    end = text.index("      - name:", start + len(block_name))
    block = text[start:end]
    text = text[:start] + text[end:]
    i = text.index(before_name)
...
def self_test():
    ...
    # The fixture's baseline holds three ids, so its recorded count is three. Left at the
    # real 107, every fixture would fail on the count and a "fail" case would prove nothing.
    def run(ci=ARMED, ignore=base_ids, baseline=base_ids, sha=BASELINE_SHA256, tracked=fixture_tracked,
            count=len(base_ids)):
        return failures_for(ci, list(ignore), list(baseline), sha, None if tracked is None else list(tracked), count)
    ...
    cases += [
        ...
        ("a baseline shrunk by one, the recorded count left alone", lambda: run(ignore=base_ids[:-1]),
         "fail", "lower BASELINE_COUNT"),
        ("a baseline shrunk by one, the recorded count lowered with it",
         lambda: run(ignore=base_ids[:-1], count=len(base_ids) - 1), "pass", None),
        ("an id re-added after a shrink, the count not raised back",
         lambda: run(ignore=base_ids, count=len(base_ids) - 1), "fail", "against the 2 this guard records"),
        ("a recorded count above the pinned baseline's size", lambda: run(count=len(base_ids) + 1),
         "fail", "more than the 3 in the pinned baseline"),
        ("comments quoting an armed gate do not arm a disarmed one",
         lambda: run(ci=ARMED.replace("            --exit-code 1 \\\n", "            # --exit-code 1 \\\n")),
         "fail", "comment"),
        ("the real tree at the recorded count only: the real .trivyignore one id short fails",
         lambda: failures_for(real_ci, real_ignore[:-1] if real_ignore and not real_ignore[-1].startswith("#")
                              else real_ignore + ["CVE-2025-7783"], real_baseline, real_sha, real_tracked),
         "fail", "BASELINE_COUNT"),
    ]
...
def main():
    ...
    print("== security gate: %d baselined advisor%s (recorded count %d)" % (n, "y" if n == 1 else "ies", BASELINE_COUNT))
    if failures:
        print("")
    ...
    print("PASS -- the gate fails on a high or critical advisory id outside the baseline, against a pinned")
    print("        database; the Security tab is published from an emptied ignore file, at every severity, by")
    print("        exactly the steps ADR 0142 decided; and the baseline is the recorded subset, at the recorded count.")
    print("        Not checked here: exemptions are per advisory id, so a baselined id in a new manifest passes.")
    sys.exit(0)
