...
> **176 as of 2026-09-21, on `fix/ical-token-minted-on-act`** (`check_read_errors_not_swallowed.py` on that tree: 1533 files scanned, 176 sites, 176 baselined, 0 allowlisted). The iCal feed moved from `CalendarService.getICalFeed` into `CalendarLinksService` (personal calendar links, ADR 0111 review trail), whose recurrence-rule read binds its error and answers 503; that retires `calendar.service.ts` calendar_recurrence_rules/rules. Branches merge in any order, so re-measure before citing.
