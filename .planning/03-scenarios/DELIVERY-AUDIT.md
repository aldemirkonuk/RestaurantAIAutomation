...
> **173 as of 2026-09-18, on `fix/sessions-follow-membership`** (`check_read_errors_not_swallowed.py` on that tree: 1498 files scanned, 173 sites, 173 baselined, 0 allowlisted). ADR 0164 retired five: `switchRestaurant` lost its organisation fallback (organization_members/orgMemberships, restaurants/ownRestaurant) and its own access read (user_restaurant_access/uraAccess), `generateTokens` binds its membership read's error and answers 503 (user_restaurant_access/membership), and `createLocation` no longer reads organizations/ownedOrg (2 to 1). Branches merge in any order, so re-measure before citing.
