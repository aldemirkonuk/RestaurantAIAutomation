...
| Built on the primitive | 41 |
| Migrate — legacy inside a house-flagged page today | 0 |
...
- **F13 — The consent step has no home.** Built 2026-09-06 as a Panel, asserted not sealed — and unreachable. Its only opener is ServicesPermissions.tsx:300, mounted solely by the LEGACY settings page (pages/Settings.tsx:1262); the rebuilt page's ServicesSection renders the same four consents as records with NO switches, because nothing in any of the four runtimes branches on them and ADR 0020 forbids a control whose effect does not exist. Either something reads a consent and the rebuilt page gets a real control back, or this act is a deletion. Not a builder's call. *(open)*
