# 059 — Mudavym · The House

The front-runner. Not a sixth idea: this is the founder's own keep-pile from directions
053–057, assembled into one identity and then finished. The job was **faithful assembly
...
|---|---|---|
| The müdavim definition block ("The regular. The one who is there every day — who the house knows, and who knows the house.") | **053** | Verbatim. Set in Fraunces instead of 053's face; typeset between a hairline and a double rule. |
| "Most restaurant software watches the restaurant like a security camera / Mudavym sits at the bar like its most loyal regular…" | **053** | Verbatim through "quietly takes care of things"; 053's direction-specific tail replaced with a paragraph that turns the story into the design brief. |
| Slogan "Every vendor. Every bottle. Every shift." | **053** (S4) | Verbatim, rationale preserved. |
| Slogan "Less counting, more cooking." | **053** (S5) | Verbatim, rationale preserved. |
| LIVE row-expand `settle` demo — PO-1038 · Bodega Álvaro, grid-rows 0fr→1fr on the house curve, chevron on the same token | **053** | Behaviour, content, curve and copy carried over unchanged. This is the motion the founder named; it is the first demo in §08 and the token keeps its name. |
| Logo "C — The Meter" (five bars spelling M, one bar carrying the signal) | **054** | Kept as the **alternate** mark. Two changes to make it belong here: the wordmark is re-cut in Fraunces (was Geist), and the meter now stands *on the double rule* so the readout and the total are one object. The lit-bar rule (one bar = one thing waiting) is preserved. |
| Slogan "Set the table. We'll keep the books." | **055** (S5) | Verbatim, with its division-of-labour rationale intact and expanded. |
| Slogan "Your back of house, front of mind." | **055** (S6) | Kept and **flagged as sales material, not in-app**, in its own callout with the reason. |
| The Fraunces house voice — variable serif, warm and slightly wonky, "a wine label that learned software", used ONLY when the product speaks; "Good evening, chef." | **055** | Kept whole. Pinned to `SOFT 42 / WONK 1` and given a 16px floor. "Good evening, chef." is the dashboard greeting. |
| One-tap / hold-to-approve interaction | **056** | Kept. 620ms hold (was 600ms), fill linear, release on the house curve; on completion the **seal lands** (`stamp`) instead of only a label change — this is the elevation. Live in §08 *and* on the dashboard mock. |
| The double-rule logo | **057** (concept 1) | Kept and **developed**: wordmark measured to the rule with `textLength`, rule weight/gap/clear-space derived from the M, minimum size stated, fallback lockup named. |
| The full stop as a mark | **057** (concept 3) | Kept and developed into Lockup C (app tile / favicon) *and* promoted to a system rule: the period is the only coloured element in the identity and it ends every verdict the product writes. |
| Slogan "Every bottle, accounted for." | **057** (02) | Verbatim, rationale preserved. |
| Slogan "Your *müdavim*, at the books." | **057** (06) | Verbatim, rationale preserved. |
| §03 "Voice — how the book writes" (the eight rules) | **057** | Kept whole, wording tightened. Rule 04's once-per-screen serif cap is load-bearing and is called out as such. |
| Paper / ink / one-seal palette structure | **057** | Structure kept exactly (paper grounds, 4-step ink ladder shared by text and icons, surfaces as alpha tints of warm ink, one non-ink colour, semantic tint twins). Values warmed one step toward 053's cream so light mode reads dining room, not office. |
| Row-expand "show the working" pattern | **057** | Kept: arithmetic beneath the row, totalled with the accountant's double rule, on the `turn` token. |
| Honesty idioms — "—" never a pass, "no comparable data" never 0%, the dash gets its own token | **057** | Kept as UI grammar, visible in the orders mock and the empty-state rewrite. |
| Wax-seal-as-approval-stamp (brand mark == product interaction) | **057** (concept 2) | Kept as the mechanism, but the *colour* is re-opened — see §05. |
...
053's three marks (the Kept Seat, the Tally, the Homecoming Dots) — the founder rejected
all of them, so none appear, not even as a favicon. 054's radius-0 monochrome system and
its decision-amber. 055's dark-primary stance (dark is here, but light is home; the
product is used from prep through close and the paper is the identity). 056's 128px
display scale and imperative expo voice. 057's burgundy `#8A2E33` seal — the seal
...
that could have collided: 053/055's warm serif-first hospitality, and 057's deadpan
...
Instrument Sans is the new pick for the working face: 053 used Instrument Sans, 055 used
Hanken Grotesk, 057 used Source Sans 3 — Instrument wins because it is faintly narrow
...
(double rule, full stop) but rejected every 053 mark, which reads as a rejection of
...
**4. Where does the ceremony go?** 056 contributed hold-to-approve; 057 contributed the
...
### The eight rules (kept from 057 §03)
...
### Slogan set
Kept: "Every vendor. Every bottle. Every shift." (053) · "Less counting, more cooking."
(053) · "Set the table. We'll keep the books." (055) · "Every bottle, accounted for."
(057) · "Your *müdavim*, at the books." (057). Sales-deck only: "Your back of house,
front of mind." (055). New: **"Warm room. Cold arithmetic."** (the mandate as a two-beat
...
**Checked, with evidence** (served from `http://localhost:8642/059-mudavym-house/`, probed
