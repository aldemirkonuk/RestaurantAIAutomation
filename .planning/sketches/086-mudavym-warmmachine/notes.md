# 086 — Mudavym: Warm Machine

Brand direction 086. The espresso machine as the model for the whole product: brass, steel,
...
## The 078 problem, and the answer

078 (The Instrument) reached for "technically superior" and landed cold; the founder kept only
...
to author warmth (078's rule R-08: "warmth is quoted, never authored").

086 keeps the precision and reverses all three:

| 078 chose | 086 chooses | Why the warmth arrives |
|---|---|---|
...
| Radius 0 everywhere, deliberately | **Chamfered** — 2px data / 4px panel / 8px touch | Machined edges, not sawn ones; answers 078's own open question about 11pm tablets |
...
- **Move 4 — "Thinking shimmer"** (manus), promoted from a loading treatment to the direction's
  entire signature. manus sweeps a gradient across text to say "the agent is working"; 086 asks
...
- **Move 2 — "Warm-paper neutral system"** (manus), pushed materially further. manus tints
  surfaces as alpha washes of a warm ink `#37352f` on `#f8f8f7`; 086 takes the principle — never
...
- **Move 12 — "Grayscale data, chroma = anomaly"** (bklit), adopted for charts only, not for the
  whole system. The chart ramp is five *warm* greys; hue enters a chart where meaning does. 078
...
- **Move 18 — "Deadpan brand, quoted warmth"** (bklit), partially rejected — and this is the
  ...
  humans carry the warmth. 086 keeps the *quoted human* (the vendor's Spanish reply is the only
...
on touch controls, which the research itself warns against and 078 carried anyway.
...
- **Bricolage Grotesque** 400–700, variable optical size — *the operator*. All UI, labels,
  ...
  "technically superior" defaults to, and defaulting is how 078 got cold.)
...
Verified at `http://localhost:8642/086-mudavym-warmmachine/index.html` two ways: live DOM and
