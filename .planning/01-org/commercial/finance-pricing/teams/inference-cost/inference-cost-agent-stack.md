...
| `nf_a.cost_per_completed_task` has two claimed owners | [[model-routing-inference-economics-charter]] `:54` — "we produce cost-per-task"; this team's charter names it primary. Same shape as **OD-29** (`OPEN-DECISIONS.md:35`), which is open. **Recorded, not resolved**; escalation runs [[finance-pricing-charter]] → [[decision-office-charter]] |
