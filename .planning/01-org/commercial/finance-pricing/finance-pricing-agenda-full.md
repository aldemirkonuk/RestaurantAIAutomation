...
| 5 | *"OD-11 is open; any column F1 adds is a bridge"* | **OD-11 is resolved** — Path C, [[0008-nf-column-contract]], 2026-08-24, "P1 unblocked" (`OPEN-DECISIONS.md:126`). [[finance-pricing-charter]]:160 still calls it open; charters are not this wave's to edit (§8.4) | register read |
...
| **FIN-B3** | **Map the restaurant-B2B money flow the product already writes.** Decides whether FIN-B1's marketplace branch is real or decorative | A one-page map of every money-shaped field already in production, each marked **our money** or **the restaurant's money** — and a stated verdict on whether the platform-payments branch has a product surface to attach to | **2026-09-25** | `providers.payment_terms` defaults to `'Net 30'` (`baseline:4899`). **`procurement_orders.payment_due_date` does not exist** — no table in the schema declares it. The daily cron this line credited it with filtered on it, got 42703, read the failed query as "nothing due" and sent nothing for its whole life; it was deleted on 2026-09-02 ([ADR 0077](../../../decisions/0077-there-is-no-payment-due-reminder.md)). No paid state exists on any table either, so FIN-B3 has **no** money-shaped field on the payables side to map. The credits/recovery surface is [[design-partner-operations-charter]]'s, not ours |
...
1. **OD-23's premise, still first.** Is `$20–50/mo` a decision or a draft? The register
   ...
   (OD-23, `OPEN-DECISIONS.md:34`). **Not resolved here.** Nothing in this agenda depends on the
