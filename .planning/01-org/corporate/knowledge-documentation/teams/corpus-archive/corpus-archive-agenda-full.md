...
| 7 | Write the archive policy: what "finished" means, where it goes, `status: archived` + superseded-by frontmatter. **Now a greenfield task** — the old `.planning/archive/` tree is gone ([[0032-vault-cleanup-cut-line]]: archive = delete + tombstone, never an in-tree copy) rather than retrofitted | Needs [[graph-retrieval-charter]] frontmatter contract | Every entry in the next archive tree carries the marker from its first commit |
...
| 10 | Execute the OD-01 restructure, link rewrites in the same commits | ~~Founder call on OD-01~~ — ~~called 2026-08-24; vault pass done: 995 files removed~~ — **that pass was reverted the same day (no founder call had been made; see `foundation/VAULT_CLEANUP_AUDIT.md`'s restore commit). The real call is 2026-08-27** ([[0032-vault-cleanup-cut-line]]): cut line C-modified executed with links rewritten in the same branch. Legacy `md/` + `md_files/` remain | Vault done (ADR 0032); `md/` + `md_files/` still open |
