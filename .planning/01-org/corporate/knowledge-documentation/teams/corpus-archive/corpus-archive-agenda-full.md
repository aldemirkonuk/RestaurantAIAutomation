...
| 7 | Write the archive policy: what "finished" means, where it goes, `status: archived` + superseded-by frontmatter. **Now a greenfield task** — the old `.planning/archive/` was deleted 2026-08-24 ([[VAULT_CLEANUP_AUDIT]]) rather than retrofitted | Needs [[graph-retrieval-charter]] frontmatter contract | Every entry in the next archive tree carries the marker from its first commit |
...
| 10 | Execute the OD-01 restructure, link rewrites in the same commits | ~~Founder call on OD-01~~ — **called 2026-08-24; vault pass done** ([[VAULT_CLEANUP_AUDIT]]): 995 files removed, links rewritten in the same pass. Legacy `md/` + `md_files/` remain | Vault done; `md/` + `md_files/` still open |
