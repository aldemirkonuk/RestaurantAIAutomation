...
| 3 | Track A "Security" ✅ | `YC_WEDGE_PLAN.md:339` | ⚠️ **Label overstatement — grade unchanged, evidence corrected 2026-09-01.** Body accurate (ux-optimizer only); label reads as the security work being done, while the platform is still unguarded **by default** — `app.module.ts:130-137` registers only `RateLimitGuard` and `TenantGuard`, with no global `JwtAuthGuard`. The two supports first recorded here were both stale and are now replaced: ~~94 endpoints unguarded by omission~~ is struck by OD-19 itself as *"a codebase two guard-sweeps ago"* (`OPEN-DECISIONS.md:31`), and the **E0 auth census has landed** (`ECOSYSTEM-PLAN.md:83`, 2026-09-01) — 468 route handlers, 444 authenticated, 23 deliberately public with evidence, **0 unauthenticated by omission**, 1 unclear; ~~OD-20 open and urgent~~ is **resolved** (`OPEN-DECISIONS.md:117`) — `analytics.controller.ts` has carried a class-level `@UseGuards` since PR #31, 2026-08-24. **The zero does not lift the grade: the defect count is zero while the defect generator is fully intact** — guarding is opt-in per controller, so a controller that declares nothing is unauthenticated by default and endpoint 469 arrives unguarded. Ships only with scope restored |
