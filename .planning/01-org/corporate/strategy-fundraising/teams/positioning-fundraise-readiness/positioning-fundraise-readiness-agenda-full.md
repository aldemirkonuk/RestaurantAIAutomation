...
| 3 | Track A "Security" ✅ | `YC_WEDGE_PLAN.md:339` | ⚠️ **Label overstatement — grade unchanged, evidence corrected 2026-09-01.** Body accurate (ux-optimizer only); label reads as the security work being done, while the platform is still unguarded **by default** — `app.module.ts:128-137` registers only `RateLimitGuard` and `TenantGuard`, with no global `JwtAuthGuard`. The two supports first recorded here are both stale: ~~94 endpoints unguarded by omission~~ is struck by OD-19 itself as *"a codebase two guard-sweeps ago"* (`OPEN-DECISIONS.md:31`), and the reconciled count is **pending the E0 auth census** (`ECOSYSTEM-PLAN.md:80`, PR #194) — do not seed a replacement figure; ~~OD-20 open and urgent~~ is **resolved** (`OPEN-DECISIONS.md:116`) — `analytics.controller.ts` has carried a class-level `@UseGuards` since PR #31, 2026-08-24. Ships only with scope restored |
