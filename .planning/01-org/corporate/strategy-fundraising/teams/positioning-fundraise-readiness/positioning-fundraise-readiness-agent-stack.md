...
|---|---|---|---|---|---|
| `claim-register-entry` | T2 | Any claim about to go outward, written or spoken | A row whose `evidence` field is a query id, `path:line` + symbol, or a demo path — prose fails the run | The **573** insight-type figure entered the YC narrative at `YC_WEDGE_PLAN.md:324` while the corpus also claimed 375; the register would have entered it `BLOCKED`. Checkable: the dispute was staged as OD-33 and **settled at 573 on 2026-08-26** by transpiling `insight-catalog.ts` standalone (`OPEN-DECISIONS.md:38`) — the block would have lifted on a measurement, which is the state machine working | NEW |
...
- **One charter line is already stale, reported here and not resolved here.** The charter calls
  ...
  (OD-33, `OPEN-DECISIONS.md:38`) and the 375 source is now a retired stub.
