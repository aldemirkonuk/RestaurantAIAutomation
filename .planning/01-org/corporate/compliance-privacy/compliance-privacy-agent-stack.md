...
| `nf_b.research_store_erasability` has neither an owner nor a register row | Verified 2026-08-27: `OPEN-DECISIONS.md` contains no erasability entry, and [[privacy-engineering-loops]]'s `nfb-research-store-erasability` loop is `owner: UNASSIGNED — escalated`. **The fork stays open here** — an ADR 0006 append-only store with no per-subject removal mechanism is the department's most consequential unanswered question, and it gets harder every day the store accumulates rows |
