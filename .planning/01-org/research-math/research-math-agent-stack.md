...
|---|---|---|---|---|---|
| `applied-ai-seam-audit` | T2 | Monthly, and any new charter or OD row naming an owner for work an RM team owns | Same job **and** same metric in two units → a merge proposal filed at the named unit (never a duplicate); else the word "clean" | OD-29 (`decisions/OPEN-DECISIONS.md:36`) — RM-1 and `aio-model-routing` share a mandate and the NF-A cost metric, found by hand cross-reading; and the TECH-F3 / OD-21 id collision (`OPEN-DECISIONS.md:145`) reconciled after the fact | NEW |
