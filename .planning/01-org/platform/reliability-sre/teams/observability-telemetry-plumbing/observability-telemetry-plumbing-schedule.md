...
| `nf-a-coverage-report` *(proposed)* | T2 department | Weekly, and on demand before any L4 claim | Coverage computed over agent tasks with whole-tuple grading, plus the three most-missing fields named | **Yes** — `decision_log` and `api_spend` cover parts of the tuple from two writers and cannot be joined (`technology.md:745-746`) |
