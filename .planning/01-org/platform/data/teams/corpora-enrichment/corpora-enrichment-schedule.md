...
| `menu-extraction` | T1 Domain | A menu PDF, photo or HTML snapshot lands in `datasets/annotation_inbox/` | Line items extracted with per-field confidence, written with `source_guarantee = scraped`, unresolved lines queued rather than guessed | `.planning/07-reference/MENU_EXTRACTION_SCALE_PLAN.md`, `datasets/menu_corpus/` |
