...
BEGIN
 ...
 INSERT INTO notification_preferences(restaurant_id,user_id,email_enabled) VALUES(house,person,false);
 SELECT to_jsonb(p) INTO expected FROM notification_preferences p WHERE restaurant_id=house AND user_id=person;
 ...
 IF EXISTS(SELECT 1 FROM notification_preferences WHERE restaurant_id=house AND user_id=person) THEN RAISE EXCEPTION 'undo kept an unstated preferences row'; END IF;
 -- Two fields of one preferences row can be restored in reverse order.
 INSERT INTO notification_preferences(restaurant_id,user_id,email_enabled) VALUES(house,person,false);
 SELECT to_jsonb(p) INTO prior FROM notification_preferences p WHERE restaurant_id=house AND user_id=person;
 UPDATE notification_preferences SET push_enabled=false WHERE restaurant_id=house AND user_id=person;
 SELECT to_jsonb(p) INTO expected FROM notification_preferences p WHERE restaurant_id=house AND user_id=person;
 ...
 -- A person holding a preferences row in a SECOND house must not lose it to
 -- the first house's undo (measured 2026-09-17, codex-audit/C2-adopt.md #5:
 -- keying the restore on user_id alone deleted whichever house's row a
 -- for-update scan without ORDER BY happened to see first).
 INSERT INTO notification_preferences(restaurant_id,user_id,email_enabled) VALUES('99999999-9999-4999-8999-999999999999',person,true);
 SELECT to_jsonb(p) INTO prior FROM notification_preferences p WHERE restaurant_id=house AND user_id=person;
 UPDATE notification_preferences SET push_enabled=false WHERE restaurant_id=house AND user_id=person;
 SELECT to_jsonb(p) INTO expected FROM notification_preferences p WHERE restaurant_id=house AND user_id=person;
 entries:=jsonb_build_array(jsonb_build_object('id',row_id,'target','notifications','field','push','value',false,'before',prior,'after',expected,'status','written'));
 UPDATE configuration_batches SET rows=entries WHERE id=batch_id;
 PERFORM public.arrival_restore_entry(house,person,batch_id,row_id,'notifications');
 IF NOT EXISTS(SELECT 1 FROM notification_preferences WHERE restaurant_id='99999999-9999-4999-8999-999999999999' AND user_id=person) THEN
   RAISE EXCEPTION 'undo of one house''s preferences deleted a second house''s row for the same person';
 END IF;
 DELETE FROM notification_preferences WHERE restaurant_id='99999999-9999-4999-8999-999999999999' AND user_id=person;
