...
**Measured main tip (refresh 2026-09-22 23:53Z / 19:53 ET):**
GitHub `main` = `cc73f9f66` (squash merge of #415). Post-merge `main` CI and
deployment status were not yet checked at this refresh.

## Current continuation status

| PR | Current head / state | Current blocker and next action |
|---|---|---|
| **#414 get-started** | `8b9fd84da`; open; **BLOCK**; now `CONFLICTING` / `DIRTY` after #415 reached `main`; required checks were green against the prior base | Exact-SHA gate: `pr-audits/414-8b9fd84.md`. Blockers: record the binding flyleaf/three-count/sketch-115 rulings in an ADR; move OD-140 into the real Open table; merge/rebase current `main` and resolve seven conflicts; renumber `20260922190000`–`190300` past main's `20260922210200` ceiling; rerun required CI + full gate on the new SHA. Founder hold remains: do not merge; do not flip `mudavym_design_arrival`. |
| **#415 admin** | `9af065bf3`; **merged** 2026-09-22 23:53Z | Required CI green; full three-angle audit approved with notes; mandatory adversary `HOLDS`; founder authorized gate-owned `.github/workflows/ci.yml`; squash merge `cc73f9f66`. Audit: `pr-audits/415-9af065bf3.md`. |
| **#430 ask + authorize** | `7d734e3f5`; open, `MERGEABLE` / `BLOCKED`; checks rerunning | Prep done and pushed: seven migrations renumbered to `20260922200000`–`200600` (clear of #414 `190000`–`190300` and #415 `210100`–`210200`), citations updated, `origin/main` merged (`3579fa0c7`). Detail: `.planning/07-reference/pr-audits/430-prep.md`. Gate-owned: `.github/workflows/ci.yml`, `.planning/decisions/README.md`. Wait for CI/Audit Gate on exact SHA; do not audit/merge without founder authorization. |
| **#434 cellar** | `320d06736`; open, mergeable, `BLOCKED`; Audit Gate red / checks rerunning | **Q9 wired and pushed** (2026-09-22): `readTillLines` mines tenant-scoped `pos_checks.items` for non-wine live sales (voided skipped; `is_wine===true` left on unresolved to avoid double-count); cellar “When it sells” already buckets that till book via `rowSeries.whenItSells`. Honest empty-until-sales left in place — no fake series. Tests: 3/3 Q9 pins green; pin fails without `pos_checks.items` in source. Claim `ADR-0160-Q9-NON-ALCOHOLIC-HEATMAP-LIVE-SALES`. Main merge-in is on the head (`028874043` + Q9 `320d06736`). Gate-owned: `.planning/decisions/README.md`. **Do not audit/merge** until founder confirms empty-until-sales counts as accepted “wired,” then CI green + full 3-angle + adversary at this exact SHA. |
| **#454 preview** | `988600e74`; open, mergeable / `BEHIND`; 36 success / 4 failure / 1 skipped | Still explicitly held by its PR body: “Do not merge without his review.” Leave untouched and do not merge. |
| **#455 arrival / first proof** | `feat/arrival-first-proof`; open; **do not merge**; flag `mudavym_design_arrival` stays off | Second-pass BUILD on the same branch. Built: source-page split / honest empty crop; footer Add it → `addMenuItem`; `/house` contents with exactly one ask (supplier); SMS-consent line when mobile is filled; generous drop-well; pencil = unmatched fallback + missing/unknown category (no invented certainty). Still stubbed: per-line extractor crops (R4 boxes), Places/Google live preview, last-invoice inscription, cellar-register ask as a later house line. Do not flip the flag. Do not touch #414 / #454. |
...
**Get-started post-122:** founder approved all 17 Opus recs as A on 2026-09-22. Locked in [ADR 0213](../../decisions/0213-get-started-is-account-then-house-then-first-proof.md) (OD-134–139, OD-141). Build is PR **#455** `feat/arrival-first-proof` — do **not** merge #414 or #454; do **not** flip `mudavym_design_arrival`. §5 Q2 is superseded by F7=A (one footer line). Second-pass stubs above are the remaining honesty residuals, not new product forks.
