...
| **Southern Glazer's Proof** | `shop.sgproof.com/robots.txt` 200, 791 B: allows all but cart/checkout/my-account, **and publishes `Crawl-delay: 10`, `Request-rate: 1/10`, `Visit-time: 0400-0845`** — honoured, and **no page on that host was fetched** because the request would have been at 12:18 UTC. `my.sgproof.com/robots.txt` returns `text/html` (a Salesforce shell), so no rules are readable from it. `southernglazers.com/terms-of-use` 200: forbids "any robot, spider, or other automatic device … including monitoring or copying any of the material", and separately **"agree not to provide any other person with access to this Website or portions of it using your username, password, or other security information"** | The credential clause is decisive for the corporate site: **declaring an SG Proof login to this product would be the breach**, before any request is made. **CORRECTED 2026-09-05 (ADR 0126's three-angle research):** those Terms define "Website" in their own first paragraph as **`southernglazers.com`**, which is not the buyer portal. **The Proof portal's own terms have never been read** — its visit window was shut on both passes — so the portal's position is UNKNOWN, not permitted. `distributor-feed.registry.ts`'s `southern-glazers-il` row now says exactly that, and the sentence a house reads no longer applies one host's terms to another |
...
**The two routes a class-C row can actually arrive by, built 2026-09-05** (ADR 0126, the founder's
batch 56: *"Invoices + the built 810 ingest, and a letter for a feed"* and *"Build the ingest route
and the panel"*). Neither holds a credential and neither reaches a distributor:

| Route | What it takes | What it does |
|---|---|---|
| `POST /procurement/documents` (store: any authenticated member; **price: manager or owner**) | The same door every invoice goes through — **not a second door**. An 810 or an 832 a person obtained, plus `distributorKey` for a catalogue and `declaredCurrency` when the file states no `CUR` | An 810 is read as an invoice by the existing X12 path. An 832 is stored as a `price_list` document (sha256, uploader, time, sender) and its lines are admitted to `vendor_price_observations` only under the code meanings the house has stated. The answer carries a per-line report — what was priced, and for every refused line the reason and the code that refused it, plus the unmapped codes by name. It never returns a bare row count. The store stays open to staff (a runner photographs paper at the door); the price-writing half runs `assertCanManageRestaurant` and refuses in the report, not with a 403 over a file already stored |
| `GET /distributor-feed/letter` | Nothing | The invoice-feed request letter the HOUSE signs on its own letterhead, Southern Glazer's first, asking for an EDI 810 feed or an order-guide equivalent. **This product never sends it** and has no route that could. Text: [`DISTRIBUTOR-INVOICE-FEED-LETTER.md`](DISTRIBUTOR-INVOICE-FEED-LETTER.md) |

Both are surfaced on `/connections` beside the per-distributor measurement (the robots rule and the
terms clause verbatim, the day they were read, `connectable: false` and why).
