...
- [ ] **TEST-PROD-04**: Wave D — Toast pipeline: a HMAC-signed test Toast webhook delivered to `POST /api/v1/pos/webhook/toast` flows through POSIntegrationAgent → InventoryEngine → NotificationAgent using `restaurant_id: "e2e-test-restaurant"` staging data. Supabase records created with deterministic IDs, verified, and torn down in session teardown. **Retired 2026-09-12, will not be satisfied** — the endpoint has no product caller and the table it wrote has no `CREATE TABLE` in the live schema; see [ADR 0137](../decisions/0137-legacy-e2e-waves-d-e-g-are-retired-not-repointed.md).
- [ ] **TEST-PROD-05**: Wave E — Gmail pipeline: a low-stock alert triggered via the NotificationAgent produces a `notification_deliveries` row in Supabase with `channel: "email"` and `status: "sent"` or `"delivered"` within 30 seconds. **Retired 2026-09-12, will not be satisfied** — its trigger mechanism (a raw table upsert) was never wired to any live event path; see [ADR 0137](../decisions/0137-legacy-e2e-waves-d-e-g-are-retired-not-repointed.md).
- [ ] **TEST-PROD-06**: Wave F — Frontend smoke (Playwright headless Chromium against production Vercel URL): login redirect succeeds, `/admin/health` displays ≥7 agent cards with Active status, dashboard loads without JS console errors in < 5s, one `/studio` write-flow completes and is torn down. **[2026-09-17: Wave F is now the ADR 0135 browser walk (`apps/web/e2e/nightly/`): read-only, four states, no agent-card assertion; `prod-smoke.spec.ts` deleted.]**
- [ ] **TEST-PROD-07**: Wave G — Calendar: a `calendar_events` row with `event_date = today + 7 days` and `id = "e2e-cal-001"` is upserted into Supabase; a corresponding scheduled reminder record is verified to exist — confirming CalendarAgent will dispatch the T-7 day reminder without needing to wait 7 days. **Retired 2026-09-12, will not be satisfied** — the Google Calendar sync it verified was investigated and explicitly cut 2026-08-26; see [ADR 0137](../decisions/0137-legacy-e2e-waves-d-e-g-are-retired-not-repointed.md).
- [ ] **TEST-PROD-08**: All 7 wave test results exported as JUnit XML via `pytest --junitxml=test-results/wave_{X}.xml` and uploaded as GitHub Actions artifacts. Wave F Playwright JUnit XML also captured and uploaded. **Waves D, E, G retired 2026-09-12** — 4 waves (A, B, C, F) now, not 7; see [ADR 0137](../decisions/0137-legacy-e2e-waves-d-e-g-are-retired-not-repointed.md).
...
- [ ] **TEST-PROD-10**: Full test suite (Waves A–G) completes in < 10 minutes. Waves B+C run in parallel via `pytest -n 2`. GitHub Actions `timeout-minutes: 15` hard cap. **[2026-09-17: raised to 25 by the ADR 0135 rebuild (commit `5e4f21f1`); the browser walk alone measured 3.0 min from a laptop on 2026-09-17 and CI has never run it. Waves D, E, G retired by ADR 0137.]** `PYTEST_RUNNING` is NEVER set in the e2e-prod.yml CI environment (would disable Sentry). **Waves D, E, G retired 2026-09-12** — see [ADR 0137](../decisions/0137-legacy-e2e-waves-d-e-g-are-retired-not-repointed.md).
