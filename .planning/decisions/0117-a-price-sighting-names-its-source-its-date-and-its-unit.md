...
**[CORRECTED 2026-09-12: all of these ARE on production now. Verified by query that day, each has a row in `supabase_migrations.schema_migrations`: `20260904200000`, `20260905220000`, `20260905235500`, `20260905235800`. PR #289 merged and 53 of its 72 migrations applied; the remaining 19 were blocked by a defect in `20260906020000` and were applied through the connector on 2026-09-12. Do not read this paragraph as "the table is absent" -- that inference is exactly what this correction exists to stop, because arming `PRICE_INDEX_UPLOAD_ENABLED` on the belief that a missing table is a second guard would remove the only guard there is.]**
