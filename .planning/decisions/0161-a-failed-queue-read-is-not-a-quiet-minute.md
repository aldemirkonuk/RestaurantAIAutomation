...
- **Status:** Proposed (agent, 2026-09-18 — implemented in this PR and in `feat/finish-relay`'s working tree; awaiting a founder lock)
  **[2026-09-25, wave-1 comms-b lane, PR #429: this record was never committed —
  it lived only as an untracked file in `.claude/worktrees/jolly-lovelace-bc70ef`
  while #429's relay code cited it. It lands here, text unchanged below this
  bracket, together with BOTH dispatcher halves: the relay half
  (`relay-email.service.ts` `dispatchQueued`, `relay-email.cron.ts`,
  `relay/relay-dispatch-read-failure.spec.ts`) was already on #429, and the
  letters half (`house-letters.service.ts` `dispatchDue` throws,
  `house-letters.cron.ts` doc, three specs in `house-letters.spec.ts`) is
  carried over from that worktree in the same PR, because #429's relay comment
  says `dispatchDue` follows the same rule and on `main` it did not. NOT landed
  here: `scripts/check_dispatch_read_failure_is_not_quiet.py`, its CI job
  `dispatch-read-failure` (`ci.yml` is gate-owned) and the four `ADR-0161-*`
  CLAIMS rows (one is a tripwire on that guard's registry). Until they land,
  "Enforced three ways" below is true of the behavioural specs only. Still
  Proposed — no founder lock is recorded.]**
- **Date:** 2026-09-18
...
- **Landing order.** **[2026-09-25: superseded — both halves land in PR #429; the guard, CI job and CLAIMS rows do not, see the Status bracket.]** The relay half lives only in the **uncommitted** working tree of another session (`/Users/aldemirkonuk/Projects/wt-fin-relay`, files still staged-not-committed); if that session is lost the relay half is lost with it and only the tripwire remains. When it merges with this, the guard finds `dispatchQueued` by rule 2 without registration, and the tripwire claim forces `EXPECTED` to name it.
...
| 2026-09-25 | agent (wave-1 comms-b) | Committed in PR #429 with both dispatcher halves; guard, CI job and CLAIMS rows left in the untracked worktree (ci.yml is gate-owned) |
