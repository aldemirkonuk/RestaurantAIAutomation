...
## 2026-09-25 — carried onto main, and joined with ADR 0213

Branch `fix/sessions-follow-membership-r6`, from `origin/main` `f7630c08b`
(#463). The lane's newest state was the pushed ref
`origin/wip/2026-09-21/sessions` (`900266b7c`), not the older dirty
`wt-sessions` tree; it was squash-merged so the "do not merge" wip
snapshot does not enter main's history. No migration is involved (the lane
adds none), and no ADR or OD number was created.

Main had moved 42 commits past this lane's base `cb756083e`. Two of them
changed what this decision has to cover:

- **ADR 0213's account-only signup (#455).** A new account now exists
  before its house, so "0 memberships, no house and no role" is a normal
  state for a new owner, not only for someone removed. Joined as follows:
  `registerAccount` and `registerAccountWithGoogle` mint a session in no
  house; `createFirstHouse` writes the owner membership row and then mints
  through `generateTokens` naming that house; `POST /auth/register/house`
  is `@AllowsNoHouse()` (the fifteenth marked route, pinned in
  `no-house-session.spec.ts` and the `ADR-0164-NO-HOUSE-NO-ROLE` claim);
  `/verify-email` asks for onboarding progress only when the new session
  names a house. Without these, a verified new owner was sent to the
  chooser and then `/no-access`.
- **Four new `@Roles` controllers** (conversations, the house counter and
  day, report exports). Every route re-measured with the exact guard and
  with main's widened one: the only difference is `admin`, which this
  decision stops widening to. `route-access.expected.json` is 118 rows.

Also on this branch: `/no-access`'s public-door branch (the one that
renders in production, since `isPublicDesignOn()` is always true) now
shows "Your access to {house} has ended." like the chooser, and both
...
still wears `AuthShell`); the phone's `houseChoice.test.ts` was not run
here (the mobile checkout has no `jest` installed, and CI does not run
