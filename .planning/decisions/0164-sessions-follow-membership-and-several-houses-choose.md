...
- Revisit when: `feat/login-flyleaf-endpaper` lands and `ChooseHouse.tsx` needs its `EndpaperShell` wrapper (F4's named follow-up above); a person with more than 8 houses appears (search is built, untested on real data); a second client starts minting sessions outside `generateTokens`.
...
5. **Register, closed this round.** `OPEN-DECISIONS.md` had no entry for
   ...
   a repeat of round 1's MEDIUM #4. Filed as OD-124 (F1/F2/F3/F5), OD-125
   (F4, carrying the ADR 0143/0149 citation above), and OD-126 (open items a
   and b) — **renumbered the same day to OD-129, OD-130 and OD-131** once
   124–128 turned out to be allocated to other lanes; every citation below
...
`OD-131(a)` describe block, 5 cases): the four refusal cases fail against the
pre-fix code (the create succeeds when it should be refused) and pass after;
CLAIMS row `ADR-0164-ORG-OWNERS-ACTIVE-HOUSE`, mutation-tested (the refusal
removed → red on all four; restored → green). See `OPEN-DECISIONS.md`
OD-131.
