...
- **Status:** Locked 2026-09-12 by the founder in session — his words, answering whether to park or land it: *"Lock 0132 and land it"*. (Built on `feat/connect-mudavym-mcp-server`; ported onto main `beb00db4`, migration re-versioned `20260906170000` → `20260912200000`.)
- **Date:** 2026-09-06
...
1. **`TenantGuard` fails open.** It returns `true` when `request.user` is unset
   (`common/tenant/tenant.guard.ts:70-75`) and an MCP request never sets it, so tenancy
...
| 2026-09-12 | Aldemir (founder) | **Locked** in session — *"Lock 0132 and land it"*, answering whether to park or land it. Ported onto main `beb00db4`: the migration was re-versioned `20260906170000` → `20260912200000`, because the old version sorted before migrations already applied on main and duplicated the version of `20260906170000_a_vendor_states_its_usual_currency_and_an_order_carries_one.sql` (`schema_migrations` keys on version, so `supabase db reset` would have died on it). The `TenantGuard` fall-open citation moved with main's edits to `tenant.guard.ts:70-75`. Tenancy tightened in the port: `logs.timeline` no longer accepts a correlation id, because `LogsTimelineService.getTimeline` reads `event_store` by correlation id alone (`logs-timeline.service.ts:122-127`) and a key could have named another house's events that way. |
| 2026-09-21 | Claude (round 5 fix pass, CLAUDE.md §5b) | **`event_store` reads are now also scoped by the WRITER-STAMPED house, not correlation id alone** — the tightening round 3's must_fix asked to have recorded here. `event_store` carries no `restaurant_id` column; `LogsTimelineService.fetchEventStore` (`logs-timeline.service.ts:341-354`) now filters `.eq("correlation_id", correlationId).contains("payload", { restaurant_id: restaurantId })`, reading the house every current production writer (`inventory_engine.py`) stamps into `payload.restaurant_id` — an unattributed row is withheld rather than guessed. Re-verified directly against both trees rather than trusted forward: `git show 8c5219516:apps/api-gateway/src/logs/logs-timeline.service.ts` (this lane's own snapshot) carries the filter; `git show origin/main:apps/api-gateway/src/logs/logs-timeline.service.ts` still calls `fetchEventStore(db, correlationId, cursor)` with no `restaurantId` argument, i.e. the row above (2026-09-12) is still an accurate description of `origin/main` today — this row describes this lane only, not yet ported. The row is the correlation-id-alone read this row's own 2026-09-12 entry described; that description is superseded here for this lane's tree, not retracted for main's. `check_decision_claims.sh` row `ADR-0132-NO-UNSCOPED-TIMELINE-LEG` continues to hold (360/360, this pass) against the stronger filter. |
