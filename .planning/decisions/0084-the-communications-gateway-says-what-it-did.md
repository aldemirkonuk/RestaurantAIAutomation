...
| 2026-09-17 | [[0149-mudavym-is-the-only-design-finish-every-page-then-delete-legacy-once]] #19 | The open relay closed: two locked doors (service key naming house, vendor and conversation or order; owner/manager JWT to the house's members and vendor contacts) and an audit row per send, in `apps/api-gateway/src/communications/relay/`. The route stays, at the same path [corrected the same day: the JWT door checks and then refuses 409 — a person's mail does not leave from the shared mailbox (ADR 0118 D1/D2), and its sender is undecided] [superseded later the same day: the founder answered which mailbox — the house's own connected `gmail_send` grant, naming the person as author; refuses with `house_mailbox_not_connected` and a plain sentence only when the house has none] |
