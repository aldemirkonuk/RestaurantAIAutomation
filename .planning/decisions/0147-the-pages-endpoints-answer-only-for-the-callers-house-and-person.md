...
| providers | [2026-09-25, PR #412] **every** `providers.controller.ts` route takes the house from `houseOf(user)` (403 for a session naming none) and the person from `userId`. Every provider-id read and write (orders, performance, rate, contacts, contact-date, intelligence, locations, retroactive order, delete) calls `getProvider` first, so another house's id is a 404. `PATCH :id` no longer drops its house filter when the session names no house; bulk import no longer writes `restaurant_id` NULL; `PATCH :id/usual-currency` no longer asks the role of `undefined` | PR #412; claim ADR-0147-PROVIDER-SUBRESOURCES-HOUSE-SCOPED |
...
- **Notification senders.** POST `/notifications/order-approval`, `low-stock`,
  ...
  **[Built 2026-09-17, founder answer 15 of [[0149-mudavym-is-the-only-design-finish-every-page-then-delete-legacy-once]]
  ...
  `NotificationsService` methods stay for internal producers.
  **[2026-09-25, lane W2-fix-comms: the `send-email` half below was built on this branch
  and then dropped from it before merge. The founder, round 4 item 14, took the
  recommended option: *"close the endpoint (merge #410; #422 drops HouseEmailService +
  its caller edits)"*. `HouseEmailService`, `SendHouseEmailDto`, the web
  `sendHouseEmail` helper and the three callers' rewiring are gone from PR #422;
  `POST /notifications/send-email` is left exactly as on main, and PR #410 closes it
  (403, never sends). The text is kept as the record of what was built and reviewed,
  not as a description of what ships. The five closed senders and their spec stand.]**
  `send-email` now takes a
...
`20260927140000_a_persons_mail_queues_like_the_houses_own.sql`; ADR 0118 D2
...
| 2026-09-25 | Lane W2-fix-comms (founder round 4 item 14) | `send-email` dropped from this PR: `HouseEmailService`, `SendHouseEmailDto`, the web `sendHouseEmail` helper and its test, and the QuickGmailModal / email-scheduler / RecurringOrders edits removed; the route handler, `NotificationsService.sendEmail` and the three callers are unchanged from main in this PR. PR #410 closes the route. The send-email tests left `notification-senders-are-closed.spec.ts`; the five-senders and preference-DTO tests stay. Three CLAIMS rows that pinned `HouseEmailService` were removed from this PR (they never reached main). |
| 2026-09-26 | PR #422 audit fix (ADR 0090 BLOCK at `b479ce4fd`) | Three gaps closed in this PR, each a CLAIMS row. (1) Row 39 was unimplemented at one live send path D5 missed: `RecommendationDigestService.readMemberPrefs` read `notification_preferences` by `user_id` alone, so a member with no row in this house but one in another had that house's email switch and quiet hours decide this house's digest (stricter than the defaults, never looser: digests silently held or expired, not a data leak). It now filters `.eq("restaurant_id", restaurantId)`; the new spec fails on the pre-fix code (toggled by hand). Re-swept after the fix (grep of every `from("notification_preferences")`/`table(...)` in apps/ and services/, 12 query sites): each now filters by house, except `NotificationsService.getPreferences`, whose filter applies only when its optional `restaurantId` is passed; every caller passes one today (audit note, not changed here). (2) Migration `20260925160500`'s `COMMENT ON CONSTRAINT` said `registerPushSubscription` upserts on `(restaurant_id, user_id)`; it upserts `notification_push_devices` on `(user_id, endpoint)`. Comment and header corrected before merge. (3) `unregisterPushSubscription`'s docstring said "forget this browser"; it deletes every device the caller registered (the DTO carries no endpoint). Docstring corrected; behaviour unchanged. Not done: narrowing unregister to one device (needs the client to send its endpoint). |
