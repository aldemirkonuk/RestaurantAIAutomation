...
- **2026-09-04 — the collapse landed; the surface count fell by four tabs and three registers.** The founder, asked the two questions this ADR left open at the bottom, answered *"Move the registers and collapse the four tabs."* Measured: `/settings` goes from **14 registers to 10 plus one line out**, and `/profile` from **7 registers to 5**. Net, with the flag on, one new route in exchange for seven surfaces — which is the arithmetic the rejected alternative below said had to hold, and until this it did not. Two consequences of this ADR are now stale and are corrected here rather than in place: *"`/profile` is unchanged except for one pointer line"* is no longer true, and *"nothing is moved out yet"* is no longer true. One thing did **not** move whole: Register IV **split** along this gateway's own role gates — declare / probe / secret / revoke are `assertCanManageRestaurant` and moved; `PUT /mcp-connections/:id/consent` has no role check (`mcp-connections.controller.ts:218-235`), and since `/connections` is manager-only, moving it would have left a staff member with no way at all to stop a server acting in their name. That is decision 2 of this ADR ("house declares, each person consents") applied to placement. The collapse also **subtracted one capability** and said so: adding or removing a card had no home while the flag was on, because `StripeCardPanel.tsx` was bound to `/profile`'s data hook and UI kit — `connections.md` §9 G-C9, `profile.md` §9 G12a.
- **2026-09-05 — the subtraction is repaid; the surface count still falls.** Asked whether the card panel should follow the register, the founder answered *"port the card panel to /connections now."* It was moved rather than copied: `pages/profile/next/StripeCardPanel.tsx` → `apps/web/src/components/mudavym/StripeCardPanel.tsx` (with `stripe-js.ts`, its only `loadStripe` caller), its data prop narrowed to the two functions it actually calls (`CardPanelClient`, which `ProfileNextData` satisfies structurally) and its four `pf-ui` primitives redrawn over the house tokens. **One component, two callers, no fork** — `/profile` renders it with the flag off, `/connections` Register II with it on. So the arithmetic in the status line above now holds with NO capability subtracted: one new route in exchange for four tabs and three registers, and nothing lost on the way. `connections.md` §9 G-C9 and `profile.md` §9 G12a are closed; measured on this tree by `npx vitest run src/pages/profile/next src/pages/connections/next` — 122 passed / 3 files, with 7 of 49 connections cases and 1 of 65 profile cases failing against a same-depth `git show HEAD:` probe. **Still not settled by this:** adding a card redeems no seal (`profile.md` §9 G-PAY-SETUP); the port did not change that and widened it from one page to two.
- **Date:** 2026-09-03
