...
3. **The lock-screen rule.** Locked, a notification shows the event, the house, the order number and a count. Unlocked, it adds the vendor, the amounts and the draft. Email bodies and bank details never leave the app. **A house cannot override this.**
   **Founder decision (2026-09-19, batch 7): the push payload is rich, and the OS redacts the lock screen.** The payload carries the vendor, the amounts, the draft and the vendor-mail gist, so an unlocked pull-down shows the proposal in Phase 1 without native code. The lock screen shows only the rule's locked fields:
   - **Android:** the channel is created with `lockscreenVisibility` PRIVATE, and its public version carries only the locked fields. [BUILD NOTE 2026-09-19, #403 adversary: expo-notifications 0.32.16 never calls `setPublicVersion`, and in the background the system tray draws the FCM notification message, not app code (`ExpoHandlingDelegate.kt`). So Phase 1 on Android gets PRIVATE with no public version, and the lock screen reads "Contents hidden". That errs safe and shows less than the locked fields. The public version needs Phase 2 native code or a data-only message.]
   - **iOS:** a lock-safe placeholder, relying on "Show Previews: When Unlocked" (the iOS default on Face ID devices; not verified for every Touch ID model).
   The founder accepted two costs: the vendor names and amounts transit Expo, APNs and FCM, and a user who sets iOS previews to "Always" sees them while locked. Email bodies and bank details are still never in the payload. The rejected alternatives were (a) lock-safe fields only, with the details fetched after the app opens, which cost the Phase 1 pull-down its proposal, and (b) rich on Android with lock-safe on iOS. Phase 2 native extensions may later fetch the details after unlock instead. Notifications carry the real Mudavym logo (`apps/web/public/logo.png`), as in the approved mockup.
