...
**The repair was itself vacuous, and this PR's own CI is what proved it.** The repaired script still shelled out to ripgrep, and `rg` is **not installed on GitHub's `ubuntu-latest` runner** — nor is it reachable from a `bash script.sh` on the founder's Mac, where `rg` is a shell function rather than a binary. So the repaired guard took its new CANNOT-CHECK branch on *every* run and exited 2: no longer lying, and still **never once having examined a line of code**. It reddened `Lint TypeScript` on this PR, which is the only reason anyone found out. A guard whose sole reachable outcome is "I cannot check" is not a guard; it is the same hole wearing an honest label. The search is now `grep -rn -E`, present by definition, with the pattern rewritten from PCRE to POSIX ERE (`(^|[^A-Za-z0-9_])` reproduces `\b` exactly here, so `old_stock_live` is still not a match — the property the file+content allowlist depends on) and `grep -r` called directly rather than through `xargs`, which collapses grep's "no match" 1 into its own 123 and would have re-introduced the exit-code confusion this file exists to record. **Measured, in five runs:** clean tree → PASS, exit 0, **973 `.ts`/`.tsx` files examined**, 4 occurrences all allow-listed — the first inspection this guard has ever performed; a planted `stock_live: 42` → **exit 1** naming it, with `old_stock_live: 7` on the next line correctly ignored; `grep`/`find` off `PATH` → exit 2; no `apps/` directory → exit 2; a 3-file corpus → exit 2. The ADR-0078 claim asserting the old ripgrep-absent behaviour is superseded by one asserting the file count, because a claim that the guard exits 2 was satisfied by the defect.
