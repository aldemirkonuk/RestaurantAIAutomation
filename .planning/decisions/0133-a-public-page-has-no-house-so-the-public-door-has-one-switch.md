...
| 2026-09-18 | founder, recorded as [[0149-mudavym-is-the-only-design-finish-every-page-then-delete-legacy-once]] row 48 on `train/finish-2` | **AMENDMENT: Decision 1's "off is byte-identical" clause is waived, effective from decision 0149 row 37 (2026-09-17, "turn the public design switch on now").** That row made the house treatment permanent-on in code; the `localStorage["mudavym.design.public"]="off"` path this ADR's Decision 1 kept reachable and compiling **[CORRECTED 2026-09-19, wave-5 lane C: the previous wording put "so the legacy rendering stays reachable and compiling" in quotes as if Decision 1 (above) said it — it does not. Decision 1 says only "Absence is off, and off is byte-identical"; the keep-it-compiling rationale is this row's own gloss, not a quotation.]** is QA-only from row 37 forward, not a second production rendering this ADR's byte-identical promise still binds. It stays reachable, and it stays compiling, but nothing requires it to render the same output as before the public-doors fix pass — and as of that pass it does not: `handleResend` in `VerifyEmail.tsx` is one function shared by both renderings, so removing its duplicate `toast.error` call (D5c — both renderings already showed `error` inline; the toast was a second, dismissable surface for the same failure) changed the legacy path's behavior too, not only the house treatment's. That is the kind of divergence Decision 1's byte-identical clause would have blocked before this waiver. The QA path is deleted outright at 0149's gated cutover, per ADR 0149 **[CORRECTED 2026-09-19, wave-5 lane C: not "per this ADR's own Decision 1" as previously written — Decision 1's 2026-09-06 text says nothing about a cutover deletion; that fact was added above as Decision 1's 2026-09-16 amendment bracket, which itself cites ADR 0149 as the source]**; until then it is kept compiling and nothing more. |
