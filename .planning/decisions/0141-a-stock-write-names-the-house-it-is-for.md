...
1. **The gateway asserts ownership before the write.** The check
   ...
   and called by every path that takes an inventory id from a request: [CORRECTED 2026-09-12 by the adversarial pass on PR #361: false as first written. Four live routes took an inventory id from a request and reached stock through SQL wrappers that derive the house from the item and call `apply_stock_movement` without `p_restaurant_id`, so neither database refusal ran: `POST /inventory-ledger/inventory/:inventoryId/reconcile` and `/inventory/:restaurantId/item/:itemId/count` via `record_stock_count`, `/pour` via `record_glass_pour`, and `/transfer` via `transfer_stock`. A PGlite probe on this tree's migrations took a foreign house's lots from 9 to 0. All four now call this check before their RPC, with a spec per route that pairs the refusal with a control.] [CORRECTED AGAIN 2026-09-12 by the second adversarial pass on PR #361: the four were not all. (a) `PATCH /inventory/:restaurantId/item/:itemId` with `stockLive` or `shadowStock` reached `set_stock_absolute` -- a wrapper that calls `apply_stock_movement` without `p_restaurant_id` -- through `InventoryService.updateInventoryItem`, whose restaurant-scoped read kept `data` and never refused on null; the probe took a foreign house's lots from 9 to empty. (b) A POS glass line poured another house's stock: `PosHubService.upsertItemMapping`, the one writer of `pos_item_mappings.inventory_id` (behind `POST /pos-hub/mappings`, the catalog matcher's auto-map and approve, and the sale-unit review), stored any id; `applyStockEffects` read the item's volumes with no restaurant filter and poured through `record_glass_pour`, which takes no restaurant, and `ToastService.applyOrderSaleEffects` had the same shape; the probe wrote a 5-glass sale into the other house. Now covered: `updateInventoryItem` calls this check before any write; `upsertItemMapping` calls it before the upsert; both depletion paths pour only an id that a read scoped to the house returned and queue anything else. Specs: `stock-wrappers-refuse-foreign-items.spec.ts` and `mapping-inventory-ids-belong-to-the-house.spec.ts`, each refusal paired with a control. The guard `scripts/check_stock_wrappers_check_ownership.py` could not have seen either route -- it listed five method names -- and now fails on any call site of the four wrappers in a method not on its reviewed allowlist.]
...
- A cross-tenant stock write through the gateway is refused with a sentence
  naming the item, and nothing is attempted. [CORRECTED 2026-09-12: true only after the four wrapper routes named under Decision 1 were gated; before that correction a cross-tenant write still succeeded through them.] [CORRECTED AGAIN 2026-09-12: still false once those four were gated -- `updateInventoryItem` (a PATCH with `stockLive`) and the POS glass path (a mapping naming another house's item, poured by `record_glass_pour` in `PosHubService.applyStockEffects` and `ToastService.applyOrderSaleEffects`) both moved another house's stock, measured by a PGlite probe on this tree's migrations. True only after those were gated as well; see the second bracket under Decision 1.] Measured: `rejects` with
...
- **`procurement_orders.inventory_id`, `procurement_document_lines.inventory_id`
  ...
  larger change and is not taken here. [CORRECTED 2026-09-12, second adversarial pass: for `pos_item_mappings.inventory_id` nothing refused at write time -- the new argument guards stock writes, not mapping writes -- and a mapping naming another house's item poured that house's stock at every POS glass sale. `upsertItemMapping` now refuses a foreign item before the upsert, and the depletion paths pour only ids a house-scoped read returns. The column itself is still unconstrained.]
...
| 2026-09-12 | Claude (second adversarial pass, then builder) | Overturned again: `updateInventoryItem` and the POS mapping / glass path still moved another house's stock, and the ownership guard listed method names so it could not see a new route. Each reproduced failing first (a gateway spec and the PGlite probe), then fixed; the reconcile race spec now refuses `select("*")`; the guard became an allowlist of every call site, with 31 mutations run against it |
