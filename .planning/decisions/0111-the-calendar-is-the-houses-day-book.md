...
- **Status:** **Proposed — 2026-09-03. Slices 1-3, the iCal fixes and slice 7
  ...
  **The scope, as fetched.** `https://developers.google.com/workspace/calendar/api/auth`
  ...
  | Migration — 3 tables, 2 columns, 1 append-only trigger | `supabase/migrations/20260912200100_a_pushed_entry_keeps_the_providers_own_id.sql` | applied on top of all **173** existing migrations in a throwaway Postgres, exit 0; a pre-fix control database without it holds none of the three tables |
...
- 2026-09-12 — **ported onto `main` and landed on the founder's word in session** (*"Land both now"*); Status stays Proposed and no decision above changed. The migration was renumbered from its 2026-09-06 version to `20260912200100` so production applies it after everything already on `main`.
