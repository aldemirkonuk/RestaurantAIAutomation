...
- **D15 — A document's vendor is resolved by IDENTITY, without a person and without a
  guess** (founder, 2026-09-11: *"do this as best practice for automation operation,
  remove human need while making sure 100% accuracy"*). Three rules, first match wins,
  on the seller's printed tax id (BT-31 / BT-32, normalised to `<COUNTRY>:<VALUE>` —
  letters and digits only, country prefix aware):
  (1) **exact, unique tax-identity match** — the value equals exactly ONE of this
  restaurant's providers' recorded tax ids → `provider_id` written at intake with
  `provider_source: matched_tax_id` and the matched-on value. 100% by construction: a tax
  id is a legal identifier, and the uniqueness is *checked by a partial unique index on*
  `(restaurant_id, tax_id_normalized)`, not assumed by the service.
  (2) **no provider carries it → create the provider from the printed identity** — legal
  name, tax id, tax office, address and country as printed; `created_from_document`,
  `provisional_until_first_order`, `created_from_document_id` set. This removes the human
  without guessing: the identity is COPIED FROM A LEGAL DOCUMENT, never inferred.
  Idempotent — a second document from the same id finds the first's row through rule 1,
  and a genuine race loses on 23505 and re-reads into it, so one vendor never becomes two.
  (3) **anything weaker stays unresolved and says so** — nothing printed, a malformed id,
  two providers sharing one id, a seller the extractor could not read, an identity with no
  seller name to file a vendor under, a self-billed or returned document, or a document
  already filed under a different vendor. `provider_id` stays NULL, the canonical read
  carries `vendorResolution.state: unresolved` with the reason **in words**, and nothing
  is written. A FAILED READ is `unavailable` with its own reason (ADR 0067), never
  `unresolved`: "this paper names no identity we can verify" and "we could not look" draw
  the same picture and only one of them is about the document.

  *Rationale.* The vendor is the key the mapping memory (D12 slice 4), the price history
  and every cost lot hang from, and it was measured absent on 15 of 15 documents because
  it had only ever come from the caller. Filling it from the trading NAME would have made
  the number go up and put a guess underneath all three; filling it from a legal
  identifier cannot, because two equal normalised tax ids are the same legal person by
  definition. The expensive direction is asymmetric — a missing `provider_id` is one
  unresolved document, a wrong one is unrecoverable once prices, lots and pairings have
  been written under it — so every ambiguity resolves toward silence.

  *Rejected.* **A person ticks a proposed vendor** — the founder's instruction removes the
  human, and a tick on a proposal manufactured from a name is a person ratifying our
  guess. **Name or fuzzy matching** — not 100% by any threshold, and it is exactly the
  failure ADR 0103 A12 rejected for shelves; two distributors trading as "Sentetik Şarap"
  are two legal persons and one of them gets the other's cost history. **"The only
  provider we have"** — true today, wrong the day a second vendor appears, and wrong
  *retroactively* for every row already written. **Caller-supplied only** — the status quo,
  measured at 0/15. **A mutable resolution column instead of a log** — a re-extraction (D5)
  re-runs resolution, and a column lets the later run erase the run that wrote the
  provider a year of cost lots now sits under.

  *What it changes in code.* `providers` gains `tax_id`, `tax_id_normalized`,
  `tax_country`, `tax_office`, `provisional_until_first_order`,
  `created_from_document_id` and the partial unique index; `restaurants` gains its own
  `tax_id_normalized`, which is what lets a self-billed document be caught from the seller
  block alone; `document_vendor_resolutions` is a new append-only log under D5's
  `refuse_append_only_mutation` trigger. The extraction contract gains `vendorTaxId`,
  `vendorTaxOffice`, `vendorAddress`, `vendorCountry`, `buyerName` and `buyerTaxId` (in
  the `extracted` snapshot — no column), with the prompt rule *transcribe, never
  construct*. `VendorResolutionService` is called from both intake doors; the canonical
  object carries `layer2.vendorResolution` and — at last — a non-null **BT-31**, which was
  NULL only because no table had the column. On the sheet it is one line under the seller:
  *"Matched on VKN 1234567890"*, or the refusal's own sentence, or nothing at all when
