...
**Rows older than the column stay unnamed, said in words** (the founder, 2026-09-06, batch 63: *"Leave them unnamed, said in words"*). A withdrawal recorded before `withdrawn_by_name` existed prints, after its date and reason, *"this withdrawal was recorded before the register began naming the person who made one, so it holds an account and no name"* (`DistributorFeedPanel.tsx`, the `withdrawnByName` branch) rather than a name joined from `public.users` at read time. No code changed for this answer; it is what 19ab0258 built, for the reason `declared_by_name` is stored and not joined: a joined name would show a later rename as the signer and vanish when the account is deleted. ADR 0126 §7 expects zero such rows in production; the migration stops with the count if any exist.
...
| Date | Reviewer | Note |
|---|---|---|
| 2026-09-16 | Aldemir (founder), in session | **Locked** via ADR 0149 row 13, alongside 0124 and 0113; the text above is unchanged and nothing was re-measured by this entry. |
