...
| 2026-09-06 | Fable — A1 proven END TO END on live rows, after #335 | **The whole arc is now measured, not asserted.** Gateway from merged `main` `81d8b91f` on :4030, Sim Meyhouse, two synthetic deliveries. **`ad64c983…`:** a door count of 5 bottles booked **5** (`bottlesMoved: 5`, transaction returned); a `PRICE_VARIANCE` proposal at ₺118,75 from each side, accepted; `agree` **201** under `both_sides_recorded`; `verify` **201** — *"Cost posted for 1 item(s); 0 left provisional"*. Read back read-only on production: `qty 5, unit_cost 118.75, cost_provenance manual, cost_state final`, **1** `inventory_transactions` row and **1** `inventory_lot_revaluations` row — the quantity never moved at verification and the prior cost was kept. **`d1aea2bc…`:** 6 bottles booked, no agreed price reaching the item, and the lot reads `unit_cost NULL, cost_provenance estimated, cost_state provisional`, 0 revaluations — **absent, not zero, and not final**. A6 also holds live: a second counted line naming no shelf came back `notBooked` with its reason and moved nothing. The pre-fix production state was 165 of 165 lots `final`; after the backfill it is 162 provisional / 3 final, exactly the row counts the PR measured. |

| 2026-09-06 | Fable — V6 fix (#336) read back live (main `fed2b7ca` on :4010, sim tenant) | **The activity door now sees the booking.** `GET /inventory/:rid/item/a23a4595…/activity` → `totalIn28d: 28` (was 0 before #336), `totalOut28d: 1`, and an `includes` sentence naming what the read covers. Log: session scratchpad `lens-vendor/v6-live.json`. |
| 2026-09-11 | Fable — A1/A12: an invoice line can now name its shelf (ADR 0104 slice 4) | **A12's named place exists, and A1 closes end to end on a multi-item delivery.** A12 said a line books stock only with a caller-supplied `inventory_id`, that description matching was REJECTED as a booking rule, and that *"slice 4's mapping memory is where a remembered pairing may later fill the id, still through a person's tick"*. That is now built: `POST /procurement/documents/:id/lines/:lineId/link-item` writes the shelf onto the line, and the per-vendor memory (`document_line_mappings`, migration `20260907120000`) proposes it next time as a tick. **Nothing was relaxed:** no description match books anything, no confidence reaches a person, and a proposal writes nothing — the proposal is only an offer, and the LINK is what `finalise_delivery_cost` reads. The `v3.0-TECH-DEBT` entry *"an invoice line carries no `inventory_id`"* (filed at the A5 stop) is CLOSED with that evidence. Two gaps filed in its place, both deliberate: a document with no `provider_id` teaches the memory nothing, and there is no reconciler replaying already-linked lines into the memory if a memory write fails after the line write succeeds (the response says `remembered: false` with the reason, so the failure is never silent). |
