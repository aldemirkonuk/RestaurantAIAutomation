...
- **Status:** Proposed 2026-09-19. Built on `fix/mime-header-encoding`; not locked until the founder says so. [2026-09-25: one part is locked — the addendum's Gmail failure split (HTTP 5xx/429 ambiguous, HTTP 403/404 a definite refusal), by the founder's answer "Keep the split". The rest of this record is still Proposed.]
- **Date:** 2026-09-19
...
- **What a refusal does.** It throws inside `sendEmail`'s `try`, so the caller gets `{ success: false, refusedBeforeSend: true }` and Gmail is never called. [corrected 2026-09-25: the boolean `refusedBeforeSend` was replaced by the typed `EmailResult.refusal` in the addendum below (PR #405); a header refusal now returns `{ success: false, refusal: { kind: "header" } }` (`gmail.service.ts`, `classifySendFailure` in `send-failure.ts`).]
  - `procurement.service.ts` `sendProviderEmail` turns `refusedBeforeSend` [corrected 2026-09-25: now `result.refusal`, any kind — header, no-transport, credentials or rejected — per the addendum below] into a `SendRefusedBeforeSendError`, and `isDefiniteSendRefusal` recognises it by `instanceof` [corrected 2026-09-25: by `instanceof` alone — the text regexes that still followed it were removed by the addendum below] — **never by text**, because the error text embeds the vendor's contact address, and an address containing "Refusing to write the To header:" plus an ambiguous Gmail failure would otherwise be classed definite, re-approved, and sent twice. So `approveDraft` releases a refused draft instead of parking it as `SEND_UNCONFIRMED` ("may or may not have reached the vendor", which would be false). The error names the header problem and no longer tells anyone to re-auth Gmail for it.
...
| 2026-09-25 | the founder | "Keep the split" on the addendum's Gmail classification: HTTP 5xx/429 ambiguous (held as `SEND_UNCONFIRMED`), HTTP 403/404 definite (draft reopens). That part locked; the body's two stale `refusedBeforeSend` sentences and the addendum's "previous one, restated" corrected in place (PR #405) |

## Addendum 2026-09-19 — a send failure is classified by typed fields, never by text

**Defect.** The header refusal was moved to a typed `SendRefusedBeforeSendError` in PR #402, but `isDefiniteSendRefusal` still matched regexes (`invalid_grant`, `does not exist`, `user unknown`, `/\b5\d{2}[ -]/`, …) against `error.message`. `sendProviderEmail` builds that message as `Email could not be delivered to ${params.to}: …`, and `params.to` is the vendor's free-text `contact_email` (`providers.dto.ts:103-105`). A contact `"Suite 500 - Orders" <v@x.example>` plus an ambiguous failure (socket hang-up after Gmail accepted the message) matched, the draft was released to `PENDING_APPROVAL`, and a re-approve sent a duplicate purchase order. Reproduced first: 43 specs red on `3dbb435db`.

**Options.** (a) Strip the address out of the message before matching: rejected — it keeps a text classifier whose inputs we do not fully control (Gmail's own response text can carry addresses too). (b) Keep the regexes, anchor them: rejected for the same reason. (c) Classify from the error object's typed fields, at the layer that holds the error. Taken.

**Decision.** `send-failure.ts` `classifySendFailure(error)` reads only `response.data.error` (OAuth code, exact-match set), nodemailer `code` (`EAUTH`, `EENVELOPE`) and `responseCode`, and the HTTP status (`response.status`, `status`, or a numeric-string `code`). It never reads `message`, `stack` or any free text. `GmailService.sendEmail` returns the result as `EmailResult.refusal` (`header`, `no-transport`, `credentials`, `rejected`), replacing the boolean `refusedBeforeSend`. `sendProviderEmail` throws `SendRefusedBeforeSendError` on it, and `isDefiniteSendRefusal` is `instanceof` alone. The allow-list keeps the old one's categories, re-expressed in fields, and changes it in two places set out in the next paragraph (Gmail HTTP 5xx/429 held ambiguous; Gmail HTTP 403/404 added as definite): OAuth credential errors, EAUTH, HTTP 401; EENVELOPE, SMTP 5xx, Gmail 400/403/404; no transport; header refusal. [corrected 2026-09-25: this sentence first read "the previous one, restated in fields", which the next paragraph contradicts — the old text rule had no 403/404 case, and its `/\b5\d{2}[ -]/` could match a Gmail 5xx whose text carried the digits.]

**Choice — locked by the founder 2026-09-25.** Gmail **HTTP 5xx and 429 stay ambiguous** (park as `SEND_UNCONFIRMED`). A 5xx/504 from a gateway can follow an accepted message, and the old text rule almost never matched them anyway (gaxios messages carry no status digits). Only SMTP 5xx *replies* (nodemailer `responseCode`) are definite. The two additions over the old behaviour are HTTP 403 and 404 from Gmail (documented not-processed responses). [locked 2026-09-25: this paragraph was written 2026-09-19 as a choice flagged to the founder, not locked, ending "say so if either should stay ambiguous". Asked on 2026-09-25, he answered "Keep the split": Gmail 5xx/429 are ambiguous — `approveDraft` parks the draft as `SEND_UNCONFIRMED` and nothing in the gateway releases it — and Gmail 403/404 are a definite refusal, so the draft is released to `PENDING_APPROVAL` for another tap. The auto-send sweep's revert-on-any-failure is the separate gap under Not covered below.]

**Evidence.** `send-failure.spec.ts` (definite and ambiguous field matrix; each of the 18 old phrases planted in `message`, `command` and `response.data.error.message` changes nothing). `approve-draft-concurrency.spec.ts`: 18 hostile contact addresses × two ambiguous Gmail failures (hang-up, HTTP 503) all park as `SEND_UNCONFIRMED`; three genuine refusals (OAuth `invalid_grant`, 401, 400) release with the same hostile address and a plain one; SMTP fallback 550/`EAUTH` release, 451/`ETIMEDOUT` park; no-transport releases through the real `GmailService`. CLAIMS row `ADR-0172-SEND-REFUSAL-BY-FIELDS`. **Mutation** (snapshot → mutate → run `send-failure`, `approve-draft-concurrency`, `mime-headers` specs and the claims check → restore → `cmp`; all restores identical): every mutant went red in the specs, and after two pins were added for M8/M9 the claims row went red on all 11.

| Mutant | Spec result |
|---|---|
| M1 old text regex back in `isDefiniteSendRefusal` | 9 red |
| M2 classifier reads `message` | 5 red |
| M3 Gmail 5xx counted definite | 21 red |
| M4 SMTP 4xx counted definite | 2 red |
| M5 `sendProviderEmail` ignores `refusal` | 10 red |
| M6 Gmail catch drops `refusal` | 8 red |
| M7 no-transport marker removed | 1 red |
| M8 `EAUTH` not credentials | 1 red |
| M9 OAuth `invalid_grant` not recognised | 1 red |
| M10 SMTP fallback drops `refusal` | 1 red |
| M11 header kind lost | 3 red |

**Not covered.** `processScheduledAutoSends` reverts to a draft on any failure (`v3.0-TECH-DEBT.md`, "The scheduled auto-send sweep…"). Nodemailer error fields were taken from its documented `code`/`responseCode`; they were tested against constructed error objects, not a live SMTP server. [added 2026-09-25: the orchestrator's twin, `ProviderConversationAgent._is_definite_send_refusal` (`services/agent-orchestrator/agents/provider_conversation_agent.py:2625`), still classifies by regexes over `str(error)` — the same text rule this addendum removed from the gateway, with the same vendor-controlled-text exposure. Owed, not fixed on this branch: another lane owned that file in the 2026-09-25 wave, and the fix needs the gateway's send response to carry `refusal` so Python can read a field instead of text.]
