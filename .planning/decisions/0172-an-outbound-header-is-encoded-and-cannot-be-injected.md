...
**Not covered.** `processScheduledAutoSends` reverts to a draft on any failure (`v3.0-TECH-DEBT.md`, "The scheduled auto-send sweep…"). Nodemailer error fields were taken from its documented `code`/`responseCode`; they were tested against constructed error objects, not a live SMTP server.
