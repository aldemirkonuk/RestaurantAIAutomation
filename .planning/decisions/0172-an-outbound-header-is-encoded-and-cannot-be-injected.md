...
| 2026-09-21 | Claude (agent), relay lane merging `origin/main` | The relay person door (ADR 0149 #19) reuses `sendThroughGrant`, which now also writes Cc, Bcc, Reply-To, In-Reply-To and References and takes a list for To. All five go through this module; the lane's own CR/LF stripper (`sanitizeHeaderValue`) and body encoder (`base64Lines`) were retired for `mime-headers.ts`. So a line break inside a person-door address is now refused before any fetch instead of folded into a space (the DTO's `@IsEmail` refuses it at the door first). The CLAIMS row now pins 17 header calls, not 12: five added, and the To pin moved to `addressListHeader("To", to)` with the line that turns a string into a one-entry list pinned beside it. Those seven pins were each mutation-proven red on a scratch copy; `house-letters.spec.ts` pins the refusal for To, Cc, Bcc and Reply-To. `createMimeMessage`'s boundary and `fromName` merged with `origin/main` (#391/#418/#421): the boundary stays `mudavym_alt_${randomBytes(16)...}` (ADR 0149 #19), `fromName` now reaches `mailboxHeader` exactly as main built it. Status unchanged |
