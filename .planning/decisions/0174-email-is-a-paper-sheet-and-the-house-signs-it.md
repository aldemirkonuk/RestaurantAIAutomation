...
- **Status:** Accepted (founder, 2026-09-19). Answered in the comms-templates lane's AskUserQuestion batches 1–4 and by approving mocks 2a–2e. Nothing is built.
- **Date:** 2026-09-19
...
- **The password reset mail.** The mock said 30 minutes and named a city and a device. The code says otherwise (see D8).
...
8. **The password reset mail says what the code does.** The link lasts **1 hour** (`supabase/migrations/20260805115406_password_resets.sql:17`) and works once (`auth.service.ts:2168-2190`, `used_at`). The mail names **no place and no device**: only `requested_ip` is stored (`:19`) and no geo lookup exists. **A reset does not sign other devices out** (`auth.service.ts:2216-2223`, deliberate), so the mail must not imply that it does.
