...
| 26 | Recommendations digest | Build the sender | page note recommendations.md (to carry) |<br>**[Residual, 2026-09-19, page-settings repair pass, not a founder review — flagged, not resolved:** two lanes independently cite this row: `wt-fin-digest` built the real per-person sender (`recommendation_digest_subscriptions`, open PR #391 `train/finish-2`), and `wt-pg-settings` built a per-restaurant Settings-page control (`recommendation_digest_prefs`/`DigestRow.tsx`) that PR #391's own sender never reads. Cross-lane sequencing (which one ships, and when) is undecided; see `.planning/06-pages/settings.md` §13.39 for the evidence.]<br>**[Corrected 2026-09-19, later pass — the residual above does not belong in this table and is answered, kept rather than deleted:** (1) a decision table's row is not where a cross-lane residual note belongs — a page's own open items are (moved, see below); (2) the residual's own claim was wrong besides: the founder settled this directly ("Lane answers batch 2", ~09:30Z, `founder-sketch-decisions-106-115.md:132-133`), "settings digest = correct the dossier to the per-house control that shipped" — it is the shipped, decided design, not one two lanes are racing over. PR #391's sender reads exactly the three columns `DigestRow.tsx` edits (`recommendation-digest.service.ts:607,612`, `readHousePref`) and only its `recipientEmail` goes unmailed (`:112`). That narrower residual, plus the founder's batch-4 follow-up (drop `recipientEmail` and add a per-person opt-in once #391 lands), is tracked in `.planning/06-pages/settings.md` §13.39's open items, not here.] |
