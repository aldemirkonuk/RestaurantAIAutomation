...
**[Deletion-manifest note, 2026-09-26 — the founder, Wave 6 deletion-manifest prep (project memory `founder-answers-2026-09-25-web-rebuild.md`, Round 8 items 51-52, verbatim in [web-rebuild census](../07-reference/deploy/WEB-REBUILD-CENSUS-2026-09-25.md) §16). The manifest draft (`scratchpad/deletion-manifest-draft.md`, this session, not yet in-tree) exists and covers G0-G8. Two rulings on it: (1) **the three capability gaps at G0.3 are to be BUILT, not waived** — vendor branch-locations CRUD (owed to #484), the held low-stock queue (no lane yet, owed to a new W7 PR) and team coverage-template delete + hand-entered sales (owed to #436; the gateway endpoints already exist, `team.controller.ts:293-335`, but no `team/next/` caller reads or writes them — confirmed absent on #436's fetched head, 2026-09-26). None of these gates the manifest's approval by being skipped; the manifest itself already lists them as G0 preconditions (`deletion-manifest-draft.md:43-48`). (2) **G8, the legacy vendor world map, is deleted** — his word takes the manifest's own recommended option (A): delete with `Providers.tsx`, no port, no held-as-reference; the replacement globe is a later, separate build (his words, "a more futuristic globe draw, and tab"; recorded in [FUTURES.md](../FUTURES.md) §9). Rejected here: (B) keep the 8 files / 2,346 lines as dead reference until a map lane ports them; (C) port the map into `/vendors` before cutover. Nothing on `main` changes from this bracket alone — the manifest still needs his approval group by group before the cutover PR opens (G0.4), and this is not that approval.]**
...
| 14 | Consent panel (opened only from legacy `/settings`, no reader) | Delete it and say plainly on `/settings` that no analytics consent is collected yet **[SUPERSEDED 2026-09-21 by the founder's round-6r answer on ADR 0134 fork 14, "Bring back, real switches (Recommended)": a consent panel that opens from the rebuilt Settings and holds only switches the product reads (the ask-training opt-out, Jev scoring, any wired legacy consent), owner-only and audited. The answer lived only in project memory; copied verbatim into [web-rebuild census](../07-reference/deploy/WEB-REBUILD-CENSUS-2026-09-25.md) §8.2 (memory line 282). Not built; census §3 L12. Bracketed 2026-09-25.]** | page note settings.md (to carry) |
...
| 2026-09-26 | Aldemir (founder, Wave 6 deletion-manifest prep, session 6c6d8b93) + records lane W7 (Sonnet 5) | Deletion-manifest note added — the three G0.3 capability gaps (vendor locations, held low-stock, team ops) are to be built, not waived; G8 (the legacy vendor world map) is deleted at cutover, its replacement a later separate build (FUTURES.md) |
