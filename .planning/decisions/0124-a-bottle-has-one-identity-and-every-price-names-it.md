...
- **Status:** **[LOCKED 2026-09-16 — the founder, in session, via [[0149-mudavym-is-the-only-design-finish-every-page-then-delete-legacy-once]] row 13 (the option he chose, "Lock the built, keep research", which named 0124, 0126 and 0113). Same session, row 17: a decision on a public-register row (no house) gains a new nullable deciding-house column, and the person's name and the undo are shown only inside that house — answered, build pending (not merged) (carried with the fault in ADR 0147). The status as written before the lock is kept below.]**
  ~~Proposed~~ — **BUILT on `feat/mudavym-design-p4`, 2026-09-05, and the
...
**[2026-09-17, ADR 0149 answer 17: on a public-register decision the person is
named, and the undo allowed, only inside the house that took it. Other houses read
the outcome and when. See the review trail, 2026-09-17.]**
...
|---|---|---|
| 2026-09-17 | Claude (build, addendum: the deciding house) | **ADR 0149 answer 17, the founder 2026-09-16: *"A new nullable deciding-house column; the person's name and undo only inside that house."*** This supersedes §Q2's read for public-register rows only, closing faults 1 and 2 of `p4-scratch/endpoint-faults/vendor-intel-identity.md` (option C). Any house may still decide a shared candidate. Migration `20260917010000_a_shared_decision_names_its_deciding_house.sql` adds `beverage_identity_decisions.deciding_restaurant_id`: nullable, FK `restaurants` ON DELETE RESTRICT, with CHECK `bid_house_row_is_decided_by_its_house` so a house row can only name its own house. It is not backfilled: the log is append-only, and a shared row's house was never recorded. `IdentityService` writes the column from the token's house on decide and on undo. The log read strips `decided_by`, `decided_by_label`, `decided_by_role` and `note` outside the deciding house. It adds `decided_in` (`this_house` / `another_house` / `unrecorded`), `person_shown` and `undo_refusal`, and never returns the other house's id. An undo from another house's OWN row is a 404, identical to a missing id (a 403 there would confirm the id exists); a SHARED row decided elsewhere stays a readable 403. **Rows with a NULL house, decided before the column existed:** a house row still reads as its own house, as before. A shared row hides its person from every house and refuses undo to every house. That is NOT because no platform-operator role exists: a person-naming allowlist, `PLATFORM_ADMIN_USER_IDS` (`ProspectsController.assertPlatformAdmin`, fail-closed, one route today), already exists and was deliberately not extended here — letting an operator undo a decision no house owns is a new permission and the founder's call. **Surfaced, not filed:** adding this as a register row was tried and reverted in this same fix pass — `OPEN-DECISIONS.md` is newest-first and inserting one row repointed 178 register citations across 89 unrelated files (`check_citation_pairing.py`'s own `--fix`, `scripts/check_citation_pairing.py:69`), a blast radius this lane's own scope does not license. The fork is instead named in the lane's fix report for the founder to route. The web log shows "Another house" or "House not recorded" and prints the refusal where the undo control would be. **Measured:** `jest src/vendor-intel` **407 passed / 20 suites**, of which 15 are new in `identity-deciding-house.spec.ts` plus 8 new or rewritten in `identity-decisions.spec.ts` (52 tests total across the two files). The same two spec files, re-measured against origin/main's service by swapping it in and restoring it (no revert on a shared worktree), fail **28 of 52** — the test files have grown since review round 2 counted 22 of 46 against the same service. Gateway `tsc` on both configs and web `tsc`: **0 errors**. `eslint --quiet` on the service and both specs: clean. `vitest` on the page and the mapping: **16 passed**. The PGlite probe applies the chain and the migration twice over a populated log: **25 of 25 pass**, including CHECK 23514, FK 23503, append-only P0001, RESTRICT 23001, a same-named CHECK with a different body, and a foreign CHECK sorting before/after ours. `scripts/check_decision_claims.sh`: 340 checked, 340 holding. **Not measured:** how many shared decisions production holds (production was not queried), and no browser check. **[Correction, 2026-09-17, review round 2: the packaging defect that review round 2 raised — a broken git index where identity-decisions.spec.ts was never staged and other files were staged against an older tree — is fixed; all 12 paths are staged together and every check above is re-run against that staged index, not the working tree alone.]** **The claim-first `decide` (review round 2, defect 1):** the candidate is claimed with one conditional update (`status` from `pending`, only where it still reads `pending`) BEFORE the link or the log is written; a second decide on the same candidate at the same moment gets a 409 with nothing written, never a silent double-decision. A claim whose later write fails is released back to `pending`. **What claim-first does not cover, stated honestly rather than overclaimed (defect 4):** the claim, link and log are three separate writes, not one transaction — a process killed between the claim and the log (a redeploy SIGTERM, an OOM) strands a candidate reading decided with no logged decision, visible only as a 409 on the next `decide` attempt naming that state; there is no queue entry and no sweep for it. This cannot happen in production yet because no application code writes a candidate row (CLAIMS `ADR-0124-NO-CANDIDATE-WRITER`; that row goes red the day a writer lands). **Deploy-window risk, both directions, stated (defect 6):** the migration is expected to apply before the gateway deploys, but Railway deploys the gateway on push independently of the migration apply (`deploy.yml:4-5`). If the new gateway serves BEFORE the column exists, `GET identity/decisions` and `undo` fail loudly (the column does not exist) for every house, and a confirm's `appendDecision` fails after `writeLink`, releasing the claim but leaving the link written with no decision behind it. If a shared decision is instead taken by the PREVIOUS gateway after the migration has applied, it is written with a NULL house and reads as "House not recorded" to every house thereafter, undoable by none. Neither window was held open on purpose; both are named rather than silently accepted. |
| 2026-09-16 | Aldemir (founder), in session | **Locked** via ADR 0149 row 13; the text above is unchanged. Row 17 of the same record answers vendor-intel faults 1 and 2 (ADR 0147, "Named and not fixed"): a new nullable deciding-house column (for decisions on rows with no house), the person's name and the undo only inside that house. Answered, not built; nothing re-measured by this entry. |
