...
| 2026-09-12 | Claude (port onto main) | **Landed on the founder's word.** In session on 2026-09-12 the founder said "Land both now" of this branch (`feat/connect-text-sender`, 6eac389b) and the calendar push; the P0 + P1 build was ported onto `main` at beb00db4 with no decision in this ADR changed. One hardening added in the port: the inbound webhook refuses (`sender_ambiguous`, nothing stored) a `phone_number_id` that live credentials in more than one house carry, because the table's only unique index is per `sender_id`. |
