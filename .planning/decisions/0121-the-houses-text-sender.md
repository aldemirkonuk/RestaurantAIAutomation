...
- **Status:** **Accepted 2026-09-05 in FIVE parts the founder decided; the rest stays Proposed.**
  **P0 and P1 are BUILT** (2026-09-06 on `feat/connect-text-sender`; ported onto
  ...
  (`.planning/handoff/PROGRESS.md:233` **[corrected 2026-09-19, PR #391 audit: was
  `:139`, stale]** **[re-pointed 2026-09-25: now `PROGRESS.md:415`, in the "Founder decisions taken in session today" table; sections added above it keep moving the line]**); reconciled and gap-fixed 2026-09-17,
...
9. **Added 2026-09-17, from the reconciliation pass's two adversarial judges —
   ...
   - **The reply seal.** Should a WhatsApp reply dispatch immediately with no
     ...
     before."*
     **[ANSWERED 2026-09-19, founder (memory `founder-sketch-decisions-106-115.md`,
     "WhatsApp reply seal", ~11:35Z), his words: *"as long as they click on the send
     button nothing to worry, they should be able to delete their messages as well,
     think about it as -> house built framed but all whatsapp features just like how
     you open on web"*. Clicking Send is the consent — no hold-to-approve seal, no
     recall window, dispatch immediate, as built. This **AMENDS locked ADR 0112 F10**
     for WhatsApp messages a person sends by clicking Send (dated bracket and a
     review-trail row added there 2026-09-19); every other item on F10's list is
     unchanged. **Two new requirements, named as follow-ups, not built here:**
     (1) a person can delete their own sent messages; (2) the surface should read
     like WhatsApp Web, inside the house frame, not today's reply-only composer.
     **Feasibility checked against Meta's public WhatsApp Cloud API docs before
     promising (1):** the Cloud API has no delete/recall/unsend endpoint for a
     message the business sent. The only related capability is the reverse —
     a `revoke` webhook (`developers.facebook.com/documentation/business-messaging/
     whatsapp/webhooks/reference/messages/revoke`) that notifies the business when
     the OTHER party (the WhatsApp end-user) deletes a message THEY sent; Meta's own
     Messages API reference documents no business-side delete action, and an open
     Meta developer-community thread asking for exactly this has no official answer
     (checked 2026-09-19). **So "delete for everyone" on a WhatsApp message the
     house sent cannot be built as advertised** — the platform gives no lever for
     it. What is buildable: a "delete for me" that hides the mirror row in
     Mudavym's own UI (`procurement_conversations`) without touching the message on
     the recipient's device, honestly labelled as local-only. Filed as a follow-up,
     not built here; the WhatsApp-Web-like surface is a separate, larger follow-up
     (its own composer/thread view), also not built here.]**
   ...
   - **Stopping a sender.** "Stop it" on Connections is one click, a fixed
     ...
     **[ANSWERED AND BUILT 2026-09-17 (F3), founder: "Stopping a house's text sender
     takes hold-to-approve and a TYPED reason, like the other revokes on
     `/connections`." Built the same day in `ConnectionsNext.tsx`'s `TextSenderRow` —
     see this ADR's 2026-09-17 (F3) review-trail row below for the full change, the
     mutation-tested claim `ADR-0121-F3-STOP-CEREMONY`, and the test count.]**
   ...
   - **The migration's fail-loud design.** Migration `20260913190100`
     ...
     **[CORRECTED 2026-09-19, PR #391 audit M1: measured 2026-09-18, founder-authorized,
     read-only (Supabase MCP SELECT only). Both predicates are 0 in production — no
     restaurant holds two live `meta_cloud` credentials sharing a `sender_ref`, and none
     holds two inbound WhatsApp receipts sharing `(restaurant_id, message_id)`. The merge
