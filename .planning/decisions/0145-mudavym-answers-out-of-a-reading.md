...
1. **The blocker -- a role refusal is saved.** `20260922220000:19`'s
   ...
   `20260922220300_a_role_refusal_is_saved_on_its_folio.sql` re-adds the
...
3. **Capture on the folio, written once** (`20260922220400`): `asked_as_role`
...
3. **Asks train only on his terms** (answer 3). Migration
   `20260922220500_ask_training_opt_out_and_reask_labels.sql`:
...
- **[Last call, 2026-09-21, FIXED 2026-09-22, round 6y] Opting back in
  ...
  "Never (Recommended)". Built:** migration 20260922220600 adds
...
1. **The opt-out gap closes** (answer 1). Migration
   `20260922220600_ask_training_opt_out_snapshot_at_ask_time.sql` adds
   ...
   - Pre-existing rows are backfilled once, guarded by the trigger's own
     ...
     Production has no such row: `ask_reading_folios` (`20260922220000`) is
   ...
   - `ask_folio_training_export` is replaced (drop + create, same columns as
     `20260922220500`) with three added clauses. The first, `and not
...
export row re-pointed at `20260922220600` (its checks hold unchanged on the
...
1. *A question asked while opted out still reached the export, through
   ...
   "never used for training". Fixed in `20260922220600`'s view: a re-ask
...
that `20260922220600` is the LAST file defining the view and the
