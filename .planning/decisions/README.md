...
| [0028](0028-planning-documents-have-one-home.md) | **Planning documents have one home, and CI keeps them there** — retire-to-write had been satisfied 469 times by *copying*: 89.8% of `.planning/archive/` was a byte-identical twin of a live file, and `.planning/phases/` held zero current-milestone phases. Eleven pairs had already drifted, so the obvious "delete the live twins" sweep would have destroyed phase 30's UAT-discharge note and a `status:` backfill. Swept per file (477 removed, 8 promoted, 8 finally archived, 17 double-archived dropped); guard verified 477/477 against the pre-sweep tree, exit 2 when it cannot look. **0027 skipped** — held by `fix/od-95-push-subscription-source` | 2026-08-26 |
