...
| [0025](0025-citations-must-disagree-loudly.md) | **A citation must carry two anchors that can be diffed — an id *and* a line — and CI fails when they disagree.** Measured: of 74 `OPEN-DECISIONS.md:N` citations, 23 name an OD id and **0 of 23 agree**. Line anchors are 100% wrong but detectable; ids renumber and still resolve, which is what hid OD-83. Also adopts "a claim that cannot run is a FAILURE" (`check_decision_claims.sh:149` reads any non-zero exit from an `open` claim as passing; cost of the fix measured at 1 of 68). **§6 landed 2026-08-26** as `scripts/check_decision_citations.py`, blocking in CI with `--strict-pairing`; the sweep it forced corrected **75 of 78** anchors, and the "0 of 23" headline turned out to understate it — 43 anchors carried no id at all and **26 of those pointed into the register's preamble, not at any row** (§10). §5 (strict claims mode) still open. **Proposed — not locked** | 2026-08-26 |
