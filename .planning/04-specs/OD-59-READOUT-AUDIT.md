...
This directly supports the OD-59 framing in `OPEN-DECISIONS.md:96` — *"`context.task_type`
partitions the column, so one task type is immediately useful."* That claim is **verified
true**: introducing a verdict for `invoice_extraction` alone would surface as its own row.
...
`OD-59 (.planning/decisions/OPEN-DECISIONS.md:96)` says *"`scripts/nf_readout.py` **refuses**
below 30 agent events"*, and `.planning/04-specs/P1-NF-A-INSTRUMENTATION.md:134-135` says
it *"**refuses** to report below 30 events."* **Both are wrong.** It reports, under a
...
| 3 | `OD-59 (OPEN-DECISIONS.md:96)` and `P1-NF-A-INSTRUMENTATION.md:134-135` both claim the readout **"refuses"** below 30 events. It does not — it prints the full table and exits 0. Only `--require-volume` changes the exit code, and the numbers still print. | `nf_readout.py:305-339` vs. the two docs | **Medium** — register rot; matches the standing `decision-register-rots` warning |
