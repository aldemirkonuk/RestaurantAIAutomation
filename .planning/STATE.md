...
**[2026-09-26T04:25Z: train 1 has also merged #450 (`0d4498d1d`) and #425 (`72690130d`) — seven merged of twelve (#463 #416 #412 #446 #451 #450 #425); #394 and #467 remain, #423 #395 #465 are in fix rounds. Neither changes the gateway or a page key: production's gateway still serves `932bd83af` (#451, booted 03:18:30Z) and mudavym.com `assets/index-2txKbWJ0.js`. `Deploy to Production` is still red on every merge commit until #464 (train 2). Wave 3 left sixteen PRs OPEN for train 2, three of them new: #481 (`/vendors`), #482 (vendor-price provenance), #483 (recommendations direction B, live in code on merge). The founder's round 6 (items 37-43) is in the [web-rebuild census](07-reference/deploy/WEB-REBUILD-CENSUS-2026-09-25.md) §14 with the PR each lands in; it resolved OD-125 (the desk stays on `procurement_orders`), OD-153 (fixed 180 days), OD-154 (minimums per wine unless the offer says mixed — a rule change owed on #474) and OD-155 (owner only). Filed: OD-161 (never-counted wines read as out of stock), OD-162 (day-strip future digits at 2.40:1), OD-163 (retention per data class). Open forks from this goal: OD-156 to OD-163.]**
