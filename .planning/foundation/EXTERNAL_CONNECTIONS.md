...
|---|---|---|---|---|
| `POST /toast/webhook` | Toast POS | HMAC-SHA256 `Toast-Signature` (`v1=…`) over `timestamp.body`, constant-time compare — `apps/api-gateway/src/toast/toast.service.ts:106` | Order/stock/menu events → stock depletion | `TOAST_WEBHOOK_SECRET`; `TOAST_MOCK_MODE` defaults **true** (`toast.service.ts:72`) but is closed in production for both signatures (`enforceSignature`, `:121`) and data (`mockDataAllowed`, `:146`) |
...
| **Serper** | `services/agent-orchestrator/services/serper_client.py:74` (`https://google.serper.dev/search`) | `SERPER_API_KEY` | Web verification, critic-score and price lookups. Receives house data: yes — a wine's producer, name and vintage from a house's list. [Bracket, 2026-09-25 (ADR 0224 narrowed `public`, founder item 33): the two house-triggered paths — `jobs/web_verify_tasks.py:312` (after a menu upload) and `jobs/research_tasks.py:1504` (the research agent) — now run only once an owner has accepted terms naming Serper (`services/house_data_terms_gate.py`); the nightly `score.rescore_stale_wines` (`jobs/celery_app.py:120`) runs on the shared library, house-blind, and is not gated.] |
...
| **Google Maps Platform** (Places) | `services/agent-orchestrator/services/google_maps_discovery.py:25` (`places.googleapis.com`, city discovery via `unified_discovery.py`); `services/agent-orchestrator/services/image_collector.py:231,264` (`maps.googleapis.com`, place search + photos); `scripts/backfill_restaurant_coordinates.py:52` (the house's own `google_place_id`; outside the guard's scan roots) | `settings.google_maps_api_key` (discovery) and `getattr(settings, "google_places_api_key")` (image collector) — neither is defined in `config/settings.py`; the backfill script reads its own key from env | Restaurant discovery, training-image collection, map-point backfill. Receives house data: yes — a restaurant's name and city, or the house's place id. No live product caller (collection router unmounted, discovery tasks unscheduled). [Bracket, 2026-09-25: was `public`-excused; now named, ADR 0224.] |
| **Apify** (OpenTable reviews actor) | `services/agent-orchestrator/services/image_collector.py:317,342,346` (`api.apify.com`) | `getattr(settings, "apify_token")` — not defined in `config/settings.py`, so unset | Training-image collection: a restaurant's name and location handed to a third-party OpenTable reader. No live caller (router unmounted). [Added 2026-09-25, ADR 0224.] |
| **Yelp Fusion** | `services/agent-orchestrator/services/image_collector.py:421,436` (`api.yelp.com`) | `getattr(settings, "yelp_api_key")` — not defined, so unset | Training-image collection: a restaurant's name and location; photos stored. No live caller. [Added 2026-09-25, ADR 0224.] |
| **Vivino** | `services/agent-orchestrator/services/image_collector.py:483` (`www.vivino.com/api/explore/explore`, unofficial) | none | Label images for a wine name. No live caller. [Added 2026-09-25, ADR 0224.] |
| **OpenTable** | `services/agent-orchestrator/services/opentable_discovery.py:195,303` (`www.opentable.com`) | none | City discovery (a city slug); results upserted into `restaurant_directory`. No schedule or mounted route (`jobs/tasks.py:622` commented beat). [Added 2026-09-25, ADR 0224.] |
| **Wine-Searcher** | `services/agent-orchestrator/services/wine_research_service.py:191` (Playwright scrape of `/find/<query>`) | none | A wine's producer, name and vintage; result auto-inserted into `master_wine_library`. Reachable only from the stub `research_unknowns_task` (`jobs/tasks.py:593`), unscheduled. [Added 2026-09-25, ADR 0224.] |
| **CellarTracker** | `services/agent-orchestrator/services/wine_research_service.py:234,245` (Playwright scrape) | none | Same query and path as Wine-Searcher. [Added 2026-09-25, ADR 0224.] |
...
| **TypeSafe (Jev)** | Two callers: `scripts/jev/prompt_gate.py` (developer machines — full, unmasked coding-agent prompts, ADR 0182); `apps/api-gateway/src/vendor-tone/jev-tone.client.ts` (the product — masked inbound vendor mail, per house, only after an owner has accepted the house's data terms, ADR 0207) | `JEV_API_KEY` (or `TYPESAFE_API_KEY`) | Prompt classification (Jev gate); vendor-mail tone scoring. Receives personal data: yes, from both callers — the prompt gate unmasked (OD-133 measured an email address and an IBAN arriving byte-identical), the product caller masked. DPA offered, unsigned; retention unagreed — **OD-133**. [Bracket, 2026-09-22: the 2026-08-25 census above (13 outbound services) was not re-measured for this row; TypeSafe is the 14th.] |
| **Expo push** | `apps/api-gateway/src/push/expo-push.service.ts:21` (`https://exp.host/--/api/v2/push/send`) | none (Expo push tokens per device) | Mobile push: each notification's title and body. Receives house data: yes. [Added 2026-09-25, ADR 0224 census.] |
| **Browser push services** | `apps/api-gateway/src/notifications/notifications.service.ts:66` (`web-push`); `services/agent-orchestrator/services/push_notification_service.py:22` (`pywebpush`) | `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` | Web Push to the endpoint in each browser's subscription — Google FCM, Mozilla, Apple or Microsoft. Payload encrypted for the browser. [Added 2026-09-25, ADR 0224.] |
| **Stripe** | `apps/api-gateway/src/billing/stripe.client.ts:90` (`https://api.stripe.com/v1`) | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PUBLISHABLE_KEY` | Billing customer (the house's name and email) and a card on file; charges are unexpressible by the client (ADR 0110). [Added 2026-09-25, ADR 0224.] |
| **Twilio** | `apps/api-gateway/src/communications/text/providers/twilio.adapter.ts:137` | per house, `house_text_sender_credentials` | Text messages, only for a house that connects its own Twilio sender. [Added 2026-09-25, ADR 0224.] |
| **Meta (WhatsApp Cloud)** | `apps/api-gateway/src/communications/text/providers/meta-cloud.adapter.ts:100` (`graph.facebook.com`) | per house, `house_text_sender_credentials` | WhatsApp messages, only for a house that connects its own sender. [Added 2026-09-25, ADR 0224.] |
| **SendGrid** | `services/agent-orchestrator/services/email_client.py:256` | `SENDGRID_API_KEY` | One of the orchestrator's mail paths (address, subject, body, attachments). [Added 2026-09-25, ADR 0224.] |
| **Google Drive / account** | `apps/api-gateway/src/communications/archive/drive-archive.writer.ts:54`; `apps/api-gateway/src/integrations/integrations-oauth.service.ts:423,808,837` | the connected person's Google grant | The mail archive written to the house's Drive; the connected person's profile; Google token exchange (`oauth2.googleapis.com`, `accounts.google.com`). [Added 2026-09-25, ADR 0224.] |
| **Microsoft Graph** | `apps/api-gateway/src/integrations/integrations-oauth.service.ts:837` (`graph.microsoft.com/v1.0/me`) | the connected person's Microsoft grant | The connected person's profile. [Added 2026-09-25, ADR 0224.] |
| **US National Weather Service** | `apps/api-gateway/src/weather/nws.provider.ts:43` (`api.weather.gov`) | none | The house's map point (4 decimals) for its forecast, US houses. [Added 2026-09-25, ADR 0224.] |
| **CloudAMQP** | gateway `common/orchestrator/orchestrator.service.ts:9` (`amqplib`); orchestrator `core/message_bus.py` (`aio_pika`) | `RABBITMQ_URL` | The message bus between our own servers — house events in transit (§4). [Added 2026-09-25, ADR 0224.] |
| **Upstash** | gateway `common/cache/cache.service.ts:8`; orchestrator `core/database.py`, Celery | `REDIS_URL`, `CELERY_BROKER_URL`, `CELERY_BACKEND_URL` | Cache and background jobs — short-lived copies of house data (§4). [Added 2026-09-25, ADR 0224.] |
| **Railway** | hosting (no call site); `vercel.json:8-12` rewrites `/api/*` to `*.up.railway.app` | — | Runs the gateway and the orchestrator: every request and job. [Added 2026-09-25, ADR 0224.] |
| **Vercel** | hosting (no call site); `vercel.json` | — | Serves the web app and relays `/api/*` to Railway. [Added 2026-09-25, ADR 0224.] |

[Bracket, 2026-09-25 (ADR 0224, PR #435): this table and the data terms' `SUBPROCESSORS` (`apps/api-gateway/src/settings/data-terms/house-data-terms.ts`) are now held together by `scripts/check_data_terms_name_every_host.py`, which fails unless every host literal and network SDK in `apps/api-gateway/src` and `services/` is named there or excused with a reason. The 2026-09-25 census: 191 hosts and 74 packages; 30 hosts named in 24 rows; 170 excused (own, local, reference, public, not-a-host). Rows above not re-measured beyond that census. The 2026-08-25 count of 13 outbound services is superseded.]

[Bracket, 2026-09-25 (ADR 0224 amendment, founder item 33: *"if they accept terms and conditions then yes, we can access their menu and so on"*): a name, place or wine taken from — or typed for — a house's list is house data even when each word is public, so the eight look-up hosts the guard had excused as `public` (Google Maps ×2, Apify, Yelp, Vivino, OpenTable, Wine-Searcher, CellarTracker) are now rows above and `SUBPROCESSORS` rows, and the guard refuses a `public` excuse for them (`LOOKUP_DOMAINS`). Re-measured: 191 hosts and 74 packages; 38 hosts named in 31 rows; 162 excused. The licence risk of what these paths store is recorded as accepted in `.planning/v3.0-TECH-DEBT.md`.]
