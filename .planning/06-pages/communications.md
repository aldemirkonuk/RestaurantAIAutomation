...
| The only weekly report that *does* send is unrelated | `@Cron("0 8 * * 1")` `sendWeeklyEmailReport` (`apps/api-gateway/src/communications/scheduled-tasks.service.ts:223`). **No longer single-restaurant** as of 2026-08-26 (OD-87 / [ADR 0022](../decisions/0022-scheduled-jobs-serve-opted-in-tenants.md)): it iterates opted-in tenants, though the legacy `DEFAULT_RESTAURANT_ID` restaurant still takes its recipients from `MANAGER_EMAIL` by design, to freeze its behaviour. It still never reads `scheduled_reports` |
