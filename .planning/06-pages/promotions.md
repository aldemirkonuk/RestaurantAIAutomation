...
> **[amended 2026-09-19: this page describes the LEGACY `/promotions` (three tabs, still live until the ADR 0149 legacy deletion). The rebuilt `/promotions` holds offers only; Trusted senders and Strangers (prospects), with hold-to-trust and add-vendor, moved to `/communications` → "Who is writing" (ADR 0160 §113 Open item 3, founder 2026-09-18). The gateway routes below never moved — they are not promotions-scoped.]** **[corrected 2026-09-21: "still live" above is stale the moment `feat/page-promos` lands. `App.tsx` routes `/promotions` only to `PromotionsNext` (`App.tsx:364`); the legacy `pages/Promotions.tsx` stays in the tree, file untouched, with no import and no route (`App.tsx:114-119`), until the founder approves the ADR 0149 deletion manifest.]** **[corrected 2026-09-25, lane W2-promos (`feat/promotions-mudavym`): the 2026-09-21 bracket above no longer describes the branch that lands. `/promotions` is now `<PageGate page="promotions" legacy={<Promotions />} next={<PromotionsNext />} />`, flag `mudavym_design_promotions`, OFF by default (column `20260926160000`), held back from `LIVE_PAGES`. So this page — the legacy three tabs, senders and prospects included — **stays live for every house** until the founder turns the flag on, and should not be turned on before "Who is writing" (PR #470) is live on `/communications`. The rebuilt page builds everything decided (offer card, offer sheet, the ADR 0165 grade with qualification and recency, house-wide dismissal) and neither of ADR 0160 §113's two owed drawings: cards are one size in rank order and bundles sit in their own fold, pending the founder's pick on sketch 124.]** **[superseded 2026-09-25, lane W3-promos: the founder picked sketch 124 (direction A "The Band", bundles as trays, a bundle ranked by its total, a withheld bundle compact, undated offers kept as "no end date" — ADR 0160 §113 round-5 bracket) and PR #474 builds it: a hero row, three large across, compact tiles five across (three under 1100px, one on a phone), trays in the same bands. Filters are NOT built — a house-first scope bar is under research; its seam is the `scoped` list and the "scope bar" slot in `PromotionsNext.tsx`.]** **[superseded 2026-09-26, lane W4-promos-filters (PR #474): the house-first filters are built (founder items 36 and 43; ADR 0160 §113 round-6 bracket). The page opens on "On my menu" (the house's current menu(s)), with "Everything I stock" and "All offers" one tap wider, each with a live count; facets vendor, ends soon, search, coarse category and running low (counted stock only); a house with no menu read opens on "Everything I stock" under a banner; box sizes are ranked once over every offer and never move with the scope; state lives in the URL; at a phone the facets apply on "Show N offers". The tray shows its first five bottles and "n more". New read: `GET /promotions` now also reads the current menu(s) through `MenusService.readCurrentMenus` and the whole shelf, and returns `house` (menus, drink-line coverage, counted shelf) and a per-offer `scope`.]**
...
**`/promotions`** — Not rebuilt. Two legacy overlays take their shapes now so the rebuild inherits them.
