...
- **"Read a menu" now ends in a question that WRITES** (built 2026-09-06, packet 2 of the overlay layer; census 102 · ADR 0112). `pages/cellar/next/IsThisTheBottlePanel.tsx`, fed by the pure `readings.ts`. Detection was always real (`scanMenuImage` → the orchestrator's four-layer pipeline); the ADD half existed nowhere — the legacy scanner tab and the legacy library page both only moved rows around in component state (`MenuScannerTab.tsx:160-172`, `WineLibrary.tsx:1813-1822`) and the legacy question (`WineValidationModal.tsx:162`) confirmed a reading into nothing.
  - **The write**: `POST /wines/submissions` (`wines.controller.ts:91`). The restaurant and the submitter come from the token; the panel sends neither. A confirmed reading enters the master library's STAGING for the dedup worker.
  - **What "yes" does is said on the panel**: it adds to the LIBRARY. It puts no bottle on a shelf and none in the book — that is the carry sheet, a different act, and claiming otherwise here is exactly what this page has refused for two passes.
  - **The readings are asked about one at a time**, and the tally afterwards counts what happened rather than what was asked: *N added to the library, M turned away*.
  - **The engine's hand stays grey** (sketch 103, 2c). A field is the engine's until a person edits it; an edited field goes ink and the panel names which fields the person changed.
  - **An unscored field says "not scored"** rather than reading as an unsure one. The legacy chip mapped an absent confidence and a low one to the same grey, so "the reader did not score this" and "the reader was unsure" looked alike. The confidence bands are also no longer three colours — this house has one.
  - **A title with no name or no producer is dropped before anybody is asked** (`readings.ts`): the library refuses both, and asking a person to confirm a bottle nobody can save wastes the only scarce thing on a floor. An unrecognised style (`fortified`, `orange` — in the detector's vocabulary, not the library's) is left EMPTY rather than filed as red.
  - Proved by `IsThisTheBottle.test.tsx` (20 assertions).
...
| `cl-bottle-settle` | The reader's question opens | a detected title is asked about — **Is this the bottle?**, the house `Panel` on `settle`. It adds no motion of its own and `prefers-reduced-motion` renders none |
