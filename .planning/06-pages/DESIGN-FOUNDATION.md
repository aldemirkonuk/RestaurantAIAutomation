...
| Delivery | **Decided: HTML sketches in `sketches/` (MANIFEST-registered, 077+) + one published Artifact gallery.** Wave 1 shipped 2026-08-27: boards 077–081 (originally numbered 053–057 — Habitué · Instrument · Cellar · Pass · Ledger) + 082 Blender marks (seal/ember/meter); gallery = Artifact "Mudavym Brand Directions". Founder pick pending; hybrid picks allowed; +5 more directions on request. |
...
### Kept — the material that survives into 083

| From | Element | Kind |
|---|---|---|
| 077 Habitué | The `mü·da·vim` definition block ("the one who is there every day — who the house knows, and who knows the house") | story |
| 077 | The "security camera vs. sits at the bar like its most loyal regular" premise paragraph | story |
| 077 | **"Every vendor. Every bottle. Every shift."** | slogan |
| 077 | **"Less counting, more cooking."** | slogan |
| 077 | The live **row-expand `settle`** demo (grid-rows 0fr→1fr on the house curve, chevron on the same token) — singled out by name | motion |
| 078 Instrument | **Logo C, "The Meter"** — "kinda liked" | mark |
| 079 Cellar | **"Set the table. We'll keep the books."** + its division-of-labour rationale | slogan |
| 079 | **"Your back of house, front of mind."** — flagged sales-material, not in-app | slogan |
| 079 | The **Fraunces house-voice**: variable serif, "a wine label that learned software", used *only* when the product speaks ("Good evening, chef.") | voice |
| 080 Pass | **One-tap / hold-to-approve** | interaction |
| 081 Ledger | The **double-rule** mark and **the full stop** | mark |
| 081 | **"Every bottle, accounted for."** · **"Your müdavim, at the books."** | slogan |
| 081 | §03 **"how the book writes"** voice section | voice |
| 081 | §04 **paper / ink / one-seal** palette structure | palette |
| 081 | **Row-expand "show the working"** | motion |

### Cut

- **Every 077 logo** — "didn't like any logo in habitue." The müdavim *story* survives; its marks do not.
- **All of 078 except the Meter** — the monochrome instrument world read as too cold against the hospitality mandate.
- Everything in 079 and 081 not listed above; 080 contributes only the approve interaction.
...
- **The seal colour** must be "one unique colour that is outside the colouring of themes
  ...
  board 083, stamped as wax on both surfaces; its recommendation is **İznik `#1A5E6B`**
...
| # | Board | Role |
|---|---|---|
| 083 | `mudavym-house` | **Front-runner.** Every kept element assembled into one identity, with three resolved lockups built from the two kept marks (double rule, full stop) and the Meter as alternate — no 077 mark survived, so these are new work. Carries the **ten-candidate seal-colour swatch board** the founder asked for. |
| 084 | `mudavym-anatolian` | New territory: the name's own homeland, İznik geometry as structure. |
| 085 | `mudavym-guestbook` | New territory: recognition and memory as the product. |
| 086 | `mudavym-warmmachine` | New territory: mechanism on show — the technical direction proving it can be the warmest. |
| 082 | `mudavym-3d-marks` | Re-rendered with real effort: six marks, computed framing, subsurface wax, brushed metal, printed ink — plus three generated scenes (higgsfield `z_image`, 0.45 credits) that gave the paper tooth and wax the renderer could not. |
| 087 | `mudavym-motion-canvas` | **One surface dedicated to motion: 145 live replayable demos in 11 families** — entrances & reveals · state & feedback · numbers & data · navigation & structure · product surfaces · procurement & money · inventory & receiving · intelligence & reporting · access, team & setup · system states · signature moments. Skin toggle (branded ↔ neutral) and a speed control make it a comparison tool rather than a showreel. *(Counted 2026-09-12 from the board's own motion ids; this row previously said 62 demos in five families — the figure is now held by a CLAIMS row so it cannot drift again.)* |
...
1. **083 `mudavym-house`** — the kept material assembled into one identity (front-runner), carrying the seal-colour swatch board and properly-resolved marks, since no 077 mark survived.
2. **Three further directions**, founder-chosen territories: **084 Anatolian** (the name's own homeland — İznik geometry as structure, not ornament), **085 The Guest Book** (recognition and memory as the product), **086 Warm Machine** (mechanism on show — the most technical direction proving it can also be the warmest).
3. **082 re-rendered with real effort** — "these are super simple put more effort": subsurface wax, brushed metal, paper fibre, depth of field, six marks instead of three.
...
were forks F8–F11 in [[0112-one-modal-policy-three-shapes-one-primitive|ADR 0112]] — **all four decided by the founder the same evening** (the non-modal class named; detents and a three-deep stack; undo-after with a closed list; all three second-authority ceremonies), with F12, a security-ceremonies catalogue, opened by his addendum — researched the same night (`research/F-security-ceremonies.md`, 24 ceremonies; the lead's adversarial pass `G-security-adversary.md`), six of them drawn, and a fit per act decided by the founder the same evening with three amendments (ceremonies fit a high-paced floor; the two-person rule becomes an authority rule — one approval when the authority is valid, double otherwise; every security change is told to the owners); the deeper pass on the assistant's ceremonies (research H, checked in I) closed F12 the same night — see ADR 0112.
