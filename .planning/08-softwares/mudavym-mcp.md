...
`supabase/migrations/20260912200000_a_house_gives_its_assistant_a_key.sql`); 35 specs.
...
1. **`TenantGuard` fails open.** It returns `true` when `request.user` is unset
   (`common/tenant/tenant.guard.ts:70-75`), and the two global guards are only
