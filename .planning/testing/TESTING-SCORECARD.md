...
| 4 | POS & Sales Ingestion | T1? | `pos-adapters.spec.ts`; `test_pos_abstraction.py` (slug `4-pos`) | CI green unverified. No Nest toast module specs; Wave D (`wave_d_toast_pipeline.py`) retired 2026-09-12 (ADR 0137) — tested a table with zero production consumer. | Phase 39 |
...
| 6 | Communications & Email Intelligence | T1? | `email-e2e.spec.ts`; `ConversationFilterBar.test.tsx`; `test_email_intel_agent.py` (slug `6-comms`) | CI green unverified. Wave E (`wave_e_gmail_pipeline.py`) retired 2026-09-12 (ADR 0137) — its trigger mechanism was never wired to any live event path. | Phase 40 |
| 7 | Calendar & Scheduling | T1? | `calendar.controller.spec.ts`; `calendar.service.spec.ts` (slug `7-calendar`) | CI green unverified. `/calendar` is the only calendar (`/calendar-classic` retired 2026-08-26). Wave G (`wave_g_calendar.py`) retired 2026-09-12 (ADR 0137) — it tested a Google Calendar sync that was investigated and cut. | Phase 40 |
