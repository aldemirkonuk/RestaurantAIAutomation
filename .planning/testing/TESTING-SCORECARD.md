...
| 3 | Inventory Operations | T1? | `inventory.service.spec.ts`; `inventory-ledger.service.spec.ts`; `test_inventory_engine_hardening.py` (slug `3-inventory`) | CI green unverified. Canonical `/inventory` vs `/inventory-legacy` — Phase 43 happy path must prefer command route. | Phase 39 |
...
| 7 | Calendar & Scheduling | T1? | `calendar.controller.spec.ts`; `calendar.service.spec.ts`; `wave_g_calendar.py` (slug `7-calendar`) | CI green unverified. Prefer `/calendar` over `/calendar-classic` for manual pass. | Phase 40 |
