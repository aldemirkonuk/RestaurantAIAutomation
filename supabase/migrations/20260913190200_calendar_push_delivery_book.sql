...
CREATE TABLE IF NOT EXISTS public.calendar_push_targets (
  ...
  provider_calendar_id TEXT CHECK (provider_calendar_id IS NULL OR btrim(provider_calendar_id) <> ''),
  creation_state TEXT NOT NULL DEFAULT 'ready' CHECK (creation_state IN ('creating', 'ready', 'retryable', 'unconfirmed')),
...
CREATE TABLE IF NOT EXISTS public.calendar_push_outcomes (
  ...
  -- other trace at all.
  target_id UUID, -- Immutable audit pointer; deleting a mapping/target must not rewrite the receipt.
  mapping_id UUID,
...
CREATE POLICY calendar_push_mappings_service_role
  ...
  FOR ALL TO service_role USING (true) WITH CHECK (true);
REVOKE ALL ON public.calendar_push_mappings FROM PUBLIC, anon, authenticated;
...
CREATE POLICY calendar_push_outcomes_service_role
  ON public.calendar_push_outcomes
  FOR ALL TO service_role USING (true) WITH CHECK (true);
REVOKE ALL ON public.calendar_push_outcomes FROM PUBLIC, anon, authenticated;
...
-- Effective privileges do not depend on deployment-specific default grants.
GRANT SELECT, INSERT, UPDATE, DELETE ON public.calendar_push_targets, public.calendar_push_mappings TO service_role;
GRANT SELECT, INSERT ON public.calendar_push_outcomes TO service_role;
REVOKE UPDATE, DELETE, TRUNCATE ON public.calendar_push_outcomes FROM service_role;

-- The local delete and its retry intent commit together, even while the
-- connector is disabled or its token cannot be read. This trigger sends nothing.
CREATE OR REPLACE FUNCTION public.calendar_push_remember_local_delete()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
BEGIN
  UPDATE public.calendar_push_mappings SET deleted_locally_at = now(), updated_at = now()
  WHERE restaurant_id = OLD.restaurant_id AND calendar_event_id = OLD.id;
  RETURN OLD;
END $$;
REVOKE ALL ON FUNCTION public.calendar_push_remember_local_delete() FROM PUBLIC, anon, authenticated, service_role;
DROP TRIGGER IF EXISTS calendar_push_remember_local_delete ON public.calendar_events;
CREATE TRIGGER calendar_push_remember_local_delete BEFORE DELETE ON public.calendar_events
FOR EACH ROW EXECUTE FUNCTION public.calendar_push_remember_local_delete();

-- Every source change has a version, including writes outside CalendarService.
CREATE OR REPLACE FUNCTION public.calendar_push_stamp_source_version()
RETURNS trigger LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
BEGIN NEW.updated_at := clock_timestamp(); RETURN NEW; END $$;
REVOKE ALL ON FUNCTION public.calendar_push_stamp_source_version() FROM PUBLIC, anon, authenticated, service_role;
DROP TRIGGER IF EXISTS calendar_push_stamp_source_version ON public.calendar_events;
CREATE TRIGGER calendar_push_stamp_source_version BEFORE UPDATE ON public.calendar_events
FOR EACH ROW EXECUTE FUNCTION public.calendar_push_stamp_source_version();

-- Rule/exception/materialized-child changes also make the parent copy stale.
CREATE OR REPLACE FUNCTION public.calendar_push_touch_series()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE row_value jsonb; rule_id uuid; parent_id uuid;
BEGIN
  row_value := CASE WHEN TG_OP = 'DELETE' THEN to_jsonb(OLD) ELSE to_jsonb(NEW) END;
  IF TG_TABLE_NAME = 'calendar_events' THEN
    parent_id := (row_value->>'parent_event_id')::uuid;
  ELSIF TG_TABLE_NAME = 'calendar_recurrence_rules' THEN
    parent_id := (row_value->>'calendar_event_id')::uuid;
  ELSE
    rule_id := (row_value->>'recurrence_rule_id')::uuid;
    SELECT calendar_event_id INTO parent_id FROM public.calendar_recurrence_rules WHERE id = rule_id;
  END IF;
  IF parent_id IS NOT NULL THEN UPDATE public.calendar_events SET updated_at = clock_timestamp() WHERE id = parent_id; END IF;
  RETURN NULL;
END $$;
REVOKE ALL ON FUNCTION public.calendar_push_touch_series() FROM PUBLIC, anon, authenticated, service_role;
DROP TRIGGER IF EXISTS calendar_push_touch_series ON public.calendar_events;
CREATE TRIGGER calendar_push_touch_series AFTER INSERT OR UPDATE OR DELETE ON public.calendar_events FOR EACH ROW EXECUTE FUNCTION public.calendar_push_touch_series();
DROP TRIGGER IF EXISTS calendar_push_touch_series ON public.calendar_recurrence_rules;
CREATE TRIGGER calendar_push_touch_series AFTER INSERT OR UPDATE OR DELETE ON public.calendar_recurrence_rules FOR EACH ROW EXECUTE FUNCTION public.calendar_push_touch_series();
DROP TRIGGER IF EXISTS calendar_push_touch_series ON public.calendar_recurrence_exceptions;
CREATE TRIGGER calendar_push_touch_series AFTER INSERT OR UPDATE OR DELETE ON public.calendar_recurrence_exceptions FOR EACH ROW EXECUTE FUNCTION public.calendar_push_touch_series();

-- Additive replay also upgrades an earlier unapplied draft shape without
-- retaining the append-only receipt's former cascading pointer FKs.
ALTER TABLE public.calendar_push_targets ALTER COLUMN provider_calendar_id DROP NOT NULL;
ALTER TABLE public.calendar_push_targets ADD COLUMN IF NOT EXISTS creation_state TEXT NOT NULL DEFAULT 'ready';
ALTER TABLE public.calendar_push_targets ADD COLUMN IF NOT EXISTS creation_started_at TIMESTAMPTZ;
ALTER TABLE public.calendar_push_mappings ADD COLUMN IF NOT EXISTS last_source_updated_at TIMESTAMPTZ;
ALTER TABLE public.calendar_push_outcomes DROP CONSTRAINT IF EXISTS calendar_push_outcomes_target_id_fkey;
ALTER TABLE public.calendar_push_outcomes DROP CONSTRAINT IF EXISTS calendar_push_outcomes_mapping_id_fkey;
