...
-- six-field signature. The two never collide, so the exact-match lookup in
...
COMMENT ON COLUMN public.master_wine_library_submissions.signature_hash IS
  'Canonical master-library dedup key. Written ONLY by hashWineSignature() / '
  'wineSignatureHashOrNull() in apps/api-gateway/src/wines/wine-signature.ts, '
  'and mirrored by public.wine_signature_hash() — sha256 over six pipe-joined '
  'normalised fields (producer|name|vintage|country|region|grape_variety), '
  'vintage rendered as NV when absent, NULL when the payload has no usable '
  'name. primary_type and appellation are deliberately excluded: they are '
  'derived classifications, and primary_type occupying a slot is what split '
  'the key space per entry point. Do not populate from any other '
  'implementation; a value in a foreign format cannot be distinguished from a '
  'genuine miss.';
