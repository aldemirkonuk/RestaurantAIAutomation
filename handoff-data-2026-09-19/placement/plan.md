...
## FIXLOG ROUND 3 (R5) -- against round 4's Opus last-call, 5 must_fix items

Run 2026-09-21 in `wt-r5-placement` (branch `r5/placement`), against the version of this plan carried on `origin/wip/2026-09-19/handoff-data`. Full detail lives in `plan.json`'s `off_page_five`, `retirement_record.dated_note_2026_09_21`, and `fixed_2026_09_21_r5` blocks, plus the item-level corrections on `3153` and `canonical-first-render-9-findings`; this is the short version.

1. **Off-page five (must_fix 1).** The 5 checklist.json items that never reached the founder's marking page (44.1b, dashboard-top-wines-wrong-table, dashboard-greeting-wrong-role-clock, procurement-pipeline-dark-lifetime-2-orders, twilio-stripe-unconfigured-plus-8-inboxes) are now documented with their register/finding lines in `plan.json`'s new `off_page_five` block, held on the existing "Off-page 5" founder question (not answered here), with the reason `retire_tech_debt.py`'s own unmarked-items check cannot catch this class stated explicitly: it reads the export (0 unmarked by construction), never `checklist.json`.
2. **Retirement-record dated note (must_fix 2).** `plan.json`'s `retirement_record` gained a `dated_note_2026_09_21` flagging that ADR 0166's Context (lines 38-51) needs a dated bracket correction at execution: the founder's page held 230 items against the checklist's 231, with 4 page-only keys (not just `seasonality-zero-fill`) and 5 checklist-only keys in the other direction.
3. **Citation ledger finished (must_fix 3).** All 143 `GENERIC-DOSSIER-UNVERIFIED` lines read individually (direct file reads at their real, current line numbers -- not the arithmetic the class's own preamble warns against) and given one of the three dispositions the must_fix specified: a mapped item's home, a drop/closed disposition, or the tombstone. 46 new citation entries landed across 17 already-placed items (each tagged with the citing item's OWN home-kind convention: `[REMOVED]` for drop items, `[claims-row -> ...]` for work items, `[adr-text -> ...]` for the 44.15/ADR-0149 items -- not a blanket tag); 2 joined the existing `__44.2_CLASS__` and `orders-wire-item3b` clusters (see `citation_rewrites.md`); 5 route through must_fix 4's gate-owned/live-instruction handling; 4 are held on the off-page-five question; 5 are `NOT-APPLICABLE` (ADR 0166 naming its own retirement target, same as the existing tier for `retire_tech_debt.py`'s own file); 86 resolved to a documented tombstone (a generic register-existence mention, a negative/moot "not in the register" reference, a historical review-trail row, or a drift case whose true content this pass could not relocate -- each with its own one-line reason in `citation_rewrites.md`, not a blanket excuse). Verified 143/143 accounted for exactly once by a Python set-diff against the original list. Three more drift clusters were found and folded into their real homes along the way, the same shape as the pre-existing iCal cluster: 8 UX-catalog citations (bare "391-392"/"391-393" or a loose "44.15" mention) that are really the register's own L151/L173/L187 entries, relocated by its own 44.15 audit to lines 1362-1369; 7 "null on all 442 rows" / "plumbing complete, data absent" citations (miscited as :432-440, :436-441, :543-549 or :544-545) that are the one fact already tracked at `inventory-add-remove-item7` (register:1304-1306); 3 "Phase 33 drift / 401-not-404" citations miscited as :229 that are really inside 44.4 (register:1083-1099).
4. **Gate-owned and live-instruction rewrites (must_fix 4).** Exact before/after text is now in `citation_rewrites.md`'s "Special / cross-cutting rows" section for `decisions/README.md:89` (ADR 0064's `:928` citation -- itself drifted to unrelated content, drop the pointer and keep the historical claim in prose), `:152` (ADR 0139 -- name the fact, drop the file reference), and `:154` (ADR 0141 -- point at the existing `adr0141-null-restaurant-id-deferred` CLAIMS row), matching the CLAUDE.md:124/130 precedent: gate-owned, not edited here, text supplied for whoever executes it. `PAGE-CONTRACT.md:30` (the page-dossier template's own instruction) and `privacy.md:137` (a pending "register the coupling" action item) are rewritten too, so future page dossiers stop being told to cite a file that will not exist.
5. **ADR 0104:694 "nine findings" double-count (must_fix 5) -- settled by reading, not asked.** ADR 0104:694's "Nine findings filed in `v3.0-TECH-DEBT.md`" names register:3239-3301's "the first render against extracted documents: nine findings" section exclusively -- the header quotes the ADR's own phrasing verbatim, and it lists exactly nine numbered, all-CLOSED-via-PR-#304 findings, which is exactly and only `canonical-first-render-9-findings`'s content. Item `3153`'s four methodology bullets (register:3150-3161, a differently-titled, differently-dated subsection from ADR 0104's earlier slice-2 row) have zero topical overlap with the nine findings -- confirmed by a direct side-by-side read of both sections. There is no double-count. The ADR:694 citation moved from `3153` to `canonical-first-render-9-findings`; `3153`'s own entry/verify/why fields carry a dated `[CORRECTED 2026-09-21]` bracket rather than a silent rewrite (CLAUDE §5b); and this concern is removed from `not_fixed_here_by_design`, which now names only the one still-genuinely-open fork (3007.5 / PR #394 sequencing).

**Not touched this round:** no CLAIMS.jsonl verify command's logic changed; the 60 claims-row verifies were not re-run this round (unrelated to citation/ledger work; last confirmed clean in the R4-FIX pass per FIXLOG above). This round is citation-ledger, off-page documentation, and two register-drift corrections only.
