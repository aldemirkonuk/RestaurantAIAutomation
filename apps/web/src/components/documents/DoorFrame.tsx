 ...
 *
 * READ-ONLY IN THIS SLICE. The count entry — the `−  10  +` stepper of sketch
 * 089 Direction C — writes a `receiving_advice`, and that write is slice 5. What
 * ships here is the FRAME: the same canonical lines, in a phone-width column,
 * with every money field suppressed.
 *
...
export function DoorFrame({ doc, countedBy, countedAt }: DoorFrameProps) {
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: '7px 0 0', fontSize: 9.5, color: 'var(--ink-3, #7C7365)' }}>
        {anyCounted
          ? 'Counting here is read-only for now — entering a count writes a door document, which is a later slice.'
