...
function formatDraftAge(createdAt: string): {
  label: string;
  isStale: boolean;
} {
  ...
  if (diffMs < 3_600_000)
    return { label: `${Math.floor(diffMs / 60_000)} min ago`, isStale };
        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
              {isLoading && (
                <p className="text-indigo-300 text-sm text-center py-8">
                  Loading drafts…
                </p>
              )}
            ...
            {conversations.length > 0 && oldestDate && (
              <div className="px-6 py-3 border-t border-indigo-700 text-xs text-indigo-400">
                {conversations.length} active draft
                {conversations.length !== 1 ? "s" : ""} — oldest from{" "}
                {oldestDate}
              </div>
            )}
          </motion.div>
