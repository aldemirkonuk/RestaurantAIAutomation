...
describe('the house policy holds in the primitive family', () => {
  ...
  it('renders no motion at all under prefers-reduced-motion', () => {
    ...
    // #387 (train 1, the overlay foundation) added a third conjunct —
    // `!holdsLevel()` — so a sheet waiting for a stack level schedules no
    // enter motion either; the `reduced` gate this test cares about is still
    // there, just no longer the whole guard. Literal kept exact on purpose
    // (see the file docstring): this asserts what is WRITTEN, not a looser
    // shape of it.
    expect(src('Sheet.tsx')).toContain('if (!live || reduced || !holdsLevel()) return;');
