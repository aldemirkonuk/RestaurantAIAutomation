...
export interface SheetStackApi {
  ...
  /** Rename an admitted level without changing its position. */
...
export function SheetStackProvider({ children, cap = SHEET_STACK_CAP }: SheetStackProviderProps) {
  ...
  const rename = useCallback((id: string, title: string) => {
    const entry = live.current.find((e) => e.id === id);
    if (!entry || entry.title === title) return;
    live.current = live.current.map((e) => (e.id === id ? { ...e, title } : e));
    setEntries(live.current);
  }, []);
  ...
  const value = useMemo<SheetStackApi>(
    ...
    [entries, cap, refusedIds, join, rename, holds, closeTo],
  );
