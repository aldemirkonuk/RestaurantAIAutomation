...
export function RemoveFromInventoryModal({ isOpen, items, onClose, onRemoved }: RemoveFromInventoryModalProps) {
  ...
  const run = async () => {
    ...
    if (failed.length === 0) {
      if (shell.on) {
        if (removed > 0) toast.success(`${removed} inventory row${removed === 1 ? '' : 's'} removed`)
      } else {
        toast.success(
          rows.length === 1
            ...
            <>
              {phase === 'error' && (
                <div className="mdv-alert" role="alert">
                  ...
                  <p>
                    {denied
                      ? 'This account is not permitted to write off stock. Removal did not complete. Stock adjustments may already be recorded; check the book before trying again.'
                      : `${failures.length} of ${rows.length} removal${rows.length !== 1 ? 's' : ''} did not complete. Stock adjustments may already be recorded; check the book before trying again.`}
                  </p>
                  {failures.map((f) => (
              ...
              {rows.length === 0 ? (
                ...
                <>
                  ...
                  <HoldToApprove
                    onApprove={async () => {
                      if (!(await run())) throw new Error('Removal did not complete')
                    }}
