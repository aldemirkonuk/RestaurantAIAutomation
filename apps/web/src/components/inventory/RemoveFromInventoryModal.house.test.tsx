...
describe('flag on — the house panel', () => {
  ...
  it('does not stamp success while the inventory write is pending', async () => {
    let finish!: (value: unknown) => void
    del.mockReturnValueOnce(new Promise((resolve) => { finish = resolve }))
    wrap(<RemoveFromInventoryModal isOpen items={[item({ id: 'w1', name: 'Bottle' })]}
      onClose={() => {}} onRemoved={() => {}} />)
    const seal = screen.getByRole('button', { name: /Hold to write off/ })
    fireEvent.keyDown(seal, { key: 'Enter' })
    fireEvent.keyDown(seal, { key: 'Enter' })
    await waitFor(() => expect(del).toHaveBeenCalledTimes(1))
    expect(screen.queryByText('Written to the book')).toBeNull()
  ...
  it('a refusal says what did not happen, in words the operator can act on', async () => {
    ...
    expect(
      screen.getByText(/Stock adjustments may already be recorded/),
    ).toBeTruthy()
  })
