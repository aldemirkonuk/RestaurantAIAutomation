 ...
 * The cellar lane added `cellar` and its new `/menu` register after that
 * (the founder's 2026-09-19 answer: build the sketch-121 layout first, then
 * go live for every house). `settings`, `recommendations` and `receiving`
 * (the desk, not the door) stay flag-gated.
 */
describe('LIVE_PAGES (ADR 0149 row 36, go-live 2026-09-17)', () => {
  const HELD_BACK = ['settings', 'recommendations', 'receiving'] as const;

  it('is exactly MUDAVYM_PAGES minus the three held-back pages', () => {
  ...
  it('the cellar and its menu register are live (the cellar lane, after row 36)', () => {
    expect(LIVE_PAGES.has('cellar')).toBe(true);
    expect(LIVE_PAGES.has('menu')).toBe(true);
  });

  it('holds back exactly settings, recommendations, receiving', () => {
