/**
 * hp-support — how `/help` reads the support channels and what it puts in a
 * message to them. Pure; tested in `hp-support.test.ts`.
 *
 * THE RULE THIS MODULE EXISTS FOR. The shipping page falls back to
 * `support@wineops.ai` and `https://wineops.slack.com` when the two build
 * variables are unset (`pages/Help.tsx:18-20`) — a domain the project is
 * migrating away from, rendered as visible text a person will copy into their
 * own mail client. There is no fallback here. An unset variable is an
 * UNCONFIGURED channel, and the page says so in a sentence; a value that is not
 * an address is UNUSABLE and the page shows what it read rather than mailing
 * it. Both are read once, at build time — the page says that too, because it
 * means changing them is a redeploy, not a setting.
 */

export type EmailChannel =
  | { state: 'configured'; address: string }
  | { state: 'unconfigured' }
  | { state: 'unusable'; raw: string; why: string };

export type SlackChannel =
  | { state: 'configured'; url: string; host: string }
  | { state: 'unconfigured' }
  | { state: 'unusable'; raw: string; why: string };

export interface SupportChannels {
  email: EmailChannel;
  slack: SlackChannel;
}

export interface SupportEnv {
  VITE_SUPPORT_EMAIL?: string;
  VITE_SUPPORT_SLACK_URL?: string;
}

/** One address, no display name, no comma list — a mailto target, nothing more. */
const EMAIL = /^[^\s@,;<>]+@[^\s@,;<>]+\.[^\s@,;<>]+$/;

export function readEmailChannel(raw: string | undefined): EmailChannel {
  const v = (raw ?? '').trim();
  if (!v) return { state: 'unconfigured' };
  if (!EMAIL.test(v)) return { state: 'unusable', raw: v, why: 'not a single mail address' };
  return { state: 'configured', address: v };
}

export function readSlackChannel(raw: string | undefined): SlackChannel {
  const v = (raw ?? '').trim();
  if (!v) return { state: 'unconfigured' };
  let url: URL;
  try {
    url = new URL(v);
  } catch {
    return { state: 'unusable', raw: v, why: 'not a URL' };
  }
  if (url.protocol !== 'https:') return { state: 'unusable', raw: v, why: 'not https' };
  return { state: 'configured', url: url.toString(), host: url.host };
}

/** Reads both channels from whatever `import.meta.env` the caller hands in. */
export function readSupportChannels(env: SupportEnv): SupportChannels {
  return {
    email: readEmailChannel(env.VITE_SUPPORT_EMAIL),
    slack: readSlackChannel(env.VITE_SUPPORT_SLACK_URL),
  };
}

/* ── What a message to support carries ─────────────────────────────────── */

/**
 * Everything is optional and every absence is written as an absence — the
 * block never invents a house, a build or a route it was not given.
 */
export interface DiagnosticsContext {
  houseName?: string | null;
  houseId?: string | null;
  role?: string | null;
  /** The path the person was on when they opened /help, if the router said. */
  cameFrom?: string | null;
  /** The gateway's own answer, when one was measured. */
  gateway?: {
    state: 'ready' | 'not_ready' | 'unreachable' | 'checking';
    commit?: string | null;
    latencyMs?: number | null;
    detail?: string | null;
  } | null;
  userAgent?: string | null;
  href?: string | null;
  at?: Date;
}

function line(label: string, value: string | null | undefined): string {
  return `${label}: ${value && value.trim() ? value.trim() : 'not recorded'}`;
}

/** The plain-text block a person can paste anywhere; also the mail body. */
export function diagnosticsBlock(ctx: DiagnosticsContext): string {
  const at = ctx.at ?? new Date();
  const g = ctx.gateway;
  const gatewayLine = !g
    ? 'not checked'
    : g.state === 'checking'
      ? 'check still in flight'
      : g.state === 'unreachable'
        ? `unreachable${g.detail ? ` — ${g.detail}` : ''}`
        : `${g.state === 'ready' ? 'ready' : 'up but not ready'}${
            g.detail ? ` — ${g.detail}` : ''
          }${typeof g.latencyMs === 'number' ? `, answered in ${Math.round(g.latencyMs)} ms` : ''}`;
  return [
    'Mudavym — what support will need',
    line('House', ctx.houseName),
    line('House id', ctx.houseId),
    line('Role', ctx.role),
    line('Came from', ctx.cameFrom),
    line('Page', ctx.href),
    `Gateway: ${gatewayLine}`,
    line('Gateway build', g && g.state !== 'unreachable' && g.state !== 'checking' ? g.commit : null),
    line('Browser', ctx.userAgent),
    `Written: ${at.toISOString()}`,
  ].join('\n');
}

/** `mailto:` with the subject and the diagnostics block prefilled. */
export function buildSupportMailto(address: string, ctx: DiagnosticsContext): string {
  const house = ctx.houseName && ctx.houseName.trim() ? ctx.houseName.trim() : 'a house';
  const subject = `Mudavym support — ${house}`;
  const body = `\n\n---\n${diagnosticsBlock(ctx)}\n`;
  return `mailto:${address}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}
