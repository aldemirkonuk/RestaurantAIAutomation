...
|---|---|---|
| `open-arrive` — opening line entrance (ent lineage) | The serif "Good evening / before service" header, once on mount | `settle` easing `cubic-bezier(.16,1,.3,1)` · 320ms · opacity + 6px rise (ADR 0134 rule 1, 2026-09-21: was a raw `{easing: settle.easing, ms: 420}` literal — folded into `settle`, no eighth token) |
| `cal-arrive` — staggered arrival (ent-01 lineage) | Every real day cell of the sales calendar, on each month's first paint (initial load and month navigation) | `settle` — `cubic-bezier(.16,1,.3,1)` · 320ms per cell · clip wipe + 6px rise · delay decays from 16ms × 0.94 per cell (ADR 0134 rule 1, 2026-09-21: was the same raw 420ms literal; last cell now lands ~642ms → ~542ms across 29 gaps) |
