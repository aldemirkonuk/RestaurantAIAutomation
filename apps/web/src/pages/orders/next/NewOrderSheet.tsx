...
export function NewOrderSheet({ open, onClose, onNoVendors, onPlaced }: NewOrderSheetProps) {
  ...
  const place = async () => {
    ...
    const tokenAtStart = localStorage.getItem('accessToken');
    const stillCurrent = () => mounted.current && currentScope.current === scope && placementGeneration.current === generation && localStorage.getItem('accessToken') === tokenAtStart && (!localStorage.getItem('activeRestaurantId') || localStorage.getItem('activeRestaurantId') === restaurantId);
    setPlacing(true);
    setFailure(null);
    setTally(null);
