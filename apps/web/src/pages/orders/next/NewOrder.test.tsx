...
const auth = vi.hoisted(() => ({ activeRestaurantId: 'rest-A', user: { userId: 'actor-A', restaurantId: 'rest-A' } }));
...
beforeEach(() => {
  ...
  auth.user = { userId: 'actor-A', restaurantId: 'rest-A' };
  localStorage.clear();
  api.get.mockReset();
  api.post.mockReset();
...
describe('the shape and the primitive', () => {
  it('is a sheet, named by its contract, closing in words', () => {
    ...
    expect(screen.getByRole('dialog', { name: /Write a new order/ })).toBeInTheDocument();
  });
...
describe('the act — the cart the rebuilt page could not build', () => {
  ...
  it('does not place successful lines again when a refused line is retried', async () => {
    api.post.mockResolvedValueOnce({ data: { orderNumber: 'PO-one' } }).mockRejectedValueOnce(Object.assign(new Error('refused'), { response: { status: 400 } })).mockResolvedValue({ data: { orderNumber: 'PO-two' } });
    draw(); await addLine(ITEM, '2'); await addLine(ITEM2, '3');
    fireEvent.click(screen.getByTestId('new-order-place'));
    await waitFor(() => expect(screen.getByTestId('new-order-tally')).toHaveTextContent(/1 of 2/));
    fireEvent.click(screen.getByTestId('new-order-place'));
    await waitFor(() => expect(api.post).toHaveBeenCalledTimes(3));
    expect(api.post.mock.calls[2][1]).toMatchObject({ inventoryId: ITEM2.id });
  });

  it('stops the remaining cart when the active house changes during a placement', async () => {
    let finish!: (value: unknown) => void;
    api.post.mockImplementationOnce(() => new Promise((resolve) => { finish = resolve; }));
    draw(); await addLine(ITEM, '2'); await addLine(ITEM2, '3');
    fireEvent.click(screen.getByTestId('new-order-place'));
    await waitFor(() => expect(api.post).toHaveBeenCalledTimes(1));
    localStorage.setItem('activeRestaurantId', 'rest-B');
    await act(async () => { finish({ data: { orderNumber: 'PO-old-house' } }); });
    expect(api.post).toHaveBeenCalledTimes(1);
    expect(screen.queryByText('PO-old-house')).not.toBeInTheDocument();
  });

  it('reports a placement whose request refreshed the access token on the way', async () => {
    // What `services/api/client.ts` does on a 401: it writes a new
    // `accessToken` and retries the same POST, which then succeeds. The order
    // is written, so the sheet must say so — not strand itself on "Placing…".
    localStorage.setItem('accessToken', 'token-before-expiry');
    api.post.mockImplementationOnce(async () => {
      localStorage.setItem('accessToken', 'token-after-refresh');
      return { data: { id: 'ord-1', orderNumber: 'PO-after-refresh' } };
    });
    draw(); await addLine(ITEM, '2');
    fireEvent.click(screen.getByTestId('new-order-place'));
    await waitFor(() => expect(screen.getByTestId('new-order-tally')).toHaveTextContent(/All 1 line was placed/));
    expect(screen.getByTestId('new-order-line-outcome')).toHaveTextContent('PO-after-refresh');
    expect(screen.getByTestId('new-order-place')).not.toHaveTextContent('Placing');
    expect(api.post).toHaveBeenCalledTimes(1);
  });

  it('keeps an unconfirmed placement from being blindly retried', async () => {
    api.post.mockRejectedValue(new Error('response lost'));
    draw(); await addLine(ITEM, '2'); fireEvent.click(screen.getByTestId('new-order-place'));
    await waitFor(() => expect(screen.getByTestId('new-order-tally')).toHaveTextContent(/0 of 1/));
    expect(screen.getByTestId('new-order-place')).toBeDisabled();
    expect(screen.getByTestId('new-order-line-outcome')).toHaveTextContent(/Check the order book/);
    // Unconfirmed is not refused: the order may exist, so it never says otherwise.
    expect(screen.getByTestId('new-order-line-outcome')).not.toHaveTextContent(/Nothing was written/);
  });
...
describe('the agreement is offered, never applied', () => {
  it('asks the new route for the pair and shows what it answered', async () => {
    ...
    expect(screen.getByTestId('new-order-unit')).toHaveValue('bottle');
    expect(screen.getByTestId('new-order-price-unit')).toHaveValue('case');
    expect(screen.getByTestId('new-order-price-pack')).toHaveValue('12');
    expect(screen.getByTestId('new-order-currency')).toHaveValue('TRY');
    fireEvent.click(screen.getByTestId('new-order-place'));
    await waitFor(() => expect(api.post).toHaveBeenCalled());
    expect(api.post.mock.calls[0][1]).toMatchObject({ unitType: 'bottle', quotedPrice: 2400, priceUom: 'case', pricePackSize: 12, currency: 'TRY' });
  });

  it('discards an agreement that returns after the vendor changed', async () => {
    let oldReply!: (value: unknown) => void;
    vendorList.value.data = [VENDOR, { id: 'prov-2', name: 'Second vendor' }];
    api.get.mockImplementationOnce(() => new Promise((resolve) => { oldReply = resolve; })).mockResolvedValueOnce({ data: { state: 'none', sentence: 'Second vendor has no agreed price.' } });
    draw(); const line = await addLine();
    fireEvent.change(line.getByTestId('new-order-vendor'), { target: { value: 'prov-2' } });
    await screen.findByText('Second vendor has no agreed price.');
    await act(async () => { oldReply({ data: { state: 'found', sentence: 'Old vendor price', price: 90, priceUom: 'bottle', pricePackSize: 1, currency: 'TRY' } }); });
    expect(screen.queryByText('Old vendor price')).not.toBeInTheDocument();
    expect(screen.getByText('Second vendor has no agreed price.')).toBeInTheDocument();
  });

  it('refuses a negative bottle count for a priced case', async () => {
    draw(); const line = await addLine();
    fireEvent.change(line.getByTestId('new-order-price'), { target: { value: '100' } });
    fireEvent.change(line.getByTestId('new-order-price-unit'), { target: { value: 'case' } });
    fireEvent.change(line.getByTestId('new-order-price-pack'), { target: { value: '-12' } });
    fireEvent.change(line.getByTestId('new-order-currency'), { target: { value: 'TRY' } });
    expect(screen.getByTestId('new-order-place')).toBeDisabled();
  });
...
describe('lineRefusal, on its own', () => {
  const base = {
    ...
    bottlesPerUnit: '',
    price: '', priceUom: '' as const, pricePackSize: '', currency: '',
    notes: '',
    offer: null,
    asking: false,
    outcome: null,
