...
import { getErrorMessage, isUnconfirmedWrite } from '@/services/api/client';
...
export function NewVendorSheet({ open, onClose, onAdded }: NewVendorSheetProps) {
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      resetMatches();
      onAdded();
      // The sheet stays open whenever there is an aside to read — a success
      // aside is still an aside. `said.every(...)` on an EMPTY array is
      // vacuously true, so it used to close on a clean create AND on an
      // all-success one, and the delivery-days / address confirmations above
      // were never seen (lane E audit D10). Only a create with nothing at all
      // to report closes on its own.
      if (said.length === 0) onClose();
    } catch (e) {
      // A 5xx or a dropped response is not the same claim as a 4xx refusal:
      // the gateway may have written the row and lost the reply, so "nothing
      // was written" is a promise this branch cannot back (lane E audit D10).
      // An ordinary refusal (4xx, or no network evidence at all) keeps the
      // confident wording unchanged.
      if (isUnconfirmedWrite(e)) {
        setFailure(
          `The vendor's save could not be confirmed (${getErrorMessage(e)}). Check the book before adding it again — it may already be there.`,
        );
      } else {
        setFailure(
