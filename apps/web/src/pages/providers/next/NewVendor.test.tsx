...
describe('door two — a vendor of your own', () => {
  ...
  it('does not claim nothing was written when the create 500s (lane E audit D10)', async () => {
    create.mutateAsync = vi.fn().mockRejectedValue(
    ...
    await waitFor(() =>
      expect(screen.getByTestId('vendor-failure')).toHaveTextContent(
        /could not be confirmed \(internal error\)/,
      ),
    );
  ...
  it('does not close on an all-success create, so the asides can be read (lane E audit D10)', async () => {
    ...
    expect(screen.getByTestId('vendor-asides')).toHaveTextContent(/main office/);
