...
export function OfferSheet({ offer, today, onClose, onDismiss, onRestore, busy }: OfferSheetProps) {
  ...
  return (
    ...
    >
      ...
      {isBundle && (
        <div className="pn-lineblk">
          <div className="pn-lineblk__w">Bundle total</div>
          {offer.bundle ? (
            <>
              <div className="pn-lineblk__d pn-lineblk__d--beats">{fmtEstimate(offer.bundle.amount, offer.bundle.currency)}</div>
              <p>
                Sum of {offer.bundle.linesCounted} bottle worths below, each measured against the house’s own lowest
                other vendor. Withheld unless every named bottle has a worth of its own — no partial sum.
              </p>
            </>
          ) : (
            <>
