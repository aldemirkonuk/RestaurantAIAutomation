        ...
        batch.rows.some((row) => row.status === 'pending') && (
          <>
            ...
            <HoldToApprove
              ...
              disabled={busy || !canManage}
              onApprove={() => void act(() => arrivalApi.apply(batch))}
            />
          </>
        )}
      {['applying', 'undoing'].includes(batch.status) && (
        <p className="ar-message">
          This batch has an unresolved write. It will not be sent again
          automatically. Read the current settings and ask the operator to
          resolve the receipt.
        </p>
      )}
      {batch.undo_until && (
