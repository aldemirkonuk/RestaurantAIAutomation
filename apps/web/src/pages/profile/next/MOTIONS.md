...
| `pf-pour` | `pour` | `linear` · 620ms | the İznik fill under **Hold to delete this account** (Register VII), under **Hold to put this card on file** (Register V, `StripeCardPanel`, since 2026-09-03; the component moved to `components/mudavym/` on 2026-09-05 and is shared with `/connections` — the token, the curve and the words are unchanged) and, since 2026-09-04, under **Charge this first** and **Remove** on every instrument row (Register V, `SealedControl`) — all inside `HoldToApprove`. Deliberately linear: the operator is timing it against their own thumb. An early release retreats on `tuck` (spring 380/32, ~300ms) and says what did not happen |
