...
export function SpotCountPanel({ item, onClose, onCommitted }: SpotCountPanelProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!qty.trim() || !Number.isInteger(countedQty) || countedQty < 0) {
    ...
    setFailure(null)
    setDenied(false)
    // A completed gesture is not proof of a durable local write or a request
    // leaving this device. The outbox result is the first confirmed rung.
    try {
      ...
      if (shell.on) {
        ...
        setReach(synced ? (receipt?.countId ? 'booked' : 'received') : 'written')
  ...
  const countIsValid = qty.trim() !== '' && Number.isInteger(parsedQty) && parsedQty >= 0
        ...
        <div className="mdv-form">
          ...
          {reach ? (
            ...
            <>
              ...
              <HoldToApprove
                onApprove={async () => {
                  if (!(await handleSubmit())) throw new Error('The count was not confirmed')
                }}
