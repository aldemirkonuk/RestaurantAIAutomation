...
import {
  computeAutoLocatePlan,
  type AutoLocateResult,
  type WineLocationScore,
} from "../../../lib/autoLocateEngine";
...
export function InventoryCommandPage() {
  ...
  const [removeTargets, setRemoveTargets] = useState<InventoryItem[] | null>(
    null,
  );
  const [autoLocateResult, setAutoLocateResult] =
    useState<AutoLocateResult | null>(null);
  const [showAutoLocate, setShowAutoLocate] = useState(false);
  const [includeAssigned, setIncludeAssigned] = useState(false);
  const [showPosMapping, setShowPosMapping] = useState(false);
  ...
  const handleAutoLocate = useCallback(() => {
    setAutoLocateResult(
      computeAutoLocatePlan(inventory, locations, mappings, {
  ...
  const toggleActive = useCallback(
    async (item: InventoryItem) => {
      ...
      try {
        await updateInventoryItem(item.inventoryId || item.id, {
          isActive: next,
        } as any);
        toast.success(
          next ? `${item.name} marked active` : `${item.name} marked inactive`,
        );
        void refetchInventory();
      } catch (e: any) {
        toast.error(
  ...
  /**
   * POS lines that arrived and moved no stock (POS lens defect 1).
   *
   * `isError` is kept and rendered separately from a zero count: a failed read
  ...
  const toVerify = useMemo(
    () =>
      [...(delivered || []), ...(partiallyReceived || [])].filter((o: any) =>
        ["DELIVERED", "PARTIALLY_RECEIVED"].includes(
  ...
  const flagCounts = useMemo(() => {
    const counts: Record<RowFlag, number> = {
      low: 0,
      recon: 0,
      count: 0,
      dead: 0,
    ...
    for (const item of filteredInventory)
      for (const f of rowFlags(item)) counts[f]++;
    return counts;
  }, [filteredInventory]);
  ...
  const allVisibleSelected =
    rows.length > 0 &&
    rows.every((r) => r.inventoryId && selected.has(r.inventoryId));
  ...
  const selectAllVisible = useCallback(() => {
    setSelected((prev) =>
      rows.length > 0 &&
      rows.every((r) => r.inventoryId && prev.has(r.inventoryId))
        ? new Set()
  ...
  const countObservation = (item: InventoryItem): string => {
    ...
    const notes: string[] = [];
    if (flags.includes("dead"))
      notes.push(
        `Dead stock${item.daysSinceSale != null ? ` — no sale ${item.daysSinceSale}d` : ""}, consider clearance/86`,
      );
    if (runway != null && runway <= 3)
      notes.push(`Stockout risk — ${Math.round(runway)}d runway`);
    else if (runway != null && runway <= 5)
      notes.push(`Reorder soon — ${Math.round(runway)}d runway`);
    if (flags.includes("recon"))
      notes.push(`Shadow variance ${shadow} — reconcile after this count`);
    ...
    if (delta != null && delta >= 15)
      notes.push(
        `Priced ${delta.toFixed(0)}% under market — margin opportunity`,
      );
    if (delta != null && delta <= -5)
      notes.push(
        `Cost ${Math.abs(delta).toFixed(0)}% above market — review supplier`,
      );
    if (
      (item.velocityPerDay ?? 0) > 0 &&
      (item.liveStock ?? 0) + shadow > (item.threshold || 0) * 3
    )
      notes.push("Overstocked vs. par — tie up cash");
    return notes.join("; ");
  };
  ...
  const valuationColumns: TableExportColumn<InventoryItem>[] = useMemo(
    () => [
      ...
      {
        header: "Velocity/day",
        value: (i) => (i.velocityPerDay ?? 0).toFixed(2),
      },
      {
        header: "Runway d",
        value: (i) => {
  ...
  const countSheetColumns: TableExportColumn<InventoryItem>[] = useMemo(
    () => [
      ...
      { header: "Type", value: (i) => i.type ?? "" },
      {
        header: "Bottle size",
        value: (i) =>
          i.bottleSizeMl ? formatVolume(i.bottleSizeMl, measurementUnit) : "",
      },
  ...
  const exportAllLocationsValuation = useCallback(
    async (format: TableExportFormat) => {
      try {
        ...
        toast.success(
          `Exported ${merged.length} rows across ${availableRestaurants.length} locations`,
        );
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Export failed");
      }
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (r?.inventoryId)
          setExpandedId((cur) =>
            cur === r.inventoryId ? null : r.inventoryId!,
          );
      } else if (e.key === " ") {
        const r = rows[focusedIndex];
        if (r?.inventoryId) {
          e.preventDefault();
          toggleSelect(r.inventoryId);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    document
      .querySelector(`[data-inv-row="${focusedIndex}"]`)
  ...
  const kpis = useMemo(() => {
    ...
    const menuPotential = filteredInventory.reduce(
      (s, i) =>
        s + (i.menuPrice ?? 0) * ((i.liveStock ?? 0) + (i.shadowStock ?? 0)),
      0,
    );
  ...
  const locName = (item: InventoryItem) => {
    ...
    const breakdown = (item.locations ?? [])
      ...
      .map(
        (l) =>
          `${locations.find((x) => x.id === l.locationId)?.name ?? "Unassigned"}: ${l.qty}`,
      )
      .join("\n");
    return {
      ...loc,
      extra: (item.locations ?? []).filter((l) => l.locationId).length - 1,
      breakdown,
    };
  };
    ...
    <div className="p-6 max-w-[1500px] mx-auto">
        ...
        >
          ...
          <div className="flex bg-gray-100 rounded-lg p-0.5">
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 text-xs font-semibold rounded-md px-3 py-1.5",
                view === "table"
          ...
          <button
            ...
            className="flex items-center gap-1.5 h-9 px-3 border border-gray-200 bg-white rounded-lg text-xs font-semibold text-gray-600 hover:bg-gray-50"
          >
            <MapPin className="w-3.5 h-3.5" /> Locations
          </button>
          <button
            onClick={() => setShowAddWineSelection(true)}
            className="flex items-center gap-1.5 h-9 px-4 bg-wine-600 hover:bg-wine-700 text-white rounded-lg text-xs font-bold shadow-sm"
          >
      ...
      >
        ...
        <Kpi
          ...
          onClick={() => setActiveFlag(activeFlag === "recon" ? null : "recon")}
        />
        <Kpi
          label="Value on hand"
        ...
        {(toVerify as any[]).length > 0 && (
          ...
          >
            ...
            <b className="font-mono text-[11px]">
              {(toVerify as any[]).length}
            </b>
          </button>
        )}
        {/* POS lens defect 1: the only entry point to the mapping surface. A
            failed read says so instead of rendering a reassuring zero. */}
        {(posQueueUnavailable || (posQueue?.summary.open_lines ?? 0) > 0) && (
          <button
            onClick={() => setShowPosMapping(true)}
            className={cn(
              "inline-flex items-center gap-1.5 text-xs font-bold rounded-full px-3.5 py-1.5 shadow-sm",
              posQueueUnavailable
            ...
            title={
              posQueueUnavailable
                ? "The unresolved-line queue could not be read. This is not a claim that it is empty."
                : "POS lines that rang up and removed nothing from stock"
            }
          >
            <Barcode className="w-3.5 h-3.5" />
            {posQueueUnavailable ? (
              <>POS queue unreadable</>
            ) : (
              <>
                POS lines moving no stock{" "}
                <b className="font-mono text-[11px]">
                  {posQueue?.summary.open_lines}
                </b>
              </>
            )}
          </button>
      ...
      <div className="flex items-center gap-2.5 flex-wrap mb-3">
        ...
        <button
          ...
          className={cn(
            "h-9 px-3 rounded-lg text-xs font-semibold border",
            !selectedLocationFilter
        ...
        {(showAllLocations ? locations : locations.slice(0, 5)).map((l) => (
          <button
            ...
            onClick={() =>
              setSelectedLocationFilter(
                selectedLocationFilter === l.id ? null : l.id,
              )
            }
            className={cn(
              "inline-flex items-center gap-1.5 h-9 px-3 rounded-lg text-xs font-semibold border",
              selectedLocationFilter === l.id
                ? "bg-gray-900 border-gray-900 text-white"
                : "bg-white border-gray-200 text-gray-500 hover:border-gray-300",
            )}
          >
            <span
              ...
              style={{ background: l.color || "#be123c" }}
            />
            {l.name}
          </button>
        ))}
        ...
        {typeOptions.map((t) => (
          <button
            ...
            className={cn(
              "h-9 px-3 rounded-lg text-xs font-semibold border capitalize",
              filterType === t
                ? "bg-wine-50 border-wine-600 text-wine-700"
                : "bg-white border-gray-200 text-gray-500 hover:border-gray-300",
            )}
          >
            {t}
        ...
        <ThemedSelect
          ...
          onChange={(v) => {
            ...
            setSortDir(SORT_DEFAULT_DIR[v as SortKey]);
          }}
        />
      </div>
      ...
      {view === "table" && (
        <>
            ...
            <div className="sticky top-2 z-20 flex items-center justify-between gap-3 mb-2.5 px-4 py-2.5 bg-wine-900 text-white rounded-xl shadow-lg">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-white/10 hover:bg-white/20 rounded-lg"
                >
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[1180px]">
                    ...
                    if (sortKey) {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider transition-colors hover:text-gray-800",
                            activeSort ? "text-wine-700" : "text-gray-500",
                ...
                {rows.map((item, idx) => {
                  ...
                  const isSelected =
                    !!item.inventoryId && selected.has(item.inventoryId);
                  const isFocused = idx === focusedIndex;
                  const run = runwayDays(item);
                  const delta = marketDeltaPct(item);
                  const value =
                    (item.wac ?? item.price ?? 0) *
                    ((item.liveStock ?? 0) + (item.shadowStock ?? 0));
                  ...
                  return (
                    <div key={item.inventoryId} data-inv-row={idx}>
                      <div
                        onClick={() =>
                          setExpandedId(isOpen ? null : item.inventoryId!)
                        }
                        onContextMenu={(e) => {
                          e.preventDefault();
                          if (item.inventoryId)
                      ...
                      >
                        ...
                        >
                          <input
                            ...
                            checked={isSelected}
                            onChange={() =>
                              item.inventoryId && toggleSelect(item.inventoryId)
                            }
                            className="w-4 h-4 rounded border-gray-300 text-wine-600 focus:ring-wine-500 cursor-pointer"
                        ...
                        <div>
                          {loc ? (
                            <button
                              ...
                              title={
                                loc.breakdown
                                  ? `${loc.breakdown}\n\nClick to adjust locations`
                                  : "Adjust locations"
                              }
                            >
                              <span
                                className="w-1.5 h-1.5 rounded-full shrink-0"
                                style={{ background: loc.color || "#be123c" }}
                              />
                        ...
                        <div className="font-mono text-xs text-gray-700">
                          {(item.velocityPerDay ?? 0).toFixed(1)}{" "}
                        ...
                        <div className="text-right font-mono text-xs font-semibold text-gray-700">
                          {fmtMoney(value)}
                        </div>
                        <div>
                      ...
                      {isOpen && (
                        <RowExpansion item={item} locations={locations} />
                      )}
                    </div>
                  );
                })}
          ...
          <div className="flex items-center justify-between mt-2.5 text-xs text-gray-400">
            ...
            {flagCounts.dead > 0 && (
              <span>
                Dead stock tip: feature slow movers by the glass this weekend.
              </span>
            )}
          </div>
        </>
      ...
      {/* ── MODALS ── */}
      <PosMappingPanel
        isOpen={showPosMapping}
        onClose={() => setShowPosMapping(false)}
        inventory={inventory as any}
        onChanged={() => {
          void refetchPosQueue();
          void refetchInventory();
        }}
      />
          ...
          return (
            ...
            >
              <MenuItem
                ...
                label={isOpen ? "Collapse row" : "Expand row"}
                onClick={() => {
                  setExpandedId(isOpen ? null : menu.id);
                  setMenu(null);
                }}
              />
              <MenuItem
              ...
              <MenuItem
                icon={Plus}
                label={`Draft PO, ${item.threshold} btl`}
                onClick={() => {
                  navigate(
                    `/orders?draft=new&inventoryId=${menu.id}&qty=${item.threshold}`,
                  );
                  setMenu(null);
                }}
              />
              ...
              <MenuItem
                ...
                onClick={() => {
                  void runInventoryExport(
                    ...
                    [item],
                    valuationColumns,
                    item.name.replace(/[^\w]+/g, "-").toLowerCase() ||
                      "inventory-row",
                    item.name,
                  );
                  setMenu(null);
              ...
              >
                <Trash2 className="w-4 h-4 text-rose-400" /> Remove from
                inventory
              </button>
            </div>
          );
