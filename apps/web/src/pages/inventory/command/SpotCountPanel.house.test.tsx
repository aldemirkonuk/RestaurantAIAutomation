...
describe('flag on — the house sheet', () => {
  ...
  it('rejects a fractional count instead of silently truncating it', () => {
    render(<SpotCountPanel item={ITEM} onClose={() => {}} onCommitted={() => {}} />)
    fireEvent.change(screen.getByRole('spinbutton'), { target: { value: '1.5' } })
    expect(screen.getByRole('button', { name: 'Enter a count first' })).toBeDisabled()
  ...
  it('a queued count does not claim the house has it', async () => {
    ...
    expect(reached).toEqual(['Written here'])
    expect(
