...
export function SommelierAI() {
  ...
  const [model, setModel] = useState<string>(
    () => localStorage.getItem(MODEL_KEY) || "sommelier",
  );
  ...
  const libraryWines = useMemo(
    () => mapApiWinesToUiWines(apiWines),
    [apiWines],
  );

  /**
   * REAL stock, from this restaurant's inventory (intelligence lens, defect 4).
   *
   * The page used to build both the orchestrator's wine context and its own
   * offline fallback from `libraryWines` — the SHARED catalogue, which carries
   * no stock and no par. Every wine went out as `liveStock: null, threshold: 6`,
   * so the fallback's `liveStock !== null` filter could never match a single
   * row and the branch was unreachable code, while the orchestrator was being
   * told a par nobody had set.
   *
   * `isError` is kept: a failed inventory read must not read as an empty
   * cellar, which would have the sommelier answer "nothing is low" (ADR 0067).
   */
  const inventoryQuery = useQuery({
    queryKey: ["inventory", "sommelier-context"],
    queryFn: () => getInventory(),
    staleTime: 60_000,
    retry: false,
  });

  /** Inventory rows with a real stock number and a real par, and nothing else. */
  const stockedWines = useMemo(() => {
    return (inventoryQuery.data ?? [])
      .map((item: any) => ({
        id: item.id,
        name: item.wineName || item.wine_name || "Unnamed wine",
        liveStock:
          typeof item.stockLive === "number"
            ? item.stockLive
            : typeof item.stock_live === "number"
              ? item.stock_live
              : null,
        threshold:
          typeof item.thresholdMin === "number"
            ? item.thresholdMin
            : typeof item.threshold_min === "number"
              ? item.threshold_min
              : null,
        bottleSizeMl: item.bottleSizeMl ?? item.bottle_size_ml ?? null,
        menuPrice: item.menuPriceCurrent ?? item.menu_price_current ?? null,
        menuPriceGlass: item.menuPriceGlass ?? item.menu_price_glass ?? null,
      }))
      .filter((w) => w.liveStock !== null);
  }, [inventoryQuery.data]);
  ...
  const generateResponse = async (userMessage: string): Promise<string> => {
    try {
      const agentOrchestratorUrl =
        import.meta.env.VITE_AGENT_ORCHESTRATOR_URL || "http://localhost:8000";
      const restaurantId = getActiveRestaurantId();

      // Prepare wine context. Stock and par come from INVENTORY — the library
      // has neither, and sending a fabricated par to a model that reasons about
      // reordering is how a made-up number becomes advice.
      const stockById = new Map(
        stockedWines.map((w) => [w.name.toLowerCase(), w]),
      );
      const wineContext = libraryWines.slice(0, 50).map((w) => {
        const stocked = stockById.get(w.name.toLowerCase()) ?? null;
        return {
          id: w.id,
          name: w.name,
          type: w.type,
          // null means "we do not know", and the model is told that rather
          // than being handed a default it cannot distinguish from a reading.
          liveStock: stocked?.liveStock ?? null,
          threshold: stocked?.threshold ?? null,
      ...
      // The orchestrator is down. What follows is NOT an answer to the
      // question asked — it is the one thing this page can compute itself,
      // offered as such, and the reply says so in its first line. A fallback
      // that reads like a sommelier's answer is worse than no fallback.
      ...
      if (asksAboutStock && inventoryQuery.isError) {
        return `**The sommelier is unavailable, and I could not read your inventory either.**

Neither the AI service nor the inventory read answered, so I have nothing to tell you about stock — not "nothing is low". Try again shortly.`;
      }

      if (asksAboutStock) {
        // Real rows, real pars. Wines with no par set are excluded rather than
        // measured against a default (ADR 0129: no par means no verdict).
        const lowStock = stockedWines.filter(
          (w) =>
            w.threshold !== null &&
            w.threshold > 0 &&
            (w.liveStock as number) < w.threshold,
        );
        const noPar = stockedWines.filter(
          (w) => w.threshold === null || w.threshold === 0,
        );
        const fmtName = (w: (typeof stockedWines)[0]) => {
          const vol = w.bottleSizeMl
            ? formatVolume(w.bottleSizeMl, measurementUnit)
            : "";
          return vol ? `${w.name} (${vol})` : w.name;
        };

        if (stockedWines.length === 0) {
          return `**The sommelier is unavailable.**

I can still read your inventory, and it has no wines with a recorded stock level — so there is nothing to check against par.`;
        }

        if (lowStock.length === 0) {
          return `**The sommelier is unavailable.** Here is the one thing I can answer without it:

**No wines are below par.** Checked ${stockedWines.length} wine(s) with a recorded stock level.${
            noPar.length > 0
              ? `\n\n${noPar.length} of them have no par set, so they were not checked — not "they are fine".`
              : ""
          }`;
        }

        const critical = lowStock.filter(
          (w) => (w.liveStock as number) <= (w.threshold as number) * 0.5,
        );
        const low = lowStock.filter(
          (w) => (w.liveStock as number) > (w.threshold as number) * 0.5,
        );

        return `**The sommelier is unavailable.** Here is the one thing I can answer without it — read from your inventory, not from the AI:

## ${lowStock.length} wine(s) below par
${
  critical.length > 0
    ? `\n### Critical — at or under half of par\n${critical
        .map(
          (w) =>
            `- **${fmtName(w)}** — ${w.liveStock} left (par: ${w.threshold})`,
        )
        .join("\n")}`
    : ""
}${
...
Checked ${stockedWines.length} wine(s) with a recorded stock level.${
          noPar.length > 0
            ? ` ${noPar.length} have no par set and were not checked.`
            : ""
        }

I cannot suggest order quantities, suppliers or alternatives while the sommelier is down — those need the AI service.`;
      }

      // Default: say what failed, and do not dress it as an answer.
      return `**The AI sommelier is currently unavailable**, so this question has not been answered — by me or by anything else.

What still works without it:
- Your inventory, on the Inventory page
- Your reports, on the Reports page

Try again shortly; if it keeps failing, that is worth reporting.`;
    }
  };
  ...
  const setMsgFeedback = (id: string, value: "up" | "down") => {
    setFeedback((prev) => ({
      ...prev,
      [id]: prev[id] === value ? (undefined as any) : value,
    }));
  };
      ...
      setMessages((prev) => {
        ...
        if (convId)
          persistConversation(
            updated,
            convId,
            updated.find((m) => m.role === "user")?.content.slice(0, 60) ||
              "New Chat",
          );
        return updated;
      });
                    ...
                    <>
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setConvMenuId(
                            convMenuId === conv.id ? null : conv.id,
                          );
                        }}
                      ...
                      {convMenuId === conv.id && (
                        <div
                          className="absolute right-1 top-9 z-50 w-36 bg-[#2f2f2f] border border-white/10 rounded-lg shadow-xl p-1"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button
                            onClick={() => startRename(conv)}
                            className="flex items-center gap-2 w-full text-left px-2.5 py-1.5 text-sm text-gray-300 hover:bg-white/5 rounded"
                          >
                            <Pencil className="w-3.5 h-3.5" /> Rename
                          </button>
                          <button
                            onClick={() => handleDeleteConv(conv)}
                            className="flex items-center gap-2 w-full text-left px-2.5 py-1.5 text-sm text-rose-400 hover:bg-white/5 rounded"
                          >
              ...
              <div className="max-w-2xl w-full">
                ...
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-400">
                    I'm your AI sommelier assistant for wine pairings,
        ...
        <div className="p-4 border-t border-white/10">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-xs text-gray-600 text-center mt-2">
              Sommelier AI uses your inventory and sales data to provide
