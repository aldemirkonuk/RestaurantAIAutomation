...
export class CommunicationsController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Send low stock alert via email, SMS, and websocket' })
  @ApiResponse({ status: 200, type: MultiChannelResultDto })
  async sendLowStockAlert(@Body() dto: LowStockAlertDto): Promise<MultiChannelResultDto> {
  ...
  @ApiResponse({ status: 200, type: CommunicationResultDto })
  async sendDailySummary(@Body() dto: DailySummaryDto): Promise<CommunicationResultDto> {
  ...
  @ApiOperation({ 
    ...
    description: 'This endpoint simulates a wine reaching its threshold level and triggers an email alert to all MANAGER_EMAIL recipients.'
  })
  @ApiResponse({ status: 200, type: MultiChannelResultDto })
  async testLowStockAlert(): Promise<MultiChannelResultDto> {
    ...
    this.logger.log('='.repeat(60));
    this.logger.log(`Test completed. Email sent to: ${this.managerEmails.join(', ')}`);
  ...
  @Post('test/email')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'TEST: Send a simple test email to all configured recipients' })
  @ApiResponse({ status: 200, type: CommunicationResultDto })
...
<body style="font-family: Arial, sans-serif; padding: 20px;">
  ...
  @ApiOperation({
    ...
    description: 'Send an email using a ready template ("test" or "low-stock") to the given addresses.',
  })
  @ApiResponse({ status: 200, type: CommunicationResultDto })
  async sendTemplateEmail(@Body() dto: SendTemplateTestDto): Promise<CommunicationResultDto> {
    this.logger.log(`Sending ${dto.template} template email to: ${dto.to.join(', ')}`);
...
<body style="font-family: Arial, sans-serif; padding: 20px;">
  ...
  async testMessagingScenario(@Body() body?: {
    ...
    try {
      ...
      const { data: provider } = await this.communicationsService.findProviderByEmail(vendorEmail, restaurantId);
      ...
      const { data: convoData, error: convoError } = await this.communicationsService.storeOutboundConversation({
        restaurantId,
      ...
      try {
        ...
        steps.push({ step: 3, action: 'event_published', routing_key: 'procurement.order.created', success: true });
      } catch (pubErr: any) {
        steps.push({ step: 3, action: 'event_published', success: false, error: pubErr?.message });
      }

      this.logger.log('='.repeat(60));
      this.logger.log('Messaging scenario completed successfully');
      this.logger.log(`Next: Vendor (${vendorEmail}) should reply to the email.`);
      this.logger.log('The EmailParsingAgent will process the reply, thread it, and generate a summary.');
      this.logger.log('='.repeat(60));
  ...
  @ApiOperation({
    ...
    description: 'Overrides a wine stock below threshold, publishes stock.manual_override to RabbitMQ. Expects push + email notification at configured manager email.',
  })
  async e2eStep1TriggerThreshold(@Body() body?: {
    restaurantId?: string;
    wineId?: string;
    wineName?: string;
    newStock?: number;
    ...
    const restaurantId = body?.restaurantId || this.configService.get('DEFAULT_RESTAURANT_ID', '00000000-0000-0000-0000-000000000001');
    const wineName = body?.wineName || 'Opus One 2019';
    ...
    this.logger.log(`Wine: ${wineName}, New Stock: ${newStock}, Threshold: ${threshold}`);
    this.logger.log('═'.repeat(60));
  ...
  @ApiOperation({
    ...
    description: 'Approves a pending procurement order, publishes conversation intent so AI drafts an email to the vendor.',
  })
  async e2eStep2ApproveReorder(@Body() body: {
    orderId: string;
    restaurantId?: string;
  }) {
    const restaurantId = body.restaurantId || this.configService.get('DEFAULT_RESTAURANT_ID', '00000000-0000-0000-0000-000000000001');
    ...
    try {
      await this.orchestratorService.publishEvent(
        ...
        'procurement.conversation_request',
        {
          ...
          target_price: data.negotiated_price_per_bottle || data.target_price_per_bottle || 0,
          max_acceptable_price: (data.negotiated_price_per_bottle || data.target_price_per_bottle || 0) * 1.1,
    ...
    return {
      ...
      nextAction: 'AI will draft a vendor email and send a push notification for approval. Check notifications, then call step3 with the conversationId.',
    };
  }
  ...
  @ApiOperation({
    ...
    description: 'Approves the AI-drafted conversation and sends the email to the vendor (konukald@msu.edu by default).',
  })
  async e2eStep3SendVendorEmail(@Body() body: {
    conversationId: string;
    modifiedMessage?: string;
  }) {
    this.logger.log('═'.repeat(60));
    this.logger.log(`E2E STEP 3: Send Vendor Email (conversation: ${body.conversationId})`);
    this.logger.log('═'.repeat(60));
    ...
    return {
      ...
      nextAction: 'Vendor should receive the email. Reply from konukald@msu.edu, then call step4 to check the inbound.',
    };
  }
  ...
  @ApiOperation({
    ...
    description: 'Lists recent inbound emails from the vendor, shows thread summary and detected intent.',
  })
  async e2eStep4CheckInbound(@Query('orderId') orderId?: string, @Query('providerId') providerId?: string) {
    this.logger.log('═'.repeat(60));
    ...
    let query = this.databaseService.supabase
      ...
      .select('id, order_id, provider_id, direction, channel, message_text, detected_intent, detected_sentiment, conversation_summary, confidence_score, created_at, delivery_status, email_headers, manager_approval_status')
    ...
    return {
      ...
      count: data?.length || 0,
      nextAction: 'If vendor replied, you should see their message above. Call step5 to approve the order confirmation.',
    };
  }
  ...
  @ApiOperation({
    ...
    description: 'Manager confirms the order. AI sends a confirmation + invoice request email to the vendor.',
  })
  async e2eStep5ApproveConfirmation(@Body() body: {
    orderId: string;
    providerId: string;
    restaurantId?: string;
  }) {
    ...
    return {
      ...
      step: 5,
      orderId: body.orderId,
      nextAction: 'AI will draft a confirmation + invoice request email. Approve it via notifications or call step3 again with the new conversationId. When vendor confirms, order auto-transitions to ORDERED.',
    };
  }
  ...
  @ApiOperation({
    ...
    description: 'Returns the current status of the order and its conversation history.',
  })
  async e2eStep6CheckStatus(@Query('orderId') orderId: string) {
    ...
    const { data: order, error: orderErr } = await this.databaseService.supabase
      .from('procurement_orders')
      .select('id, order_number, wine_name, quantity, status, negotiated_price_per_bottle, target_price_per_bottle, approved_at, created_at, updated_at, provider_id')
      .eq('id', orderId)
      .single();
    ...
    const { data: conversations } = await this.databaseService.supabase
      .from('procurement_conversations')
      .select('id, direction, detected_intent, conversation_summary, delivery_status, created_at')
      .eq('order_id', orderId)
    ...
    return {
      ...
      scenarioComplete: order?.status === 'CONFIRMED' || order?.status === 'ORDERED',
    };
  }
  ...
  async handleGmailWebhook(@Body() body: any): Promise<{ status: string }> {
    ...
    if (!this.gmailWatchService.isReady()) {
      this.logger.warn('Gmail Watch not configured — ignoring push notification');
