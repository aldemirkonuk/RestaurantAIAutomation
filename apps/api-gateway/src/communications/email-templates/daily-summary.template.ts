...
export function dailySummaryTemplate(data: DailySummaryData): string {
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin: 20px 0;">
      <tr>
        ${metricBox({
          ...
          backgroundColor: data.metrics.lowStockCount > 0 ? '#fef2f2' : '#ecfdf5',
          textColor: data.metrics.lowStockCount > 0 ? colors.danger : colors.success,
        })}
        <td width="10px"></td>
        ${metricBox({
          value: data.metrics.pendingOrders,
          label: 'Pending Orders',
          backgroundColor: data.metrics.pendingOrders > 0 ? '#fef3c7' : colors.gray[100],
          textColor: data.metrics.pendingOrders > 0 ? colors.warning : colors.gray[700],
        })}
        <td width="10px"></td>
        ${metricBox({
          value: data.metrics.deliveriesToday,
          label: 'Deliveries Today',
          backgroundColor: data.metrics.deliveriesToday > 0 ? '#eff6ff' : colors.gray[100],
          textColor: data.metrics.deliveriesToday > 0 ? colors.info : colors.gray[700],
        })}
      </tr>
    </table>
  `;
  ...
  const alertsHtml = data.alerts.length > 0 ? `
    <div style="margin: 25px 0;">
      ...
      ${data.alerts.map(alert => alertBox({
