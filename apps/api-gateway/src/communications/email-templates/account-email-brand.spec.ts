...
describe('public account email identity', () => {
  it('uses Mudavym throughout reset mail while preserving its recovery link', () => {
    const html = passwordResetEmailTemplate({ name: 'Reader', resetUrl: 'https://mudavym.com/reset-password?token=example' });
  ...
  it('uses Mudavym for Studio invitations, preserving recipient and role details', () => {
    const html = studioInviteEmailTemplate({ roleLabel: 'Reviewer', inviteUrl: 'https://mudavym.com/studio/invite/example', invitedEmail: 'reader@example.test', expiresOn: 'Sep 20, 2026' });
    expect(html).not.toContain('WineOps');
  ...
  it('preserves unrelated message defaults', () => {
    expect(baseTemplate({ title: 'Example', content: '<p>Example</p>' })).toContain('WineOps AI');
  });
});
