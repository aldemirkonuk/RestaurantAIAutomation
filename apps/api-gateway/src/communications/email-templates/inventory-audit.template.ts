...
export function inventoryAuditTemplate(data: InventoryAuditData): string {
  ...
  const metricsHtml = `
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom: 20px;">
      <tr>
        ...
        <td width="15px"></td>
        ${metricBox({
          value: data.daysSinceLastAudit
            ? `${data.daysSinceLastAudit}d`
            : "N/A",
  ...
  const content = `
    ...
    ${alertBox({
      ...
      message:
        "Count all bottles by location. Compare physical counts with system records. Report any discrepancies. Check wine condition and storage temperatures.",
    })}
  `;
