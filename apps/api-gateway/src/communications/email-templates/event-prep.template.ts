...
export function eventPrepTemplate(data: EventPrepData): string {
  ...
  const metricsHtml = `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin: 20px 0;">
      <tr>
        ...
        <td width="15px"></td>
        ${metricBox({
          value: data.totalBottlesNeeded || data.wineRequirements?.reduce((sum, w) => sum + w.quantityNeeded, 0) || 0,
  ...
  const wineTableHtml = data.wineRequirements && data.wineRequirements.length > 0 ? `
    <div style="margin: 25px 0;">
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border: 1px solid ${colors.gray[200]}; border-radius: 8px; overflow: hidden;">
        ...
        <tbody>
          ${data.wineRequirements.map((wine, index) => `
  ...
  const content = `
    ...
    ${hasShortfalls ? alertBox({
      type: 'danger',
      title: 'Stock Shortfall Detected',
      message: 'Some wines needed for this event are not in stock. Place orders immediately to ensure delivery before the event.',
    }) : ''}

    ${data.specialRequests ? alertBox({
      type: 'info',
