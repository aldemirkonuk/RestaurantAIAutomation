...
function deriveApprovalData(conversations: Array<{ direction: string; rollingSummary: string | null }>) {
  ...
  const latestSummary    = [...conversations]
    ...
    ?.rollingSummary ?? '';
  return { hasNegotiation: hasProviderReply, conversationSummary: latestSummary };
}
...
describe('Real Email Conversation Flow (E2E)', () => {
  ...
  afterAll(async () => {
    ...
    if (inboundConvId) {
      await db.supabase.from('procurement_conversations').delete().eq('id', inboundConvId);
    }
    if (outboundConvId) {
      await db.supabase.from('procurement_conversations').delete().eq('id', outboundConvId);
    }
    if (orderId) {
  ...
  it('Step 1 — creates a test procurement order in pending_approval status', async () => {
    ...
    if (!inventoryId) {
      console.warn('[Step 1] No inventory items found for restaurant — skipping DB-dependent steps');
      return;
    }
  ...
  it('Step 2 — inserts an OUTBOUND PENDING_APPROVAL conversation (AI draft)', async () => {
    if (!orderId) { console.warn('[Step 2] No orderId — skipped'); return; }
...
WineOps AI`;
  ...
  it('Step 3 — sends the vendor outbound email (real Gmail send if configured)', async () => {
    if (!outboundConvId) { console.warn('[Step 3] No outboundConvId — skipped'); return; }
...
<p>Please confirm availability and pricing. <em>This is an automated E2E test email — no action required.</em></p>`,
    ...
    if (result.success) {
      console.log(`[Step 3] Email SENT → ${VENDOR_EMAIL} | MessageID: ${result.messageId}`);
      console.log(`         CC: ${MANAGER_EMAIL}`);
      console.log(`         ✉  Reply to this email from ${VENDOR_EMAIL} to trigger inbound flow.`);
    } else {
      console.warn(`[Step 3] Email send skipped/failed (Gmail not configured): ${result.error}`);
    }
  ...
  it('Step 4 — simulates provider inbound reply (mirrors EmailParsingAgent output)', async () => {
    if (!orderId) { console.warn('[Step 4] No orderId — skipped'); return; }
    expect(orderId).toBeTruthy();
...
${VENDOR_EMAIL}`;
  ...
  it('Step 5 — GET conversations returns OUTBOUND + INBOUND with correct direction fields', async () => {
    if (!orderId || !outboundConvId || !inboundConvId) { console.warn('[Step 5] Prerequisites missing — skipped'); return; }
    expect(orderId).toBeTruthy();
    expect(outboundConvId).toBeTruthy();
  ...
  it('Step 6 — deriveApprovalData returns hasNegotiation=true and real rolling summary', async () => {
    if (!orderId) { console.warn('[Step 6] No orderId — skipped'); return; }
  ...
  it('Step 7 — hasNegotiation=false when only OUTBOUND conversations exist', () => {
    const outboundOnly = [
      { direction: 'OUTBOUND', rollingSummary: 'Round 1 draft ready for approval' },
    ];

    const { hasNegotiation, conversationSummary } = deriveApprovalData(outboundOnly);
    ...
    console.log(`[Step 7] Pre-reply: hasNegotiation=${hasNegotiation} → summary section hidden ✓`);
  });
});
