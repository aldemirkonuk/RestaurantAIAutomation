...
<p>Thank you,</p>`,
  ...
  it("should send Conversation Summary email (vendor reply digest)", async () => {
    ...
    expect(result.messageId).toBeTruthy();
    console.log(
      `[3/8] Conversation Summary sent. MessageID: ${result.messageId}`,
    );
  }, 15000);
  ...
  it("should send Order Inquiry email (legacy template)", async () => {
    const html = orderInquiryTemplate({
      ...
      notes:
        "Prefer 750ml bottles. Flexible on delivery date — any time before May 20.",
      bottleSizeMl: 750,
    });
  ...
  it("should send Counter Offer email (legacy template)", async () => {
    const html = counterOfferTemplate({
      ...
      providerName: "Napa Valley Imports",
      reason:
        "We are a regular customer with 3+ orders per quarter. Our budget ceiling is $295/bottle for this SKU.",
      bottleSizeMl: 750,
    });
  ...
  it("should send Order Confirmation email (legacy template)", async () => {
    ...
    expect(result.messageId).toBeTruthy();
    console.log(
      `[6/8] Order Confirmation sent. MessageID: ${result.messageId}`,
    );
  }, 15000);
