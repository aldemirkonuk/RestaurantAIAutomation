...
describe('Email Templates E2E', () => {
  ...
  it('should send Daily Summary email', async () => {
    ...
    const result: EmailResult = await gmailService.sendDailySummaryEmail({
      ...
      alerts: [
        { type: 'low_stock', title: 'Low Stock Warning', message: `${lowStockCount} wines are below threshold` },
        { type: 'delivery', title: 'Delivery Today', message: 'Premium Wine Distributors delivery expected by 2 PM' },
      ],
      actionItems: [
        { priority: 'high', description: 'Review and approve pending order from Vineyard Imports' },
        { priority: 'medium', description: 'Schedule physical inventory count for Friday' },
      ],
    });
  ...
  it('should send Weekly Report email', async () => {
    ...
    const totalBottles = inventory?.reduce((sum, item) => sum + (item.stock_live || 0), 0) ?? 1247;
    const lowStockItems = inventory?.filter(i => (i.stock_live || 0) < (i.threshold_min || 10)) ?? [];
  ...
  it('should send Order Approval email', async () => {
    const result: EmailResult = await gmailService.sendOrderApprovalEmail({
      ...
      urgency: 'high',
      notes: 'Chateau Margaux is critically low. Recommended by AI based on sales velocity.',
    });
  ...
  it('should send Delivery Notification email', async () => {
    const result: EmailResult = await gmailService.sendDeliveryNotificationEmail({
      to: [TEST_EMAIL],
    ...
    expect(result.success).toBe(true);
    expect(result.messageId).toBeTruthy();
    console.log(`[5/11] Delivery Notification sent. MessageID: ${result.messageId}`);
  }, 15000);
  ...
  it('should send Recurring Order Reminder email', async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.messageId).toBeTruthy();
    console.log(`[6/11] Recurring Order Reminder sent. MessageID: ${result.messageId}`);
  }, 15000);
  ...
  it('should send Inventory Audit Reminder email', async () => {
    ...
    const result: EmailResult = await gmailService.sendInventoryAuditReminder({
      ...
      discrepancyCount: 3,
      focusAreas: [
        { area: 'Main Bar Storage', reason: 'High traffic area with frequent discrepancies' },
        { area: 'Wine Cellar - Rack B', reason: 'Chateau Margaux count mismatch reported last week' },
      ],
  ...
  it('should send Event Prep Reminder email', async () => {
    const result: EmailResult = await gmailService.sendEventPrepReminder({
      ...
      wineRequirements: [
        ...
        { name: 'Barolo 2018', quantityNeeded: 4, currentStock: 4, shortfall: 0 },
        { name: 'Sancerre 2021', quantityNeeded: 6, currentStock: 3, shortfall: 3 },
      ],
      totalBottlesNeeded: 24,
      estimatedCost: 4200,
      organizer: 'Chef Marco',
      specialRequests: 'All reds need to be decanted 1 hour before service. Prepare tasting notes cards.',
    });
  ...
  it('should send Custom Reminder email', async () => {
    const result: EmailResult = await gmailService.sendCustomReminder({
      ...
      description: 'Monthly reminder to verify all wine storage units are maintaining proper temperature ranges. Red wines: 55-65°F, White wines: 45-55°F, Sparkling: 40-50°F.',
      ...
      notes: 'Last check found Zone 3 running 2 degrees warm. Verify repair was completed.',
    });
