...
@Injectable()
export class TextConfigService implements OnModuleInit {
  ...
  /**
   * Nest calls this once when `TextSendersModule` initialises, which is what
   * makes the boot warning below actually fire. Until 2026-09-17 `warnOnce`
   * had no caller at all — its promise of a boot-time line was a comment, and
   * the only trace of an unconfigured door was one warning per refused
   * request, which on a deployment Meta never calls is no trace at all.
   */
  onModuleInit(): void {
    this.warnOnce();
  }
