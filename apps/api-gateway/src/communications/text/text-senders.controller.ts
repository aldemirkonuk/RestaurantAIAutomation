...
export class TextSendersController {
  ...
  window(
    @CurrentUser() token: TokenUser,
    // A UUID or a 400. Without the pipe any string reached the query, and the
    // database's own "invalid input syntax for type uuid" came back to the
    // caller inside `says` (measured 2026-09-17 on a non-UUID id).
