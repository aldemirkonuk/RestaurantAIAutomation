...
export class WhatsAppSendService {
  ...
  async reply(params: {
    ...
    if (!currentContact || currentContact.providerId !== params.providerId) {
      return this.refuse(
        currentContact === null ? "read_failed" : "not_in_book",
    ...
    if (mirrorError || !mirrored?.id) {
      ...
      return {
        ...this.refuse(
          "mirror_failed",
          `This message was NOT sent, because it could not first be written to this house's own conversation book (${mirrorError?.message ?? "no receipt returned"}). Nothing was queued. The book holds the record and the provider only carries it, so a message that cannot be recorded is not sent.`,
        ),
        guardrails: hits,
        window,
      };
    ...
    return {
      ...
      refusal:
        outcome.kind === "accepted_by_provider" ? null : "refused_by_provider",
