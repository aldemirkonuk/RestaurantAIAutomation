...
export function verifyMetaHandshake(params: {
  ...
  if (params.mode !== "subscribe") {
    return {
      ok: false,
      reason: "wrong-mode",
      // NOT the caller's `hub.mode`, verbatim, echoed here: this refusal
      // reaches an anonymous caller's response body, and a probe measured
      // 2026-09-17 showed an `<img onerror>` string round-tripping through
      // it. `.type("text/plain")` in the controller stops it rendering as
      // markup, but CodeQL flags the reflection itself, and a fixed sentence
      // needs neither defence.
      says: 'hub.mode was not "subscribe", which is what Meta\'s handshake sends. Nothing was echoed.',
    };
  }
  if (!params.token) {
    return {
