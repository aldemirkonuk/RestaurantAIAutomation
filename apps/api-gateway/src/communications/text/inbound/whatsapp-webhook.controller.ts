 ...
 *
 * Acknowledged only after durable handling. Permanent routing refusals are
 * counted and acknowledged; transient database failures return 503 so Meta can
 * retry. The inbound book has a database uniqueness constraint so concurrent
 * retries cannot record the same message twice.
 *
 * Delivery-status callbacks ride the same POST. Each is written onto the
 * house's own outbound row (`WhatsAppInboundService.applyStatus`), forward
 * only, and a transient failure writing one is a 503 for the same reason.
 *
 * RATE LIMIT: the `webhook` profile, stated on the route. The global
 * `RateLimitGuard` also picks that profile for any path containing
 * `/webhook`, and this path does — but that is a substring rule in another
 * file, and a route Meta bursts against should not depend on the spelling of
 * its own URL to avoid the 100-per-minute default.
 */
...
export class WhatsAppWebhookController {
  ...
  handshake(@Query() query: Record<string, string>, @Res() res: Response) {
    ...
    if (!result.ok) {
      ...
      res.status(403).type("text/plain").send(result.says);
      return;
    }
  ...
  @Post()
  @HttpCode(200)
  @RateLimit(DEFAULT_RATE_LIMITS.webhook)
  async receive(@Req() req: Request, @Body() body: unknown) {
    ...
    for (const callback of parsed.statuses) {
      statusResults.push(await this.inbound.applyStatus(callback));
    }
    ...
    const statusCounts = statusResults.reduce<Record<string, number>>(
      (acc, r) => {
        acc[r.disposition] = (acc[r.disposition] ?? 0) + 1;
        return acc;
      },
    ...
    this.logger.log(
      `WhatsApp webhook: ${parsed.messages.length} message(s), ${parsed.statusCount} status callback(s), ${parsed.skipped.length} unreadable; messages ${JSON.stringify(counts)}; statuses ${JSON.stringify(statusCounts)}`,
    );
    for (const s of parsed.skipped)
      this.logger.warn(`WhatsApp webhook: ${s.why}`);

    if (
      results.some(
        (r) =>
          r.disposition === "book_unreadable" ||
          r.disposition === "write_failed",
      ) ||
      statusResults.some(
        (r) =>
          r.disposition === "book_unreadable" ||
      ...
      throw new ServiceUnavailableException(
        "Some messages could not be recorded. Retry this webhook.",
      );
    }
    ...
    return {
      ...
      statusCounts: statusCounts as Record<StatusDisposition, number>,
