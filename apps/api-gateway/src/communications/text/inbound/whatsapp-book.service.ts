...
export class WhatsAppBookService {
  ...
  async phoneBook(restaurantId: string): Promise<PhoneBook> {
    ...
    if (error) {
      // The database's own text is LOGGED, never returned: `reason` reaches a
      // client through `GET phone-book`, and a Postgres message names columns,
      // types and constraints that are ours to read and nobody else's.
      this.logger.error(`phoneBook providers read failed: ${error.message}`);
      return {
        entries: [],
        readable: false,
        reason: "the vendor book could not be read",
      };
    }
    ...
    if (ids.length > 0) {
      ...
      if (contactError) {
        this.logger.error(
          `phoneBook provider_contacts read failed: ${contactError.message}`,
        );
        return {
          entries,
          readable: false,
          reason: "the vendor book's contact list could not be read",
        };
      }
   ...
   *
   * FOUR ANSWERS, NEVER FEWER, and the caller must keep them apart:
   *   an entry      exactly one vendor holds the number;
   *   `undefined`   no vendor holds it;
   *   `"ambiguous"` MORE than one vendor holds it — a fact about the book;
   *   `null`        the book could not be read — a fact about us.
   * The caller must not thread on any of the last three.
   */
  async providerForWaId(
    restaurantId: string,
    waId: string,
  ): Promise<PhoneBookEntry | "ambiguous" | null | undefined> {
    ...
    /**
     * One vendor, or a refusal. Counted by VENDOR, not by entry: `phoneBook`
     * keeps one entry per vendor per number, but a vendor may hold the same
     * line twice in different formats ("0532 …" and "+90 532 …"), and both of
     * those matching is still one vendor.
     */
    const single = (
      matches: PhoneBookEntry[],
      how: string,
    ): PhoneBookEntry | "ambiguous" | undefined => {
      if (matches.length === 0) return undefined;
      const vendors = new Set(matches.map((m) => m.providerId));
      if (vendors.size === 1) return matches[0];
      // Ambiguity is refused, not resolved by picking the first. Two vendors
      // holding one number is a fact about the book, and guessing hides it:
      // threading a vendor's reply onto the wrong vendor's conversation is
      // worse than not threading it at all.
      this.logger.warn(
        `providerForWaId: ${vendors.size} vendors in restaurant ${restaurantId} hold one arriving number (${how} match); not threading`,
      );
      return "ambiguous";
    };

    // Exact digits first. This branch used to return the FIRST exact match,
    // so two vendors in one house holding the same number silently resolved to
    // whichever row the read returned first — the ambiguity the suffix branch
    // below already refused, accepted one branch earlier (measured 2026-09-17:
    // two vendors on +905321112233 resolved to the first).
    const exact = single(
      book.entries.filter((e) => e.digits === digits),
      "exact",
    );
    if (exact !== undefined) return exact;

    // Then a suffix match, because a book may hold a national-format number
    // ("0532 …") for a vendor whose wa_id is the full international one
    // ("90532 …"). The suffix must be at least nine digits: shorter than that
    // and two different vendors can collide.
    const MIN_SUFFIX = 9;
    return single(
  ...
  async windowFor(
    ...
    if (error) {
      ...
      // The database's own text goes to the LOG, never into `says`: this
      // sentence is returned by `GET whatsapp/window/:providerId`, and a
      // Postgres message is ours to read, not the caller's.
      this.logger.error(`windowFor read failed: ${error.message}`);
      return {
        state: "unknown",
        lastInboundAt: null,
        says: "Whether this conversation's 24-hour window is open could not be read, so nothing was attempted. That is not the same as the window being closed.",
      };
    }
    ...
    if (Number.isNaN(at) || at > nowMs) {
      return {
        state: "unknown",
        lastInboundAt: stamp,
        says: `This vendor's last WhatsApp reply carries a timestamp this build cannot use ("${stamp}"), so whether the 24-hour window is open is unknown and nothing was attempted.`,
      };
    }
