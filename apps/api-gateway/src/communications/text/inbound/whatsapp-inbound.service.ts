...
export class WhatsAppInboundService {
  ...
  private async senderFor(
    ...
    // TWO rows, not one (port onto main, 2026-09-12). The only unique index on
    // this table is one live credential per `sender_id`
    // (`20260905215000_a_senders_credential_is_a_pointer_not_a_token.sql:146`);
    // nothing stops two HOUSES holding live credentials with the same
    // `sender_ref`. `.limit(1)` would then pick one house by row order and
    // write a vendor's message into it. Reading two lets that case be refused
    // by name, the same rule `providerForWaId` applies to two vendors.
    ...
    const houses = new Set(rows.map((r) => String(r.restaurant_id)));
    if (houses.size > 1) {
      this.logger.warn(
        `senderFor: more than one house holds a live meta_cloud credential for phone number id ${phoneNumberId}; not threading`,
      );
      return "ambiguous";
    }
    return {
  ...
  async thread(message: InboundWhatsAppMessage): Promise<InboundResult> {
    ...
    if (sender === "ambiguous") {
      return {
        ...
        says: `More than one house on this deployment holds a live WhatsApp credential for business phone number id ${message.phoneNumberId}, so which house's book this belongs to cannot be established. Nothing was stored; picking one would put a vendor's message in the wrong house.`,
      };
    }
    if (sender === undefined) {
      return {
