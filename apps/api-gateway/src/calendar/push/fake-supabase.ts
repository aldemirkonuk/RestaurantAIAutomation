...
export function fakeSupabase(
  ...
  function query(name: string) {
    ...
    const selected = () => {
      const rows = table(name).filter((r) => matches(r, filters)).sort((a,b) => { for (const order of orders) { const cmp = String(a[order.column] ?? "").localeCompare(String(b[order.column] ?? "")); if (cmp) return order.ascending ? cmp : -cmp; } return 0; });
      const ranged = rangeEnd === null ? rows : rows.slice(rangeStart, rangeEnd + 1);
      return limit === null ? ranged : ranged.slice(0, limit);
    };

    const runWrite = () => {
      if (mode === "insert") {
        const row = { id: nextId(), ...payload };
        if (conflictsWith(name, row)) return { data: [], error: { code: "23505", message: "unique violation" } };
        table(name).push(row);
    ...
    self.in = (column: string, value: unknown[]) => { filters.push({ kind: "in", column, value }); return self; };
    self.order = (column: string, opts?: { ascending?: boolean }) => { orders.push({ column, ascending: opts?.ascending !== false }); return self; };
    self.range = (start: number, end: number) => { rangeStart = start; rangeEnd = end; return self; };
    self.limit = (n: number) => {
      limit = n;
      return self;
    };
    ...
    self.then = (resolve: (v: unknown) => unknown) => {
      ...
      return Promise.resolve(
        counting
          ? { data: head ? null : rows, count: table(name).filter((r) => matches(r, filters)).length, error: result.error }
          : { data: rows, error: result.error },
      ).then(resolve);
    };
