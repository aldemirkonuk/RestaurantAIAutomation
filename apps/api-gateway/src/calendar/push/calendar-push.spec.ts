...
describe("the idempotency key", () => {
  ...
  it("makes a retried create ONE provider event, not two", async () => {
    ...
    // An existing ID does not prove it carries the latest body. The retry
    // follows the duplicate with an addressed update before claiming delivery.
    expect(google.calls.at(-1)?.method).toBe("PUT");
    expect(outcomes(db).at(-1)?.outcome).toBe("delivered");
...
describe('durable delivery corrections', () => {
  it('claims calendar creation before concurrent first pushes can call Google twice', async () => {
    const { service, google, db } = harness();
    await Promise.all([service.push(HOUSE, ENTRY, 'create'), service.push(HOUSE, ENTRY, 'create')]);
    expect(google.calls.filter((call) => call.path === '/calendars')).toHaveLength(1);
    expect(db.rows('calendar_push_targets')).toHaveLength(1);
  });
  it('does not repeat a calendar creation whose reply was lost', async () => {
    const { service, google, db } = harness();
    google.override_ = {status:0,body:null,reason:null,message:'reply lost',retryAfterSeconds:null};
    expect((await service.push(HOUSE, ENTRY, 'create')).outcome).toBe('failed');
    google.override_ = null;
    expect((await service.push(HOUSE, ENTRY, 'create')).outcome).toBe('failed');
    expect(google.calls.filter((call) => call.path === '/calendars')).toHaveLength(1);
    expect(db.rows('calendar_push_targets')[0].creation_state).toBe('unconfirmed');
  });
  it('keeps a failed update owed even though an older provider copy exists', async () => {
    const { service, google, db } = harness();
    await service.push(HOUSE, ENTRY, 'create');
    Object.assign(db.rows('calendar_events').find((row) => row.id === ENTRY)!, {title:'New delivery',updated_at:'2026-09-13T21:30:00Z'});
    google.override_ = {status:503,body:null,reason:'backendError',message:'unavailable',retryAfterSeconds:null};
    await service.push(HOUSE, ENTRY, 'update');
    expect(await service.entriesOwedACopy(HOUSE,25)).toEqual([ENTRY]);
    expect((await service.status(HOUSE)).pushed).toBe(0);
    google.override_ = null; await service.push(HOUSE, ENTRY, 'create');
    expect(await service.entriesOwedACopy(HOUSE,25)).toEqual([]);
    expect(google.events.get(idempotencyKey(HOUSE,ENTRY,CONNECTION))?.summary).toBe('New delivery');
  });
  it('finds older missing copies beyond a full first page of current entries', async () => {
    const { service, db } = harness(); await service.push(HOUSE,ENTRY,'create');
    const target = db.rows('calendar_push_targets')[0];
    for (let i=0;i<605;i++) {
      const id = `000-${String(i).padStart(4,'0')}`;
      db.seed('calendar_events',{id,restaurant_id:HOUSE,start_date:'2026-09-13',updated_at:'2026-09-13T00:00:00Z'});
      db.seed('calendar_push_mappings',{id:`map-${id}`,target_id:target.id,restaurant_id:HOUSE,calendar_event_id:id,provider_event_id:`provider-${id}`,last_source_updated_at:'2026-09-13T00:00:00Z'});
    }
    db.seed('calendar_events',{id:'zz-old',restaurant_id:HOUSE,start_date:'2020-01-01',updated_at:'2020-01-01T00:00:00Z'});
    expect(await service.entriesOwedACopy(HOUSE,25)).toEqual(['zz-old']);
  });
  it('finds an orphaned copy even when deletion happened while the connector was disabled', async () => {
    const { service, db } = harness(); await service.push(HOUSE,ENTRY,'create');
    const row = db.rows('calendar_events'); row.splice(row.findIndex((entry)=>entry.id===ENTRY),1);
    expect(await service.copiesAwaitingRemoval(HOUSE,25)).toEqual([ENTRY]);
  });
  it('does not count or delete a previous connection copy through the current grant', async () => {
    const { service, google, db } = harness(); await service.push(HOUSE,ENTRY,'create');
    db.rows('integration_oauth_connections')[0].revoked_at = '2026-09-13T00:00:00Z';
    db.seed('integration_oauth_connections',{id:'new-connection',user_id:USER,restaurant_id:HOUSE,integration_id:'google_calendar',revoked_at:null});
    expect((await service.status(HOUSE)).pushed).toBe(0);
    const before = google.calls.length;
    await service.push(HOUSE,ENTRY,'delete');
    expect(google.calls).toHaveLength(before);
    expect(google.events.size).toBe(1);
  });
  it('finishes an uncertain insert deletion by the derived provider ID', async () => {
    const { service, google, db } = harness(); await service.push(HOUSE,ENTRY,'create');
    mappings(db)[0].provider_event_id = null;
    await service.push(HOUSE,ENTRY,'delete');
    expect(google.events.size).toBe(0); expect(mappings(db)).toHaveLength(0);
  });
  it('carries an overnight timed end into the next day', async () => {
    const { service, google, db } = harness();
    Object.assign(db.rows('calendar_events').find((row)=>row.id===ENTRY)!,{all_day:false,start_time:'23:30',end_time:null});
    await service.push(HOUSE,ENTRY,'create');
    expect(google.events.get(idempotencyKey(HOUSE,ENTRY,CONNECTION))?.end).toMatchObject({dateTime:'2026-09-11T00:30:00'});
  });
});
