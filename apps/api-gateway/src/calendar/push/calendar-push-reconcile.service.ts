...
export class CalendarPushReconcileService {
  ...
  private async house(restaurantId: string): Promise<HouseReconcileReport> {
    ...
    if (held === null) { report.sentence = `${restaurantId}: the rate-limit record could not be read, so this house was left untouched.`; return report; }
    if (held > 0) {
    ...
    for (const entryId of removals.slice(0, MAX_PER_HOUSE_PER_SWEEP)) {
    ...
    for (const entryId of owed.slice(0, Math.max(0, MAX_PER_HOUSE_PER_SWEEP - report.attempted))) {
