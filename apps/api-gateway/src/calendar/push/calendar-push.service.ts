...
import type { CalendarOccurrenceRow, CalendarOccurrenceRule, CalendarOccurrenceException } from "../calendar-occurrences";
import { readCalendarPages } from "../calendar-read-pages";
...
export class CalendarPushService {
  ...
  private async attempt(
    ...
    const persisted = await this.persistedBackoffSeconds(restaurantId);
    if (persisted === null) throw new Error("The durable rate-limit record could not be read; no provider request was made.");
    const until = Math.max(this.backoffUntil.get(restaurantId) ?? 0, Date.now() + persisted * 1000);
    if (until > Date.now()) {
    ...
    try {
      ...
      const detail = stopped
        ? message
        : `${message} The token could not be obtained; no calendar request was made.`;
      await this.record({
        restaurantId,
        calendarEventId,
        verb,
  ...
  private async pushUpsert(
    ...
    const { data: entry, error: entryError } = await this.db.client
      ...
      .select(
        "id, restaurant_id, title, description, event_type, start_date, end_date, all_day, start_time, end_time, status, updated_at, is_recurring, recurrence_rule_id, parent_event_id",
      )
    ...
    let body = this.eventBody(entry, target.timeZone, restaurantId);
    if (entry.is_recurring && !entry.parent_event_id) {
      const rules = await readCalendarPages<CalendarOccurrenceRule>(() => this.db.client.from("calendar_recurrence_rules").select("*", { count: "exact" }).eq("restaurant_id", restaurantId).eq("calendar_event_id", calendarEventId));
      if (rules.length !== 1) throw new Error("The series must have one readable repeat rule before it can be copied.");
      const exceptions = await readCalendarPages<CalendarOccurrenceException>(() => this.db.client.from("calendar_recurrence_exceptions").select("*", { count: "exact" }).eq("recurrence_rule_id", rules[0].id));
      const children = await readCalendarPages<CalendarOccurrenceRow>(() => this.db.client.from("calendar_events").select("*", { count: "exact" }).eq("restaurant_id", restaurantId).eq("parent_event_id", calendarEventId));
      // Modified replacements may be standalone rows. Read their exact IDs in
      // the same house; missing replacements refuse the whole series copy.
      const known = new Set(children.map((child) => child.id));
      const ids = exceptions.map((ex) => ex.replacement_event_id).filter((id): id is string => !!id && !known.has(id));
      for (let offset = 0; offset < ids.length; offset += 100) children.push(...await readCalendarPages<CalendarOccurrenceRow>(() => this.db.client.from("calendar_events").select("*", { count: "exact" }).eq("restaurant_id", restaurantId).in("id", ids.slice(offset, offset + 100))));
      const recurrence = googleRecurrence(entry as CalendarOccurrenceRow, rules[0], exceptions, children, target.timeZone);
      const durationDays = (Date.parse(String(entry.end_date ?? entry.start_date)) - Date.parse(String(entry.start_date))) / 86_400_000;
      body = this.eventBody({ ...entry, start_date: recurrence.firstDate, end_date: entry.end_date ? addDays(recurrence.firstDate, durationDays) : null }, target.timeZone, restaurantId);
      body.recurrence = recurrence.lines;
    }
    let restored = false;
    ...
    if (duplicate) {
      answer = await this.google.call(accessToken, { method: "PUT", path: `/calendars/${encodeURIComponent(target.providerCalendarId)}/events/${encodeURIComponent(key)}`, body });
    }

    if (answer.status >= 200 && answer.status < 300) {
      const providerEventId =
        (typeof answer.body?.id === "string" ? answer.body.id : null) ?? key;
      await this.markPushed(mapping.id, providerEventId, verb, typeof entry.updated_at === "string" ? entry.updated_at : null);
  ...
  private async pushDelete(
    ...
    if (!target?.providerCalendarId) return { outcome: "delivered", detail: "This connection has no addressed calendar copy to remove.", providerEventId: null, restored: false };
    const { data: rows, error } = await this.db.client
      .from("calendar_push_mappings")
      .select("id, target_id, provider_event_id, idempotency_key")
      .eq("restaurant_id", restaurantId)
    ...
    const mapping = (rows ?? [])[0] as
      | { id: string; target_id: string; provider_event_id: string | null; idempotency_key: string }
      | undefined;
    ...
    const answer = await this.google.call(accessToken, {
      method: "DELETE",
      path: `/calendars/${encodeURIComponent(target.providerCalendarId)}/events/${encodeURIComponent(mapping.provider_event_id ?? mapping.idempotency_key)}`,
    });
    ...
    if (gone) {
      const closed = await this.db.client
        .from("calendar_push_mappings")
        .delete()
        .eq("id", mapping.id).eq("restaurant_id", restaurantId);
      if (closed.error) throw new Error("Google removed the copy but its local mapping could not be closed.");
      const detail =
  ...
  private async ensureTarget(
    ...
    if (existing?.providerCalendarId) return existing;
    const house = await this.house(restaurantId);
    const summary = `Mudavym — ${house.name}`;
    // Claim BEFORE the non-idempotent calendars.insert request. A lost reply
    // leaves an unconfirmed claim; an automatic retry must not make a duplicate.
    const payload = { restaurant_id: restaurantId, connection_id: connection.id, provider: "google",
      provider_calendar_id: null, provider_calendar_summary: summary, time_zone: house.timeZone,
      created_by: connection.userId, creation_state: "creating", creation_started_at: new Date().toISOString() };
    const claimed = existing?.creationState === "retryable"
      ? await this.db.client.from("calendar_push_targets").update(payload).eq("id", existing.id).eq("restaurant_id", restaurantId).eq("creation_state", "retryable").select("id").maybeSingle()
      : existing ? { data: null, error: null }
      : await this.db.client.from("calendar_push_targets").insert(payload).select("id").single();
    if (!claimed.data || claimed.error) {
      const winner = await this.targetFor(restaurantId, connection.id);
      if (winner?.providerCalendarId) return winner;
      throw new Error("Creating this house's Google calendar is in progress or was not confirmed. Its Google account must be checked before another calendar can be created.");
    }
    const targetId = String(claimed.data.id);
    const answer = await this.google.call(accessToken, { method: "POST", path: "/calendars", body: {
      summary, description: "This restaurant's day-book, written by Mudavym. Edit entries in Mudavym; changes in Google are not read back.", timeZone: house.timeZone ?? undefined,
    } });
    const providerCalendarId = typeof answer.body?.id === "string" && answer.body.id.trim() ? answer.body.id : null;
    if (answer.status < 200 || answer.status >= 300 || !providerCalendarId) {
      // A definitive 4xx refusal permits a later attempt. Network/5xx/empty
      // success may have created the calendar and therefore remain unconfirmed.
      const retryable = answer.status >= 400 && answer.status < 500 && answer.status !== 408;
      const saved = await this.db.client.from("calendar_push_targets").update({ creation_state: retryable ? "retryable" : "unconfirmed" }).eq("id", targetId).eq("restaurant_id", restaurantId);
      if (saved.error) throw new Error("The calendar creation result could not be recorded; its claim remains held.");
      return { failure: await this.recordFailure({ restaurantId, targetId, verb: "ensure_calendar", answer: providerCalendarId ? answer : { ...answer, status: answer.status >= 200 && answer.status < 300 ? 0 : answer.status, message: answer.message ?? "Calendar creation was not confirmed. Check the connected account before retrying." }, fromReconcile }) };
    }
    const saved = await this.db.client.from("calendar_push_targets").update({ provider_calendar_id: providerCalendarId, creation_state: "ready", updated_at: new Date().toISOString() }).eq("id", targetId).eq("restaurant_id", restaurantId).eq("creation_state", "creating").select("id").single();
    if (saved.error || !saved.data) throw new Error("Google created the calendar but its address could not be saved. The held creation claim prevents a duplicate; check the connected account.");
    await this.record({ restaurantId, targetId, verb: "ensure_calendar", outcome: "delivered", detail: `A secondary calendar "${summary}" was created in the connected Google account (${providerCalendarId}).`, providerStatus: answer.status, fromReconcile });
    const stored = await this.targetFor(restaurantId, connection.id);
    if (!stored?.providerCalendarId) throw new Error("The saved secondary calendar could not be read back.");
    return stored;
  }
  ...
  async status(restaurantId: string): Promise<PushStatus> {
    ...
    let book: Awaited<ReturnType<CalendarPushService["deliveryBook"]>>;
    try { book = await this.deliveryBook(restaurantId, connection); }
    catch { base.error = "The current connection's delivery book could not be read completely; its counts are unknown."; base.sentence = base.error; return base; }
    const entries = book.entries.length;
    const pushed = book.entries.filter((entry) => this.hasCurrentCopy(entry, book.mappings.get(String(entry.id)))).length;
    base.entries = entries;
    base.pushed = pushed;
    base.unpushed = entries - pushed;
    base.pendingDeletes = book.removals.length;
    base.lastOutcome = await this.lastOutcome(restaurantId);
    if (target && !target.providerCalendarId && target.creationState !== "retryable") {
      base.sentence = "Creating this house's Google calendar is in progress or was not confirmed. Check the connected account before creating another calendar.";
      return base;
    }
    ...
    base.sentence = pushSentence({
      ...
      hasCalendar: Boolean(target?.providerCalendarId),
      entries,
      pushed,
  ...
  async entriesOwedACopy(
    ...
    try {
      const book = await this.deliveryBook(restaurantId);
      return book.entries.filter((entry) => !this.hasCurrentCopy(entry, book.mappings.get(String(entry.id)))).slice(0, limit).map((entry) => String(entry.id));
    } catch { return null; }
  }

  async copiesAwaitingRemoval(restaurantId: string, limit: number): Promise<string[] | null> {
    try { return (await this.deliveryBook(restaurantId)).removals.slice(0, limit); }
    catch { return null; }
  }

  private hasCurrentCopy(entry: Record<string, unknown>, mapping: Record<string, unknown> | undefined): boolean {
    if (!mapping?.provider_event_id || mapping.deleted_locally_at || typeof entry.updated_at !== "string" || typeof mapping.last_source_updated_at !== "string") return false;
    const sourceTime = Date.parse(entry.updated_at), copiedTime = Date.parse(mapping.last_source_updated_at);
    return Number.isFinite(sourceTime) && sourceTime === copiedTime;
  }

  /** Page before comparing; limiting the newest entries first starves history. */
  private async deliveryBook(restaurantId: string, knownConnection?: HouseConnection) {
    const connection = knownConnection ?? await this.connectionFor(restaurantId);
    const target = connection ? await this.targetFor(restaurantId, connection.id) : null;
    const entries = await readCalendarPages<Record<string, unknown>>(() => this.db.client.from("calendar_events").select("id,updated_at", { count: "exact" }).eq("restaurant_id", restaurantId));
    const rows = target ? await readCalendarPages<Record<string, unknown>>(() => this.db.client.from("calendar_push_mappings").select("calendar_event_id,provider_event_id,deleted_locally_at,last_source_updated_at", { count: "exact" }).eq("restaurant_id", restaurantId).eq("target_id", target.id)) : [];
    const mappings = new Map<string, Record<string, unknown>>();
    for (const row of rows) {
      const key = String(row.calendar_event_id);
      if (mappings.has(key)) throw new Error("The current target has duplicate entry mappings.");
      mappings.set(key, row);
    }
    const live = new Set(entries.map((entry) => String(entry.id)));
    const removals = rows.filter((row) => !live.has(String(row.calendar_event_id)) || row.deleted_locally_at).map((row) => String(row.calendar_event_id));
    return { entries, mappings, removals };
  }

  /** Seconds this house is still being held back, from the DURABLE record. */
  async persistedBackoffSeconds(restaurantId: string): Promise<number | null> {
    const { data, error } = await this.db.client
    ...
    return Number.isFinite(elapsed) && Number.isFinite(seconds) ? Math.max(0, Math.ceil(seconds - elapsed)) : null;
  }
  ...
  private async targetFor(
    ...
    const { data, error } = await this.db.client
      ...
      .select(
        "id, provider_calendar_id, provider_calendar_summary, time_zone, created_at, creation_state",
      )
      .eq("restaurant_id", restaurantId)
  ...
  private async markPushed(
    ...
    if (error) {
      ...
      throw new Error("Google accepted the entry, but its delivery receipt could not be recorded.");
    }
  }
  ...
  private eventBody(
    ...
    let endDate = String(entry.end_date ?? entry.start_date);
    if (!allDay) {
      if (!calendarZone) throw new Error("Set the house timezone before copying a timed entry to Google.");
      if (!entry.start_time) throw new Error("A timed calendar entry needs its recorded start time.");
      const endTime = padTime(String(entry.end_time ?? plusMinutes(String(entry.start_time), DEFAULT_EVENT_MINUTES)));
      if (endDate === startDate && endTime <= padTime(String(entry.start_time))) endDate = addDays(endDate, 1);
    }
...
function toTarget(row: Record<string, unknown>): Target {
  return {
    id: String(row.id),
    providerCalendarId: typeof row.provider_calendar_id === "string" ? row.provider_calendar_id : "",
    creationState: typeof row.creation_state === "string" ? row.creation_state : "ready",
...
function buildDescription(entry: Record<string, unknown>): string {
  ...
  lines.push(
    "Written by Mudavym. Edit this in Mudavym — changes made here are not read back, and deleting this copy restores it on its next attempted update. Unchanged copies are not checked in Google.",
  );
  return lines.join("\n\n");
}
