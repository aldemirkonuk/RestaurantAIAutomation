...
describe("DistributorDiscoveryController", () => {
  ...
  beforeEach(async () => {
    service = {
      search: jest
        .fn()
        .mockResolvedValue({ data: [], total: 0, limit: 50, offset: 0 }),
  ...
  it("takes the restaurant from the authenticated user", async () => {
    ...
    expect(service.search).toHaveBeenCalledWith(user.restaurantId, {
      q: "burgundy",
    });
  });
  ...
  it("passes facet filters through to the service", async () => {
    ...
    expect(service.facetCounts).toHaveBeenCalledWith(user.restaurantId, {
      territoryOnly: false,
    });
  });
  ...
  it("surfaces unexpected failures as a 500", async () => {
    ...
    await expect(controller.search(user, {})).rejects.toBeInstanceOf(
      HttpException,
    );
    await expect(controller.search(user, {})).rejects.toMatchObject({
      status: 500,
    });
  });
});
