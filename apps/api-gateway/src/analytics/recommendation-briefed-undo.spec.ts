...
describe("which patches are a note change", () => {
  it("taking a briefing back is one; stamping it is not", () => {
    expect(notesTouchedBy({ acted: false })).toEqual(["acted"]);
    expect(notesTouchedBy({ acted: true })).toEqual([]);
    expect(touchesNotes({ acted: false })).toBe(true);
    expect(touchesNotes({ acted: true })).toBe(false);
  });
});

describe("mark as briefed: the tap is the act, and its Undo is gated like a pin", () => {
  it("staff mark a card briefed: acted_at is stamped and names them, no note audit", async () => {
    ...
    expect(out.noteAudit).toBeNull();
    expect(auditWrites(t.writes)).toEqual([]);
  });
