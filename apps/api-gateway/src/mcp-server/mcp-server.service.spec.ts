...
describe("McpServerService — tenancy", () => {
  ...
  it("never forwards a correlation id to the timeline, because event_store is not house-scoped", async () => {
    // `LogsTimelineService.getTimeline` reads `event_store` by correlation id
    // alone. Offering that argument would let a key name another house's events.
    const timelineTool = READ_TOOLS.find((t) => t.name === "logs.timeline");
    const props = Object.keys(
      (timelineTool?.inputSchema as { properties?: Record<string, unknown> })
        .properties ?? {},
    );
    expect(props).toEqual(["limit"]);

    const timelineRead = jest.fn().mockResolvedValue({
      value: { events: [] },
      provenance: { readAt: "2026-09-12T00:00:00.000Z", rows: 0, source: "x" },
    });
    const { service } = build({ timelineRead } as never);
    await service.handle(
      {
        ...
        params: {
          name: "logs.timeline",
          arguments: { limit: 5, correlationId: "another-house-correlation" },
        },
      },
      credential(),
    );
    ...
    expect(timelineRead).toHaveBeenCalledWith(HOUSE_A, { limit: 5 });
  });
