...
export class ScanParserService {
  ...
  async parse(imageBase64: string): Promise<WineExtractItem[]> {
    ...
    try {
      ...
      throw new ServiceUnavailableException('Menu scan service temporarily unavailable');
    }
  }

  private detectMediaType(base64: string): 'image/jpeg' | 'image/png' | 'image/webp' | 'image/gif' {
  ...
  private parseJsonResponse(text: string): WineExtractItem[] {
    try {
      ...
      this.logger.warn(`Failed to parse LLM JSON response: ${text.slice(0, 200)}`);
      return [];
    }
  }
}
