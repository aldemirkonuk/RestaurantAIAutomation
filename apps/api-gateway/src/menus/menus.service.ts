...
export class MenusService {
  ...
  async importMenu(
    dto: ImportMenuDto,
    userId: string,
  ): Promise<{ menuId: string; itemsExtracted: number; submissionsCreated: number }> {
    ...
    if (items.length > 0) {
      const menuItemRows = items.map((item, idx) => ({
        ...
        source: dto.method,
        status:
          dto.method === 'manual' && idx >= FREE_TIER_MANUAL_LIMIT ? 'flagged' : 'approved',
        review_notes:
    ...
    this.submitToWineLibrary(items, restaurantId, userId, menu.id).catch((err) =>
    ...
    return { menuId: menu.id, itemsExtracted: items.length, submissionsCreated: items.length };
  }
  ...
  private async upsertMenu(restaurantId: string): Promise<{ id: string }> {
    ...
    if (error || !created) {
      throw new Error(`Failed to create restaurant_menus row: ${error?.message}`);
    }
  ...
  private async addToInventory(items: WineExtractItem[], restaurantId: string): Promise<void> {
    if (items.length === 0) return;
    ...
    const { error } = await this.dbService.supabase.from('inventory').insert(inventoryRows);
    if (error) throw new Error(error.message);
  }
  ...
  async getOnboardingProgress(userId: string) {
    ...
    if (!data) throw new NotFoundException('Onboarding progress not found for this user');
    return data;
  }

  async updateOnboardingProgress(userId: string, dto: UpdateOnboardingProgressDto) {
    const updates: Record<string, boolean | string> = {};
    if (dto.menu_uploaded !== undefined) updates.menu_uploaded = dto.menu_uploaded;
    if (dto.vendor_added !== undefined) updates.vendor_added = dto.vendor_added;
    if (dto.team_member_invited !== undefined) updates.team_member_invited = dto.team_member_invited;
