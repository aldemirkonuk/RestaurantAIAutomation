...
describe("spend tiers", () => {
  it("core is a depleting credit, not a daily reset", () => {
    ...
    expect(a.limitUsd).toBe(5);
    expect(a.mode).toBe("credit");
    expect(windowStartIso("credit")).toBeNull(); // sums ALL TIME
  });
  it("plus and pro are daily ceilings that reset", () => {
    expect(allowanceForTier("plus")).toMatchObject({
      limitUsd: 5,
      mode: "daily",
    });
    expect(allowanceForTier("pro")).toMatchObject({
      limitUsd: 10,
      mode: "daily",
    });
  ...
  it("live default 'pilot' and unknown tiers resolve to core, never the largest", () => {
    for (const t of [
      ...
      if (String(t).trim().toLowerCase() === "pro") {
        expect(a.limitUsd).toBe(10);
        continue;
      }
      expect(a.mode).toBe("credit");
      expect(a.limitUsd).toBe(5);
