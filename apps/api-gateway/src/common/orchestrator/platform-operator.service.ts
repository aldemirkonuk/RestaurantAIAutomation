  ...
  async isOperator(userId: string | undefined): Promise<boolean> {
    ...
    const [grant, role] = await Promise.all([
      this.database.client.from("platform_operator_grants").select("enabled")
        .eq("user_id", userId).eq("enabled", true).is("revoked_at", null).maybeSingle(),
...
/**
 * ADR 0143 §2 / ADR 0149 row 10: the operations desk's health read is for house
 * owners and platform operators, not managers or staff — deliberately narrower
 * than `ProtectedRoute requiredRole="owner"` on the frontend, which (elsewhere
 * in this app) treats "owner" and "manager" as equivalent. Enforced here, not
 * only in the browser: on `origin/main` these routes carried only
 * `JwtAuthGuard`, so any authenticated member of ANY house — not just this
 * one's owner — could read the platform-wide agent roster.
 */
@Injectable()
export class OwnerOrPlatformOperatorGuard implements CanActivate {
  constructor(private readonly operators: PlatformOperatorService) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const user = context.switchToHttp().getRequest().user;
    if (user?.role === "owner") return true;
    if (await this.operators.isOperator(user?.userId)) return true;
    throw new ForbiddenException(
      "This reading is available to house owners and platform operators.",
    );
  }
}
