...
export class SenderTrustController {
  ...
  @Post('trust')
  async setTrust(
    @Body() body: { domain?: string; email?: string; trusted?: boolean; providerId?: string },
    @CurrentUser() user: { userId: string; restaurantId: string },
  ): Promise<{ domain: string; trusted: boolean }> {
    const target = (body?.domain || body?.email || '').trim();
    if (!target) throw new HttpException('domain or email is required', HttpStatus.BAD_REQUEST);
    const trusted = body?.trusted !== false;
    try {
      const domain = await this.senderReputation.setTrust(user.restaurantId, target, trusted, body?.providerId ?? null);
      return { domain, trusted };
    } catch (error: any) {
      throw new HttpException(error.message || 'Failed to update sender trust', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async list(@CurrentUser() user: { userId: string; restaurantId: string }): Promise<any[]> {
    try {
      ...
      throw new HttpException(error.message || 'Failed to load sender reputation', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
