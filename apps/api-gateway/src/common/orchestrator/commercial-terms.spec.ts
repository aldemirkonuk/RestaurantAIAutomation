...
describe("commercial-terms", () => {
  ...
  describe("parseCommercialTerms", () => {
    it("coerces currency symbols and money strings to numbers", () => {
      ...
      expect(t.discount_tiers[0]).toMatchObject({
        threshold_qty: 24,
        discount_pct: 5,
      });
    });
  ...
  describe("validateCommercialTerms", () => {
    ...
    it("flags MOQ not met when the minimum exceeds what we asked for", () => {
      expect(
        validateCommercialTerms(parseCommercialTerms(exampleRaw), 4)
          .moq_not_met,
      ).toBe(true);
      expect(
        validateCommercialTerms(parseCommercialTerms(exampleRaw), 6)
          .moq_not_met,
      ).toBe(false);
      expect(
        validateCommercialTerms(parseCommercialTerms(exampleRaw), 0)
          .moq_not_met,
      ).toBe(false); // unknown qty
    });

    it("flags unknown tax basis", () => {
      expect(
        validateCommercialTerms(parseCommercialTerms({ unit_price: 135 }), 6)
          .tax_status_unknown,
      ).toBe(true);
      expect(
        validateCommercialTerms(parseCommercialTerms(exampleRaw), 6)
    ...
    it("flags ambiguous currency when the model marks it", () => {
      const v = validateCommercialTerms(
        parseCommercialTerms({ currency: "USD", currency_ambiguous: true }),
        6,
      );
