...
export class PromotionExtractorService {
  ...
  async extractAndStore(
    ...
    try {
      ...
      const likelyPromo =
        (t != null && (t.bulk || t.listMail || t.esp != null)) ||
        looksPromotional(ctx.subject, ctx.body);
      ...
      const daysToExpiry =
        promo.valid_until != null
          ? (new Date(promo.valid_until).getTime() - Date.now()) / 86_400_000
          : null;
      const urgency =
        daysToExpiry != null && daysToExpiry <= 3
          ...
          : daysToExpiry != null && daysToExpiry <= 7
            ? 0.6
            : promo.promo_type === "closeout"
              ? 0.7
              : 0.2;
      const savings =
        promo.discount_pct != null
          ? Math.min(1, promo.discount_pct / 30)
          : promo.discount_amount != null
            ...
            : promo.free_shipping
              ...
              : 0.2;
      const relevance =
        applicableWines.length > 0
          ? Math.min(1, 0.5 + applicableWines.length * 0.2)
          : 0.1;
      const trust =
        ctx.transport?.senderVerified === true
          ? 0.7
          : ctx.transport?.senderVerified === false
            ? 0.3
            : 0.5;
      const { bucket } = computePriority({
        relevance,
        savings,
        urgency,
        trust,
      ...
      if (promo.threshold_amount != null)
        conditions.min_amount = promo.threshold_amount;
      ...
      if (error) {
        this.logger.warn(
          `PromotionExtractor: insert failed — ${error.message}`,
        );
  ...
  async sendDailyDigests(): Promise<void> {
    try {
      ...
      const { data } = await this.databaseService.supabase
        .from("provider_promotions")
        .select(
          "restaurant_id, promo_type, discount_value, providers!left(name)",
        )
      ...
      if (byRestaurant.size)
        this.logger.log(
          `Promo digest sent to ${byRestaurant.size} restaurant(s).`,
        );
    } catch (e: any) {
  ...
  private digestLine(row: any): string | null {
    ...
    else if (dv.amount != null)
      offer = `${dv.currency === "EUR" ? "€" : dv.currency === "GBP" ? "£" : "$"}${dv.amount} off`;
  ...
  private async matchApplicableWines(
    restaurantId: string,
    text: string,
  ): Promise<string[]> {
    try {
