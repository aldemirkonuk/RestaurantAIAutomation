...
function stubDb(opts: {
  ...
  const builder: any = {
    ...
    in: (col: string, val: unknown) => { filters.push(["in", col, val]); return builder; },
    order: (col: string, options: unknown) => { filters.push(["order", col, options]); return builder; },
    limit: () =>
      opts.error
...
describe("ProcurementService.lastAgreementFor", () => {
  it("scopes by restaurant, vendor and item — all three", async () => {
    ...
    expect(eqs).toContain("procurement_order_items.inventory_id=inv-9");
    expect(eqs).toContain("provider_id=prov-1");
    expect(eqs).toContain("procurement_order_items.restaurant_id=rest-A");
  });

  it("orders the parent agreements by their own date, excluding unconfirmed requests", async () => {
    const db = stubDb({ rows: [] }); await callLastAgreement(db);
    expect(db.filters).toContainEqual(["order", "requested_at", { ascending: false, nullsFirst: false }]);
    const statuses = db.filters.find((f) => f[0] === "in" && f[1] === "status")?.[2] as string[];
    expect(statuses).toContain("CONFIRMED"); expect(statuses).not.toContain("PENDING"); expect(statuses).not.toContain("CANCELLED");
  });
  ...
  it("reads the matched line whether it arrives as an object or an array", async () => {
    const line = {
    ...
    const asObject = await callLastAgreement(
      stubDb({ rows: [{ ...parent, procurement_order_items: line }], vendorName: "Banfi" }),
    );
  ...
  it("calls two lines for the same item on one order ambiguous, not unreadable (lane E audit D9)", async () => {
    const line = {
      final_unit_price: 2400,
      price_uom: "case",
      price_pack_size: 12,
      currency: "TRY",
      unit_type: "case",
    ...
    const a = await callLastAgreement(
      stubDb({
        rows: [
          {
            order_number: "PO-77",
            requested_at: "2026-07-04T00:00:00.000Z",
            procurement_order_items: [line, { ...line, final_unit_price: 2500 }],
          },
        ],
        vendorName: "Banfi",
      }),
    );
    expect(a.state).toBe("ambiguous");
    expect(a.sentence).toContain("more than once");
    // Ambiguity is not a read failure: the two sentences must stay apart.
    const unreadable = await callLastAgreement(stubDb({ error: "timeout" }));
    expect(unreadable.state).toBe("unreadable");
    expect(a.sentence).not.toEqual(unreadable.sentence);
    // No price is offered for an answer with no single line behind it.
    expect(a.price).toBeUndefined();
  });
});

describe("lastAgreementAnswer — the ambiguous state", () => {
  it("is distinct from unreadable, with its own sentence", () => {
    const ambiguous = lastAgreementAnswer(null, false, "Banfi", true);
    const unreadable = lastAgreementAnswer(null, true, "Banfi");
    expect(ambiguous.state).toBe("ambiguous");
    expect(ambiguous.sentence).toContain("more than once");
    expect(ambiguous.sentence).not.toContain("could not be read");
    expect(ambiguous.sentence).not.toEqual(unreadable.sentence);
  });
});
