...
import { ReadingCommitService, ReadingSealContext } from "../ask-readings/reading-commit.service";
import {
  policyNote,
...
export class ProcurementService {
  ...
  constructor(
    ...
    @Optional()
    private readonly readingCommit?: ReadingCommitService,
  ) {}
  ...
  async createOrder(
    ...
    provenance?: {
      source: OrderSource;
      /** Server-owned folio only; never a client DTO field. */
      readingFolioId?: string;
    ...
    if (provenance?.readingFolioId) {
      if (!this.readingCommit) throw new InternalServerErrorException("The draft's Reading cannot be checked. No order was created.");
      await this.readingCommit.assertCurrent(restaurantId, provenance.readingFolioId);
    }
    ...
    if (!fulfilled && !recurrence && !provenance?.readingFolioId) {
    ...
    const payload = {
      ...
      ask_reading_folio_id: provenance?.readingFolioId ?? null,
  ...
  async lastAgreementFor(
    ...
    try {
      ...
      if (lines.length !== 1) return lastAgreementAnswer(null, true, vendorName);
      const row = lines[0];
  ...
  private async readOrderSealArgs(
    ...
    const { data, error } = await this.databaseService.supabase
      .from("procurement_orders")
      .select("id, total_cost, provider_id, ask_reading_folio_id")
      .eq("restaurant_id", restaurantId)
      .eq("id", orderId)
      .maybeSingle();
    ...
    const row = data as {
      ...
      total_cost: string | number | null;
      provider_id: string | null;
      ask_reading_folio_id?: string | null;
    };
    const reading = row.ask_reading_folio_id
      ? await this.requireReadingCommit().assertCurrent(restaurantId, row.ask_reading_folio_id) : null;
  ...
  private requireReadingCommit(): ReadingCommitService {
    if (!this.readingCommit) throw new InternalServerErrorException("The draft's Reading cannot be checked. Nothing was approved or sent.");
    return this.readingCommit;
  }

  private async readDraftReadingContext(restaurantId: string, orderId: string): Promise<ReadingSealContext | null> {
    const { data, error } = await this.databaseService.supabase.from("procurement_orders")
      .select("id, ask_reading_folio_id").eq("id", orderId).eq("restaurant_id", restaurantId).maybeSingle();
    if (error) throw new InternalServerErrorException("The order's Reading could not be checked. Nothing was sent.");
    if (!data) throw new NotFoundException("This order is not available here.");
    return (data as any).ask_reading_folio_id
      ? this.requireReadingCommit().assertCurrent(restaurantId, (data as any).ask_reading_folio_id) : null;
  }
  ...
  async issueDraftSendSeal(
    ...
    const readingContext = await this.readDraftReadingContext(restaurantId, orderId);
    const issued = await this.sealChallenges.issue({
      restaurantId,
      actorUserId: userId,
      subjectKind: "procurement_order",
      ...
      args: { ...draftSealArgs({ body: letter.body, to: recipient, cc: letter.cc }), draftId: pending.id, ...(readingContext || {}) },
    });
    return {
      challenge: issued.challenge,
      expiresAt: issued.expiresAt,
  ...
  async sendDraftedReply(
    ...
    await this.organizations.assertCanManageRestaurant(userId, restaurantId, "send a drafted reply");
    return this.approveDraft(restaurantId, orderId, dto, { userId, challenge });
  }
  ...
  async approveDraft(
    ...
    const readingContext = await this.readDraftReadingContext(restaurantId, orderId);
    if (readingContext && !seal) throw new ForbiddenException("This Mudavym draft needs a person's held approval. Nothing was sent.");
    if (seal) {
      ...
      await this.sealChallenges.redeem({
        ...
        action: ORDER_SEND_DRAFT_ACT,
        args: { ...draftSealArgs({ body: rawEmailBody, to: providerEmail, cc: dto.ccEmails }), draftId: conversationId, ...(readingContext || {}) },
    ...
    for (const row of due) {
      ...
      try {
        const { data: order, error: orderError } = await this.databaseService.supabase
          .from("procurement_orders")
          .select(
            "id, status, ai_autonomy_paused, ask_reading_folio_id, providers!left(contact_email, name, contact_first_name, primary_contact), restaurant_inventory:inventory_id(wine_name)",
          )
          .eq("id", row.order_id)
          .eq("restaurant_id", row.restaurant_id)
          .single();
        if (orderError || !order) throw new Error("The scheduled reply's owned order could not be checked.");
        if ((order as any).ask_reading_folio_id) {
          await this.revertScheduledToDraft(row.id,
            "Mudavym's Reading-backed draft requires personal held approval after current evidence is checked");
          continue;
        }
