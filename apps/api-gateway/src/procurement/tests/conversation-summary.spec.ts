...
describe('getOrderConversations row mapping — direction field', () => {
  ...
  it('returns rollingSummary when present', () => {
    const summary = 'Provider agreed to $42/bottle, 6 bottles, delivery in 5 days.';
  ...
  it('maps draftContent from content field (preferred over message_text)', () => {
    const row = makeRow({ content: 'Content field value', message_text: 'Fallback text' });
...
describe('Negotiation approval summary visibility logic (handleApprove)', () => {
  it('hasNegotiation=true when at least one INBOUND conversation exists', () => {
    const conversations: Array<{ direction: string; status: string; rollingSummary: string | null }> = [
  ...
  it('hasNegotiation=false when all conversations are OUTBOUND', () => {
    const conversations: Array<{ direction: string; status: string; rollingSummary: string | null }> = [
      { direction: 'OUTBOUND', status: 'SENT', rollingSummary: null },
      { direction: 'OUTBOUND', status: 'PENDING_APPROVAL', rollingSummary: null },
    ];
    const hasProviderReply = conversations.some(c => c.direction === 'INBOUND');
  ...
  it('picks the most recent outbound rolling summary', () => {
    const conversations = [
      { direction: 'OUTBOUND', status: 'SENT', rollingSummary: 'Round 1 summary' },
      { direction: 'INBOUND', status: 'SENT', rollingSummary: null },
      { direction: 'OUTBOUND', status: 'PENDING_APPROVAL', rollingSummary: 'Round 2 summary' },
    ];
  ...
  it('skips INBOUND entries when looking for rolling summary', () => {
    const conversations = [
      { direction: 'OUTBOUND', status: 'SENT', rollingSummary: 'Outbound summary' },
      { direction: 'INBOUND', status: 'SENT', rollingSummary: 'Provider reply text' },
    ];
...
describe('OrderApprovalModal summary section visibility', () => {
  it('shows summary section when hasNegotiation=true AND conversationSummary is non-empty', () => {
    const orderData = { hasNegotiation: true, conversationSummary: 'Provider agreed to $40/bottle.' };
    const shouldShow = !!(orderData.hasNegotiation && orderData.conversationSummary);
  ...
  it('hides summary section when orderData has no hasNegotiation field (undefined)', () => {
    const orderData = { hasNegotiation: undefined, conversationSummary: 'Some summary' };
    const shouldShow = !!(orderData.hasNegotiation && orderData.conversationSummary);
    expect(shouldShow).toBe(false);
  });
});
