...
export class RecurringOrdersService {
  ...
  async createRecurringOrder(
    ...
    this.logger.log(`Recurring order created: ${data.id} (${template.frequency})`);

    // Pre-create calendar events for the next 12 months
    try {
      await this.preCreateCalendarEvents(data);
    } catch (calErr) {
      this.logger.warn(`Failed to pre-create calendar events: ${calErr?.message}`);
    }
    ...
    if (!template.auto_approve) {
      try {
        ...
        this.logger.warn(`Failed to publish approval_needed event: ${pubErr?.message}`);
      }
    }
  ...
  async getRecurringOrder(restaurantId: string, id: string) {
    ...
    if (error) {
      this.logger.error(`Failed to get recurring order ${id}: ${error.message}`);
      throw error;
    }
  ...
  async updateRecurringOrder(
    ...
    if (error) {
      this.logger.error(`Failed to update recurring order ${id}: ${error.message}`);
      throw error;
    }
  ...
  async deleteRecurringOrder(restaurantId: string, id: string) {
    ...
    if (error) {
      this.logger.error(`Failed to deactivate recurring order ${id}: ${error.message}`);
      throw error;
    }
  ...
  async executeDueRecurringOrders(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        this.logger.error(`Failed to query due recurring orders: ${error.message}`);
        return;
      }
      ...
      this.logger.log(`Found ${dueOrders.length} recurring orders due for execution`);
  ...
  async sendRecurringOrderReminders(): Promise<void> {
    ...
    try {
      const { data: upcomingOrders, error } = await this.databaseService.supabase
        .from('recurring_orders')
        .select('*')
        .eq('active', true)
      ...
      if (error) {
        this.logger.warn(`Failed to query upcoming recurring orders: ${error.message}`);
        return;
      }
      ...
      for (const order of upcomingOrders as RecurringOrderRow[]) {
        try {
          ...
          this.logger.warn(`Failed to send reminder for ${order.id}: ${err?.message}`);
        }
      }
    } catch (err) {
      this.logger.error(`Recurring order reminder cron failed: ${err?.message}`);
    }
  }
  ...
  private async executeRecurringOrder(recurringOrder: RecurringOrderRow): Promise<void> {
    this.logger.log(
    ...
    try {
      ...
      this.logger.warn(`Calendar event update failed for recurring order: ${calErr?.message}`);
    }
    ...
    try {
      ...
      this.logger.warn(`Failed to publish recurring order event: ${pubErr?.message}`);
    }
  ...
  private async preCreateCalendarEvents(recurringOrder: any): Promise<void> {
    ...
    while (true) {
      ...
      currentDate = this.calculateNextOrderDate(currentDate, frequency, frequencyDay);
    }
    ...
    if (error) {
      this.logger.warn(`Failed to bulk-insert calendar events: ${error.message}`);
    } else {
      this.logger.log(`Pre-created ${events.length} calendar events for recurring order ${recurringOrder.id}`);
    }
  }
    ...
    switch (frequency) {
      ...
      case 'monthly': {
        ...
        if (frequencyDay !== undefined && frequencyDay >= 1 && frequencyDay <= 28) {
          date.setDate(Math.min(frequencyDay, this.daysInMonth(date.getFullYear(), date.getMonth())));
        }
        break;
      }
      case 'quarterly': {
