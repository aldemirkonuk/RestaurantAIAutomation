 ...
 *   ambiguous  — the read SUCCEEDED and named more than one line (lane E audit
 *                D9: the latest order has two lines for the same item, so
 *                which one is "the" agreement is a real question, not a fault
 *                worth retrying). Not "unreadable" — nothing was broken, and
 *                telling a manager to "try again" for a shape that will not
 *                change on retry is its own kind of dishonest absence.
 *
 * The third and fourth are the whole reason this file is not four lines. A
 * failed read rendered as "no agreement on file" is the fault this house calls
 * *absence reported as health*: the sheet would quietly ask a vendor to quote a
 * wine it has an agreed price for, and nobody downstream could tell. An
 * ambiguous read rendered as a failed one tells a manager to retry a network
 * call for what is actually a data question this file cannot answer alone.
 *
...
export type LastAgreementState = "found" | "none" | "unreadable" | "ambiguous";
 ...
 * exists to make impossible. `ambiguous` means it ran, found more than one
 * candidate line, and this function was handed none of them rather than a
 * silent pick — the caller decides how to choose, this file only refuses to
 * call that outcome a failure.
 */
export function lastAgreementAnswer(
  ...
  if (ambiguous) {
    return {
      state: "ambiguous",
      sentence:
        `The last order with ${vendor} named this item more than once, so ` +
        `there is no single last agreement to show. State the price yourself, ` +
        `or open the order to read both lines.`,
    };
  }
