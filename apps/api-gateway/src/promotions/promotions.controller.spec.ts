...
describe("PromotionsController", () => {
  ...
  it("refuses a session that names no house before any read (PR #416's shape)", async () => {
    await expect(controller.read({ userId: "u-1" }, undefined)).rejects.toMatchObject({ status: 403 });
    await expect(controller.dismiss({ userId: "u-1", restaurantId: null }, offerId)).rejects.toMatchObject({ status: 403 });
    await expect(controller.restore({}, offerId)).rejects.toMatchObject({ status: 403 });
    expect(service.readForHouse).not.toHaveBeenCalled();
    expect(service.dismiss).not.toHaveBeenCalled();
    expect(service.restore).not.toHaveBeenCalled();
  });

  it("refuses a dismissal whose token names no person, so dismissed_by is never anonymous", async () => {
    await expect(controller.dismiss({ restaurantId: user.restaurantId }, offerId)).rejects.toMatchObject({ status: 403 });
    expect(service.dismiss).not.toHaveBeenCalled();
  });
